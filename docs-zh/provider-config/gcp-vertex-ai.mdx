...
3. **重启 VS Code：**\
   确保重启 VS Code，以便 <BrandName /> 扩展获取新凭据
...
4. **设置环境变量：**
    ...
    - 这会指示 Google Cloud 客户端库（和 <BrandName />）使用此密钥
...
通过遵循这些步骤，您的企业团队可以安全地将 GCP Vertex AI 与 <BrandName /> VS Code 扩展集成，以利用 **Claude 3.5 Sonnet v2** 的强大功能：
...
-   **在 VS Code 中配置 <BrandName />：**\
    安装 <BrandName />，输入您的项目 ID，选择适当的区域，并选择模型
-   **设置身份验证：**\
...
_本指南将随着 GCP Vertex AI 和 <BrandName /> 的发展而更新。请始终参考最新文档以了解当前实践。_
