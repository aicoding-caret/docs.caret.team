---
title: "Cerebras"
description: "了解如何配置和使用 Cerebras 超快推理与 Careti。体验晶圆级芯片架构和实时推理模型的每秒高达 2,600 个 token。"
---
...
3.  **创建密钥：** 生成新的 API 密钥。给它一个描述性名称（例如"<BrandName />"）。
4.  **复制密钥：** 立即复制 API 密钥。安全存储。

### 支持的模型

<BrandName /> 支持以下 Cerebras 模型：
...
#### 无 IDE 锁定
适用于任何 OpenAI 兼容工具——Cursor、Continue.dev、<BrandName /> 或任何其他支持 OpenAI 端点的编辑器。
