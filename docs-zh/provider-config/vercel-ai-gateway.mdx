...
Vercel AI Gateway 为您提供一个 API 来访问来自多个提供商的模型。您只需通过模型 ID 切换，无需交换 SDK 或处理多个密钥。<BrandName /> 直接集成，因此您可以在下拉菜单中选择 Gateway 模型，像使用任何其他提供商一样使用它，并在流中查看 token 和缓存使用情况。
...
4. 从列表中选择一个模型。<BrandName /> 会自动获取目录。您也可以粘贴确切的 ID
...
Gateway 支持大量且不断变化的模型集。<BrandName /> 从 Gateway API 拉取列表并在本地缓存。有关当前目录，请参阅 https://vercel.com/ai-gateway/models
...
- 多模型评估 - 仅在 <BrandName /> 中交换模型 ID，并比较延迟和输出 tokens
