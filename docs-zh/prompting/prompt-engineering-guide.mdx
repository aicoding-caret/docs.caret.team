...
`.caretignore` 文件是一个项目级配置文件，告诉 Caret 在分析您的代码库时要忽略哪些文件和目录。类似于 `.gitignore`，它使用模式匹配来指定应从 Caret 的上下文和操作中排除哪些文件。
...
-   **开始新任务：** "Caret，让我们开始一个新任务。创建 `user-authentication.js`。我们需要使用 JWT 令牌实现用户登录。需求如下…"
-   **总结之前的工作：** "Caret，总结我们在上一个用户仪表板任务中所做的工作。我想要记录主要功能和未解决的问题。将其保存到 `caret_docs/user-dashboard-summary.md`。"
