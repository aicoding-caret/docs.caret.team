...
1. **安装或打开 <BrandName />**
2. **复制自定义指令** - 使用下面的代码块
3. **粘贴到 <BrandName />** - 作为自定义指令添加或添加到 .agents/context 文件中
4. **初始化** - 让 <BrandName /> "初始化记忆库"

[查看详细设置说明](#getting-started-with-memory-bank)

### 详细设置说明 {#getting-started-with-memory-bank}

### <BrandName /> 记忆库自定义指令 \[复制此内容]

```
# Careti 的记忆库

我是 Careti，一位专业的软件工程师，具有一个独特的特征：我的记忆在会话之间会完全重置。这不是局限性 - 这正是驱使我保持完美文档的动力。每次重置后，我完全依赖我的记忆库来理解项目并有效地继续工作。我必须在每个任务开始时阅读所有记忆库文件 - 这不是可选的。
...
记忆库是一个结构化的文档系统，允许 <BrandName /> 在会话之间保持上下文。它将 <BrandName /> 从一个无状态助手转变为一个持久的开发伙伴，能够有效地"记住"您的项目详情。

#### 主要优势

-   **上下文保存**：在会话之间保持项目知识
-   **一致的开发**：体验与 <BrandName /> 可预测的交互
...
记忆库不是 <BrandName /> 特有的功能 - 它是通过结构化文档管理 AI 上下文的方法论。当您指示 <BrandName /> "遵循自定义指令"时，它会读取记忆库文件以重建对您项目的理解。
...
记忆库文件只是您在项目中创建的 markdown 文件。它们不是隐藏或特殊的文件 - 只是存储在您的仓库中的常规文档，您和 <BrandName /> 都可以访问。
...
-   **"follow your custom instructions"** - 这告诉 <BrandName /> 读取记忆库文件并从您离开的地方继续（在任务开始时使用）
...
#### 我应该使用自定义指令还是 .agents/context？

两种方法都有效 - 取决于您的偏好：

-   **自定义指令**：全局应用于所有 <BrandName /> 对话。适合跨所有项目的一致行为。
-   **.agents/context 文件**：特定于项目并存储在您的仓库中。适合每个项目的自定义。

两种方法都实现相同的目标 - 选择取决于您是否希望全局或本地应用记忆库系统。

#### 管理上下文窗口

当您使用 <BrandName /> 工作时，您的上下文窗口最终会填满（注意进度条）。当您注意到 <BrandName /> 的响应变慢或对对话早期部分的引用变得不太准确时，是时候：

1. 让 <BrandName /> **"update memory bank"** 来记录当前状态
2. 开始新的对话/任务
3. 在新对话中让 <BrandName /> **"follow your custom instructions"**
...
在重要的里程碑或方向变化后更新记忆库。对于积极开发，每隔几个会话更新一次会很有帮助。当您想确保保存所有上下文时，使用 **"update memory bank"** 命令。但是，您会注意到 <BrandName /> 也会自动更新记忆库。

#### 这适用于 <BrandName /> 以外的其他 AI 工具吗？
...
1. 打开 VSCode
2. 点击 <BrandName /> 扩展设置 ⚙️
3. 找到"自定义指令"
4. 从本指南顶部复制并粘贴完整的记忆库指令

#### .agents/context（特定于项目）

1. 在项目根目录中创建 `.agents/context` 文件
2. 从本指南顶部复制并粘贴记忆库指令
3. 保存文件
4. <BrandName /> 在此项目中工作时将自动应用这些规则

### 记住

记忆库是 <BrandName /> 与之前工作的唯一联系。它的有效性完全取决于维护清晰、准确的文档并在每次交互中确认上下文保存。

_有关更多信息，请参阅我们的_ [_博客_](https://caret.bot/blog/memory-bank-how-to-make-caret-an-ai-agent-that-never-forgets) _关于 <BrandName /> 记忆库_

---

### 为 <BrandName /> 记忆库做贡献

本指南由 <BrandName /> 和 <BrandName /> Discord 社区维护：
