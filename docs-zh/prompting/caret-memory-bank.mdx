...
1. **安装或打开 Careti**
2. **复制自定义指令** - 使用下面的代码块
3. **粘贴到 Careti** - 作为自定义指令添加或添加到 .agents/context 文件中
4. **初始化** - 让 Careti "初始化记忆库"

[查看详细设置说明](#getting-started-with-memory-bank)

### 详细设置说明 {#getting-started-with-memory-bank}

### Careti 记忆库自定义指令 \[复制此内容]
...
记忆库是一个结构化的文档系统，允许 Careti 在会话之间保持上下文。它将 Careti 从一个无状态助手转变为一个持久的开发伙伴，能够有效地"记住"您的项目详情。

#### 主要优势

-   **上下文保存**：在会话之间保持项目知识
-   **一致的开发**：体验与 Careti 可预测的交互
...
记忆库不是 Careti 特有的功能 - 它是通过结构化文档管理 AI 上下文的方法论。当您指示 Careti "遵循自定义指令"时，它会读取记忆库文件以重建对您项目的理解。
...
记忆库文件只是您在项目中创建的 markdown 文件。它们不是隐藏或特殊的文件 - 只是存储在您的仓库中的常规文档，您和 Careti 都可以访问。
...
-   **"follow your custom instructions"** - 这告诉 Careti 读取记忆库文件并从您离开的地方继续（在任务开始时使用）
...
-   **自定义指令**：全局应用于所有 Careti 对话。适合跨所有项目的一致行为。
...
当您使用 Careti 工作时，您的上下文窗口最终会填满（注意进度条）。当您注意到 Careti 的响应变慢或对对话早期部分的引用变得不太准确时，是时候：

1. 让 Careti **"update memory bank"** 来记录当前状态
2. 开始新的对话/任务
3. 在新对话中让 Careti **"follow your custom instructions"**
...
在重要的里程碑或方向变化后更新记忆库。对于积极开发，每隔几个会话更新一次会很有帮助。当您想确保保存所有上下文时，使用 **"update memory bank"** 命令。但是，您会注意到 Careti 也会自动更新记忆库。

#### 这适用于 Careti 以外的其他 AI 工具吗？
...
2. 从本指南顶部复制并粘贴记忆库指令
3. 保存文件
4. Careti 在此项目中工作时将自动应用这些规则

### 记住

记忆库是 Careti 与之前工作的唯一联系。它的有效性完全取决于维护清晰、准确的文档并在每次交互中确认上下文保存。

_有关更多信息，请参阅我们的_ [_博客_](https://caret.bot/blog/memory-bank-how-to-make-caret-an-ai-agent-that-never-forgets) _关于 Careti 记忆库_

---

### 为 Careti 记忆库做贡献

本指南由 Careti 和 Careti Discord 社区维护：
