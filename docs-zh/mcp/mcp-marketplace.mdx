---
title: "MCP 简易指南"
description: "了解如何使用 MCP 市场发现、安装和配置 MCP 服务器，这些服务器通过附加工具和资源增强 Careti 的能力。"
---

## 什么是 MCP 服务器？

MCP 服务器是增强 <BrandName /> 能力的专门扩展。它们使 <BrandName /> 能够执行额外的任务，如获取网页、处理图像、访问 API 等等。
...
-   在 <BrandName /> 中，单击顶部工具栏中的"扩展"按钮（方形图标）
...
-   单击所选服务器的安装按钮
-   如果服务器需要 API 密钥（大多数都需要），<BrandName /> 将引导您完成：
    -   在哪里获取 API 密钥
    -   如何安全地输入它
-   服务器将自动添加到您的 MCP 设置中
...
-   安装完成时，<BrandName /> 将显示确认
-   在 <BrandName /> 的 MCP 设置 UI 中检查服务器状态

### 5. 使用新服务器

-   成功安装后，<BrandName /> 将自动集成服务器的功能
-   您将在 <BrandName /> 的系统提示中看到新工具和资源可用
-   只需要求 <BrandName /> 使用新服务器的功能
-   示例："使用 Perplexity 搜索网络以获取最新的 React 更新"

**企业用户：** 如果您在企业环境中使用 <BrandName />，请确保根据组织的安全策略获得安装第三方 MCP 服务器的权限。
...
-   服务器代码被克隆/安装到 `/Users/<username>/Documents/Careti/MCP/`
-   安装依赖项
...
-   <BrandName /> 检测配置更改
-   <BrandName /> 将您的服务器作为单独的进程启动
-   通过 stdio 或 HTTP 建立通信

### 4. 与 <BrandName /> 集成

-   您的服务器功能已添加到 <BrandName /> 的系统提示中
-   工具通过 `use_mcp_tool` 命令可用
-   资源通过 `access_mcp_resource` 命令可用
-   <BrandName /> 现在可以在用户提示时使用这些功能
...
-   尝试删除并重新安装服务器（从 `cline_mcp_settings.json` 文件和 `/Users/<username>/Documents/Careti/MCP/` 目录中删除）
-   使用终端直接运行带参数的命令。这将允许您看到 <BrandName /> 看到的相同错误

## MCP 服务器规则

<BrandName /> 已经知道您的活动 MCP 服务器及其用途，但是当您启用了许多 MCP 服务器时，定义何时使用每个服务器可能很有用。

利用 `.agents/context` 文件或自定义指令通过基于关键字的触发器支持智能 MCP 服务器激活，使 <BrandName /> 的工具选择更加直观和上下文感知。

### MCP 规则的工作原理

MCP 规则将您连接的 MCP 服务器分组到功能类别中，并定义触发关键字，当在与 <BrandName /> 的对话中检测到这些关键字时，会自动激活它们。
...
-   **上下文工具选择**：<BrandName /> 根据对话上下文选择适当的工具
-   **减少摩擦**：无需手动指定要使用的工具
...
当您编写"你能抓取这个网站吗？"时，<BrandName /> 检测"抓取"和"网站"作为触发器，自动选择与网页相关的 MCP 服务器。
...
将此添加到您的 `.agents/context` 文件或自定义指令中，使 <BrandName /> 的 MCP 服务器选择更加直观和上下文感知。
