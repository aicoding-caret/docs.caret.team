---
title: "MCP 服务器开发协议"
description: "此协议旨在简化使用 Careti 构建 MCP 服务器的开发过程。"
---

> 🚀 **构建并与世界分享您的 MCP 服务器。** 创建出色的 MCP 服务器后，将其提交到 [<BrandName /> MCP 市场](https://github.com/caret/mcp-marketplace)，使其可被数千名开发人员发现并一键安装。

## 什么是 MCP 服务器？

模型上下文协议（MCP）服务器通过赋予 AI 助手（如 <BrandName />）以下能力来扩展它们：
...
有效 MCP 服务器开发的核心是遵循结构化协议。该协议通过位于 MCP 工作目录**根目录**（/Users/your-name/Documents/Careti/MCP）的 `.agents/context` 文件实现。

### 使用 `.agents/context` 文件

`.agents/context` 文件是 <BrandName /> 在其所在目录工作时自动读取的特殊配置。这些文件：

-   配置 <BrandName /> 的行为并强制执行最佳实践
-   将 <BrandName /> 切换到专门的 MCP 开发模式
-   提供构建服务器的分步协议
-   实施安全措施，如防止过早完成
-   引导您完成规划、实施和测试阶段
...
当此 `.agents/context` 文件存在于您的工作目录中时，<BrandName /> 将：
...
首先，使用上述协议将 `.agents/context` 文件添加到 MCP 工作目录的根目录。此文件配置 <BrandName /> 在此文件夹中工作时使用 MCP 开发协议。

### 2. 从清晰的描述开始聊天

通过清晰地描述您想要构建的内容来开始您的 <BrandName /> 聊天。具体说明：
...
<BrandName /> 将自动在 PLAN 模式下启动，引导您完成规划过程：
...
帮助 <BrandName /> 构建 MCP 服务器的最有效方法之一是在开始时就分享官方 API 文档：
...
提供全面的 API 详细信息（端点、身份验证、数据结构）显著提高了 <BrandName /> 实现有效 MCP 服务器的能力。

## 理解两种模式

### PLAN 模式

在此协作阶段，您与 <BrandName /> 一起设计 MCP 服务器：
...
资源让您的 MCP 服务器在不执行代码的情况下向 <BrandName /> 公开数据。它们非常适合提供上下文，如文件、API 响应或 <BrandName /> 在对话期间可以引用的数据库记录。
...
资源使您的 MCP 服务器更具上下文感知能力，允许 <BrandName /> 访问特定信息而无需您复制/粘贴。有关更多信息，请参阅[官方文档](https://modelcontextprotocol.io/docs/concepts/resources)。
