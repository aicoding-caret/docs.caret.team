...
title: "工作流最佳实践"
sidebarTitle: "最佳实践"
description: "构建高效且稳定的 Careti 工作流的技巧与策略"
---
...
<BrandName /> 能理解项目上下文，是工作流设计的优秀搭档。

### 用工作流来生成工作流

写工作流没有想象中困难，甚至可以用工作流来创建。

先将 [create-new-workflow.md](https://github.com/caretive-ai/project-careti/blob/main/prompts/workflows/create-new-workflow.md) 保存到工作区（例如 `.agents/context/workflows/`）。

然后运行 `/create-new-workflow.md`，<BrandName /> 会按以下步骤引导你：
...
<Tip>
  **自动化工作记录：** 完成的任务是最佳工作流候选。任务结束后说“把刚才的过程变成工作流”，<BrandName /> 会分析对话并生成工作流文件。
</Tip>

项目级工作流放在 `.agents/context/workflows/`，全局工作流放在 `~/Documents/Careti/Workflows/`。同名时以项目工作流为准。
...
像写代码一样，解释“为什么”很重要。清晰的注释能让 <BrandName /> 和团队更理解你的意图。
