...
-   利用 Caret 的[内置工具](/zh/exploring-carets-tools/caret-tools-guide)，如 `ask_followup_question`、`read_file`、`search_files` 和 `new_task`
-   使用您已安装的命令行工具，如 `gh` 或 `docker`
-   引用外部 [MCP 工具调用](/zh/mcp/mcp-overview)，如 Slack 或 Whatsapp
-   将多个操作按特定顺序链接在一起
...
我的工作流程使用 `gh` 命令行工具和 Caret 内置的 `ask_followup_question` 来：
...
-   如果我说"是"，Caret 会自动使用 `gh` 命令批准 PR

这已经将我的 PR 审查过程从手动的多步骤操作转变为单个命令，为我提供做出明智决策所需的一切。

> 这只是工作流程文件的一个示例。您可以在我们的[提示存储库](https://github.com/caret/prompts)中找到更多灵感。
