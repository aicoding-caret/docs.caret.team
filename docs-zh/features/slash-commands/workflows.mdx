...
-   利用 Careti 的[内置工具](/zh/exploring-carets-tools/caret-tools-guide)，如 `ask_followup_question`、`read_file`、`search_files` 和 `new_task`
...
我的工作流程使用 `gh` 命令行工具和 Careti 内置的 `ask_followup_question` 来：
...
-   如果我说"是"，Careti 会自动使用 `gh` 命令批准 PR
