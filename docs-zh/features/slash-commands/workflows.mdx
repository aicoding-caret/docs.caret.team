...
工作流程与 [Caret 规则](/features/caret-rules) 共存。创建一个很简单：
...
-   利用 Caret 的[内置工具](/exploring-clines-tools/caret-tools-guide)，如 `ask_followup_question`、`read_file`、`search_files` 和 `new_task`
