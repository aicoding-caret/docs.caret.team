...
工作流程允许您定义一系列步骤来指导 <BrandName /> 完成重复性任务集，例如部署服务或提交 PR。

要调用工作流程，请在聊天中输入 `/[workflow-name.md]`。

## 如何创建和使用工作流程

工作流程与 [<BrandName /> 规则](/zh/features/caret-rules) 共存。创建一个很简单：


	<img src="https://storage.googleapis.com/cline_public_images/docs/assets/workflows.png" alt="Workflows tab in Careti" />


1. 创建一个包含 <BrandName /> 应采取步骤明确说明的 markdown 文件
...
-   利用 <BrandName /> 的[内置工具](/zh/exploring-carets-tools/caret-tools-guide)，如 `ask_followup_question`、`read_file`、`search_files` 和 `new_task`
...
2. 粘贴 PR 号码
3. 让 <BrandName /> 处理其他一切

我的工作流程使用 `gh` 命令行工具和 <BrandName /> 内置的 `ask_followup_question` 来：
...
-   如果我说"是"，<BrandName /> 会自动使用 `gh` 命令批准 PR
