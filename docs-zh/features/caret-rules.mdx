...
您可以通过单击规则选项卡中的 `+` 按钮来创建规则。这将在您的 IDE 中打开一个新文件，您可以使用它来编写规则。
	...
	<img src="https://storage.googleapis.com/cline_public_images/docs/assets/caret-rules.png" alt="创建规则" />
...
您还可以通过在聊天中使用 [`/newrule` 斜杠命令](/features/slash-commands/new-rule) 让 Caret 为您创建规则。
...
Caret 自动处理 `.clinerules/` 目录内的**所有 Markdown 文件**，将它们组合成统一的规则集。数字前缀（可选）有助于按逻辑顺序组织文件。
