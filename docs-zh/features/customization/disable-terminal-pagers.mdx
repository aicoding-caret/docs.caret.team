---
title: "在 Caret 会话中禁用终端分页器"
description: "当 Caret 执行命令时禁用 less 等分页器，实现非交互输出"
---

很多 CLI 工具(如 Git)会使用 `less` 作为分页器。Caret 执行命令时分页器会阻塞输出，影响自动化。可以仅在 Caret 会话中禁用分页器。

## 原理

Caret 会在它启动的终端中设置环境变量：

- `CLINE_ACTIVE` — 表示该 shell 由 Caret 启动

在 shell 启动脚本中检测该变量，仅对 Caret 会话生效。
