...
<Note>
本文基于 <BrandName /> v3.38.1 合并版本整理。如有存储位置/检查点/认证/路由差异，会以 `<Note>` 标注。
</Note>
...
在 <BrandName /> 中，几乎所有交互都发生在 **任务(Task)** 中。无论是新功能、修复、重构或探索代码库，都是一个任务。
...
<Note>
想系统学习提示词？可查看 [<BrandName /> Learn](https://clinelearn.com) 的 Prompting 模块。
</Note>
