...
<Note>
**第一次使用 Careti CLI？** 本示例假设你已完成 [安装指南](/zh/caret-cli/installation)。建议先看更简单的 [GitHub RCA 示例](/zh/caret-cli/samples/github-issue-rca)。
</Note>
...
- **已掌握 Careti CLI** ([安装指南](/zh/caret-cli/installation))
...
该工作流会检测 `@caret` 提及，启动 Careti CLI 实例进行分析并回复。
