...
使用 <BrandName /> CLI 自动分析 GitHub Issue 的示例。通过 <BrandName /> 自治 AI 获取、分析并定位根因，输出清晰可解析的结果，便于集成到工作流中。

<Note>
**第一次使用 <BrandName /> CLI？** 请先完成 [安装指南](/zh/caret-cli/installation) 并执行 `caret auth`。
</Note>
...
- **已安装并认证 <BrandName /> CLI** ([安装指南](/zh/caret-cli/installation))
...
1. 使用 <BrandName /> 构建自治 CLI 工具
2. 解析 <BrandName /> CLI JSON 输出
3. 编写灵活的自动化脚本
