...
**任务(Task)** - Careti 执行的单个工作(如“给 utils.js 添加测试”)。

**实例(Instance)** - 独立的 Careti 工作空间。每个实例一次只跑一个任务。
...
Careti 会在当前目录开启会话，进入 Plan 模式给出逐步计划。确认后执行：
...
`-y`(YOLO) 让 Careti 自动计划并执行，适合 CI/脚本。
