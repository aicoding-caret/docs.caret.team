---
title: "三大核心流程"
description: "Careti CLI 的三种使用方式：交互模式、无头自动化、多实例并行"
---

需要先理解两个概念：

**任务(Task)** - <BrandName /> 执行的单个工作(如“给 utils.js 添加测试”)。

**实例(Instance)** - 独立的 <BrandName /> 工作空间。每个实例一次只跑一个任务。
...
<BrandName /> 会在当前目录开启会话，进入 Plan 模式给出逐步计划。确认后执行：
...
`-y`(YOLO) 让 <BrandName /> 自动计划并执行，适合 CI/脚本。
