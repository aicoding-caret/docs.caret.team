...
### 内存库 + .agents/context
用于非平凡项目。内存库将项目知识作为 Markdown 捕获在您的存储库中。`.agents/context` 是版本控制的指令，使 Caret 的行为与您的团队保持一致。[了解更多关于内存库的信息](/zh/prompting/caret-memory-bank)和 [Caret 规则](/zh/features/caret-rules)。
