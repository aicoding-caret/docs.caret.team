...
description: "Automatisierte GitHub-Issue-Analyse mit der Careti CLI, um Ursachen zu identifizieren."
---

# GitHub Ursachenanalyse

Automatisierte GitHub-Issue-Analyse mit der Careti CLI. Dieses Skript nutzt die autonomen KI-Funktionen von Careti, um GitHub-Issues abzurufen, zu analysieren und die Ursachen zu identifizieren. Es gibt saubere, parsierbare Ergebnisse aus, die einfach in Ihre Entwicklungs-Workflows integriert werden können.

<Note>
**Neu bei Careti CLI?** Dieses Beispiel setzt voraus, dass Sie die [Installationsanleitung](https://docs.caret.bot/german/caret-cli/installation) bereits abgeschlossen und sich mit `caret auth` authentifiziert haben. Wenn Sie die Careti CLI noch nicht eingerichtet haben, beginnen Sie bitte zuerst dort.
</Note>
...
- **Careti CLI** installiert und authentifiziert ([Installationsanleitung](https://docs.caret.bot/german/caret-cli/installation))
- **Mindestens einen KI-Modellanbieter** konfiguriert (z. B. OpenRouter, Anthropic, OpenAI)
- **Grundlegende Vertrautheit** mit Careti CLI Befehlen
...
### Verwenden einer bestimmten Careti Instanz

Richten Sie sich an eine bestimmte Careti Instanz nach Adresse:
...
- Mehrere Careti Instanzen ausgeführt werden
- Ein Remote-Careti-Server verwendet wird
- Tests mit bestimmten Konfigurationen durchgeführt werden
</Warning>

<Note>
Das Skript erledigt automatisch alles: Abrufen des Issues, Analysieren mit Careti und Anzeigen der Ergebnisse. Die Analyse dauert in der Regel 30-60 Sekunden, abhängig von der Komplexität des Issues.
</Note>
...
- Unterstützt optionale Careti Instanzadressen
...
**2. `--mode act`**
   ...
   - Ermöglicht Careti die Verwendung von Tools (Dateien lesen, Befehle ausführen usw.)
...
Dieses Beispiel zeigt, wie Sie mit der Careti CLI ein autonomes GitHub-Issue-Analysetool erstellen:

1. **Erstellen autonomer CLI-Tools** mit den Funktionen von Careti
2. **Parsen strukturierter JSON-Ausgabe** von der Careti CLI
