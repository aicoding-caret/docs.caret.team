...
Workflows ermöglichen es Ihnen, eine Abfolge von Schritten zu definieren, um Caret durch wiederkehrende Aufgaben zu führen, wie zum Beispiel das Deployen eines Service oder das Einreichen eines PR.

Um einen Workflow aufzurufen, geben Sie `/[workflow-name.md]` im Chat ein.

## Wie man Workflows erstellt und verwendet

Workflows befinden sich direkt neben den [Caret Rules](/german/features/caret-rules). Das Erstellen eines Workflows ist unkompliziert:

<Frame>
	<img src="https://storage.googleapis.com/cline_public_images/docs/assets/workflows.png" alt="Workflows tab in Caret" />
</Frame>

1. Erstellen Sie eine Markdown-Datei mit klaren Anweisungen für die Schritte, die Caret ausführen soll
...
-   Die [integrierten Tools](/german/exploring-carets-tools/caret-tools-guide) von Caret wie `ask_followup_question`, `read_file`, `search_files` und `new_task` nutzen
...
3. Lassen Sie Caret den Rest erledigen

Mein Workflow nutzt das `gh` Command-line-Tool und die in Caret integrierte Funktion `ask_followup_question`, um:
...
-   Wenn ich "Ja" sage, genehmigt Caret den PR automatisch über den `gh` Befehl
