Caret Rules ermöglichen es Ihnen, Caret Anweisungen auf Systemebene zu geben. Betrachten Sie diese als eine dauerhafte Möglichkeit, Kontext und Präferenzen für Ihre Projekte oder global für jede Konversation einzubinden.

Caret Rules ermöglichen es Ihnen, Caret Anweisungen auf Systemebene zu geben. Betrachten Sie diese als eine dauerhafte Möglichkeit, Kontext und Präferenzen für Ihre Projekte oder global für jede Konversation einzubinden.

<Note>
Dies ist ein Referenzdokument für Caret. Es folgt dem Caret v3.38.1 Merge-Stand. Falls es spezifische Caret-Richtlinien gibt (Unterschiede bei der Verwendung von AGENTS.md, Speicherort/Gültigkeitsbereich von Regeln, Authentifizierung/Routing), werden diese im Text durch `<Note>` gekennzeichnet.
</Note>

<Note>
Caret Agent Standardization: Basierend auf `caret-docs/features/f06-agent-standardization.md` erfolgt die Standardisierung über `.agents/context` SoT + `AGENTS.md`-Hierarchie + `/init`-Scaffold. Legacy-Regelpfade werden nicht berücksichtigt.
</Note>
...
|------------------|------------------|-------|
| **Windows** | `Documents\Caret\Rules` | Verwendet den System-Ordner Documents |
| **macOS** | `~/Documents/Caret/Rules` | Verwendet den Benutzer-Ordner Documents |
| **Linux/WSL** | `~/Documents/Caret/Rules` | Kann auf einigen Systemen auf `~/Caret/Rules` ausweichen |

> **Hinweis für Linux/WSL-Benutzer**: Wenn Sie Ihre globalen Regeln nicht unter `~/Documents/Caret/Rules` finden, prüfen Sie `~/Caret/Rules`, da der Speicherort je nach Systemkonfiguration und Vorhandensein des Documents-Verzeichnisses variieren kann.

Sie können Caret auch eine Regel für Sie erstellen lassen, indem Sie den [`/newrule` Slash-Befehl](/german/features/slash-commands/new-rule) im Chat verwenden.
...
Der System-Prompt von Caret ist hingegen nicht benutzer-editierbar ([hier finden Sie ihn](https://github.com/caret/caret/blob/main/src/core/prompts/system.ts)). Für einen umfassenderen Einblick in Best Practices für Prompt Engineering besuchen Sie [diese Ressource](https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/overview).

### AGENTS.md Standard Support

Caret unterstützt zudem den [AGENTS.md](https://agents.md/) Standard als Fallback (zusätzlich zu Caret Rules), indem `AGENTS.md` Dateien im Root-Verzeichnis Ihres Workspaces automatisch erkannt werden. Dies ermöglicht es Ihnen, dieselbe Regeldatei über verschiedene AI-Coding-Tools hinweg zu verwenden.
...
### Tipps für das Schreiben effektiver Caret Rules
...
Caret verarbeitet automatisch **alle Markdown-Dateien** innerhalb des `.agents/context/` Verzeichnisses und fasst sie zu einem einheitlichen Satz von Regeln zusammen. Die numerischen Präfixe (optional) helfen dabei, Dateien in einer logischen Reihenfolge zu organisieren.
...
Das Ordner-System transformiert Ihre Caret Rules von einem statischen Dokument in ein dynamisches Wissenssystem, das sich an die wechselnden Kontexte und Anforderungen Ihres Teams anpasst.

### Regeln verwalten mit dem umschaltbaren Popover

Um die Verwaltung sowohl einzelner `.agents/context` Dateien als auch des Ordner-Systems noch einfacher zu gestalten, führt Caret v3.13 ein spezielles Popover-UI ein, das direkt über das Chat-Interface zugänglich ist.
