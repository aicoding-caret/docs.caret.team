...
Die Terminalintegration von Caret ermöglicht es Ihnen, Ihre Terminalausgabe direkt in Ihre Konversationen mit Caret zu bringen. Anstatt Fehlermeldungen oder Befehlsergebnisse zu kopieren und einzufügen, können Sie diese mit einem einfachen Rechtsklick im Terminal an Caret senden.
...
Wenn Sie im VS Code-Terminal arbeiten und eine Ausgabe sehen, die Sie mit Caret besprechen möchten:

1. Klicken Sie mit der rechten Maustaste in das Terminal
2. Wählen Sie im Kontextmenü "Zu Caret hinzufügen"
3. Die Terminalausgabe wird sofort an das Caret-Chatfenster gesendet
...
Anstatt den Kontext zum Caret-Chatfenster zu wechseln und eine Beschreibung des Problems einzugeben, können Sie die Terminalausgabe mit nur wenigen Klicks direkt an Caret senden.

Alternativ können Sie die [`@terminal`](/german/features/at-mentions/terminal-mentions)-Erwähnung verwenden, um die vollständige Terminalausgabe an Caret zu senden.
...
Wenn Sie die Rechtsklick-Terminalintegration verwenden, geht Caret wie folgt vor:
...
Dies gibt Caret den vollständigen Kontext, den es benötigt, um genaue Hilfe bei terminalbezogenen Problemen zu leisten.

## Hinter den Kulissen

Die Terminalintegration verwendet eine clevere Technik, um die Terminalausgabe zu erfassen:

1. Wenn Sie die Integration auslösen, Caret:
...
-   **Verwenden Sie die Terminalintegration für Fehlermeldungen**: Wenn Sie im Terminal auf einen Fehler stoßen, führt das Senden an Caret oft zu einer schnelleren Lösung, als zu versuchen, den Fehler zu beschreiben.
...
-   **Kombinieren Sie Terminalausgaben mit Datei-Erwähnungen**: Nachdem Sie Terminalausgaben an Caret gesendet haben, können Sie Ihre Frage verbessern, indem Sie relevante Dateien mit der @-Erwähnungsfunktion erwähnen.
...
Wenn Sie das nächste Mal auf eine kryptische Fehlermeldung in Ihrem Terminal starren, versuchen Sie, die Terminalintegration von Caret zu verwenden, anstatt zu kopieren und einzufügen. Sie erhalten genauere Hilfe, da Caret den vollständigen Terminalkontext mit korrekter Formatierung sehen kann.
...
<Tip>
	**Schnelle Lösung**: Die meisten Terminalprobleme können behoben werden, indem Sie in den Caret-Einstellungen auf bash umsteigen und das Shell
