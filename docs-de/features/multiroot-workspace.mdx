...
description: "Caret's Multiroot Feature funktioniert nahtlos mit VSCode's Multi-Root Workspaces, wodurch Sie mehrere Projektordner in einem einzigen Workspace verwalten können."
---

Caret's Multiroot Feature funktioniert nahtlos mit VSCode's multi-root workspaces, letting you manage multiple project folders in a single workspace.
...
Anstatt auf einen Projektordner beschränkt zu sein, kann Caret Dateien lesen, Code schreiben und Befehle in allen Ordnern Ihres VSCode-Workspaces ausführen. Dies ist hilfreich, wenn Sie mit Monorepos, Microservices arbeiten oder wenn Sie gleichzeitig an verwandten Projekten arbeiten.

### Wie es funktioniert

Wenn Sie mehrere Workspace-Ordner in VSCode öffnen, führt Caret automatisch Folgendes aus:
...
2. **Verwenden Sie Caret** - Caret erkennt automatisch alle Ihre Workspace-Ordner und interagiert bei Bedarf mit ihnen.
...
**Workspace-Erkennung**
- Caret erkennt alle Workspace-Ordner, wenn eine Aufgabe startet
...
- Caret versucht intelligent zu bestimmen, zu welchem Workspace eine Datei gehört
...
Diese Syntax hilft Caret, Mehrdeutigkeiten aufzulösen, wenn mehrere Workspaces ähnlich benannte Dateien enthalten.
...
Fragen Sie Caret: *"Update the API endpoint in both web and mobile apps to match the new backend route"*
...
Caret kann Aufgaben erledigen, die sich über mehrere Workspaces erstrecken:
...
Wenn Sie mit großen Multiroot Workspaces arbeiten, starten Sie im [Plan Mode](/german/features/plan-and-act), damit Caret Ihre Projektstruktur versteht, bevor Änderungen vorgenommen werden.
...
3. **Benennen Sie Ordner eindeutig**, damit Caret Ihre Projektstruktur verstehen kann
