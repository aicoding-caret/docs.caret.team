---
title: "Terminal-Pager während Caret-Sitzungen deaktivieren"
description: "Sorge dafür, dass die CLI-Ausgabe nicht-interaktiv ist, wenn Caret Befehle ausführt, indem die CLINE_ACTIVE-Umgebungsvariable erkannt und Pager wie less deaktiviert werden."
---

Viele CLI-Tools (wie Git) verwenden einen Pager wie `less` für interaktive, scrollbare Ausgaben. Wenn Caret Befehle in Ihrem Terminal ausführt, stört diese Interaktivität - der Pager kann auf der ersten Seite pausieren und den Fortschritt blockieren. Sie können Ihre Shell so konfigurieren, dass Pager deaktiviert und Ausgaben normal durchgeleitet werden, wenn ein Terminal von Caret gestartet wird.

## Funktionsweise

Caret setzt eine Umgebungsvariable für Terminals, die es öffnet, um Befehle auszuführen:

- `CLINE_ACTIVE` — nicht leer, wenn die Shell unter Caret läuft

Sie können diese Variable in Ihrer Shell-Startup-Datei erkennen und Umgebungsvariablen oder Aliase nur für von Caret ausgeführte Sitzungen anpassen. Dies hält Ihre normalen interaktiven Terminals unverändert.
...
- Öffnen Sie eine Aufgabe in Caret, die Terminalbefehle ausführt, und prüfen Sie:
...
- Sie können das Paging auch über Shell-Aliase überschreiben, die auf Caret-Sitzungen beschränkt sind, indem Sie Optionen anstelle von Umgebungsvariablen verwenden:
...
- Wenn Sie Umgebungsvariablen bevorzugen, berücksichtigen viele CLIs auch eine generische oder toolspezifische Pager-Variable:
  ...
  - Man pages: `MANPAGER=cat` (in der Regel nicht für Caret-gesteuerte Befehle erforderlich)
