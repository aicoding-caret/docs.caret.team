...
title: "Hooks"
sidebarTitle: "Hooks"
description: "Integrieren Sie benutzerdefinierte Logik in den Workflow von Caret, um Operationen zu validieren, die Tool-Nutzung zu überwachen und KI-Entscheidungen zu beeinflussen"
---

Hooks ermöglichen es Ihnen, an entscheidenden Stellen benutzerdefinierte Logik in den Workflow von Caret zu integrieren. Betrachten Sie sie als automatisierte Kontrollpunkte, an denen Sie Operationen vor ihrer Ausführung validieren, die Tool-Nutzung in Echtzeit überwachen und die Entscheidungsfindung von Caret steuern können.
...
Das Aktivieren von Hooks in Caret ist unkompliziert. Hier ist die Vorgehensweise:

<Steps>
<Step title="Hooks in den Einstellungen aktivieren">
Öffnen Sie die Caret-Einstellungen und aktivieren Sie das Kontrollkästchen **"Enable Hooks"**.
...
Starten Sie einen Task in Caret und überprüfen Sie, ob Ihr Hook ausgeführt wird.
</Step>
</Steps>
...
Der Schlüssel liegt in der Kombination von Hooks mit externen Tools. Ein Hook kann das Bindeglied zwischen dem Workflow von Caret und dem Rest Ihres Entwicklungs-Ökosystems sein.


## Hook-Typen

Caret bietet mehrere Hook-Typen, mit denen Sie auf verschiedene Phasen des KI-Workflows zugreifen können. Sie sind in Kategorien unterteilt, basierend auf ihren Trigger-Punkten und Anwendungsfällen.
...
Diese Hooks überwachen und verbessern die Kommunikation des Benutzers mit Caret. Verwenden Sie sie, um Eingaben zu validieren, Kontext einzufügen und Interaktionsmuster zu verfolgen.

#### UserPromptSubmit

Wird ausgeführt, wenn ein Benutzer eine Nachricht an Caret sendet. Verwenden Sie ihn, um Eingaben zu validieren, Kontext basierend auf dem Prompt einzufügen und Interaktionsmuster zu verfolgen.
...
Wird ausgeführt, wenn ein neuer Task beginnt. Verwenden Sie ihn, um den Projekttyp zu erkennen, das Tracking zu initialisieren und den initialen Kontext einzufügen, der die Herangehensweise von Caret an die Arbeit prägt.
...
Diese Hooks überwachen interne Caret-Operationen und Ereignisse auf Systemebene. Verwenden Sie sie, um die Kontextnutzung zu verfolgen, das Systemverhalten zu protokollieren und Performance-Muster zu analysieren.
...
Ihr Hook-Skript kann während der Ausführung Logging- oder Diagnoseinformationen auf stdout ausgeben, solange die JSON-Antwort das letzte ist, was geschrieben wird. Caret parst nur das finale JSON-Objekt von stdout.
...
- Suchen Sie nach Fehlern im Output-Panel von VS Code (Caret-Kanal)
...
Denken Sie daran, dass Kontextänderungen zukünftige KI-Entscheidungen beeinflussen, nicht die aktuelle Operation. Das aktuelle Verhalten der KI basiert auf dem vorangegangenen "API Request..."-Block, und Ihre `contextModification` wird in den nächsten "API Request..."-Block eingefügt. Das bedeutet, wenn Sie eine sofortige Wirkung benötigen, sollten Sie PreToolUse-Hooks zur Validierung verwenden und `cancel: true` in der JSON-Antwort Ihres Hooks zurückgeben, um Caret an der Fortsetzung zu hindern.
...
Hooks ergänzen andere Caret-Funktionen:

- [Caret Rules](/german/features/caret-rules) definieren übergeordnete Richtlinien, die durch Hooks durchgesetzt werden können
