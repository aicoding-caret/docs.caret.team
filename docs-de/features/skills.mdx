...
title: "Skills-System"
sidebarTitle: "Skills"
description: "Erweitern Sie die Funktionen von Caret mit wiederverwendbaren Wissensmodulen."
---
...
Im Gegensatz zu Rules (die immer aktiv sind), laden Skills **on-demand**. Sie können Dutzende von Skills installieren, ohne den Kontext oder die Performance zu beeinträchtigen, da Caret nur den Namen und die Beschreibung des Skills sieht, bis dieser tatsächlich benötigt wird.
...
- **Beim Start**: Caret sieht nur eine kurze Beschreibung jedes Skills
...
- `description`: Teilt Caret mit, wann dieser Skill verwendet werden soll (max. 1024 Zeichen)

Die Beschreibung ist entscheidend, da Caret anhand dieser entscheidet, ob ein Skill aktiviert werden soll. Seien Sie präzise darüber, was der Skill macht und wann er eingesetzt werden sollte.
...
<Note>
**Caret-Standardisierung**: Caret nutzt eine duale Verzeichnisarchitektur.
...
## Wie Caret Skills verwendet

Wenn Sie eine Nachricht senden, sieht Caret eine Liste der verfügbaren Skills mit deren Beschreibungen. Wenn Ihre Anfrage zur Beschreibung eines Skills passt, aktiviert Caret diesen über das `use_skill`-Tool, welches die vollständigen Anweisungen lädt.
...
Die Frage "deploy this to AWS" würde Caret dazu veranlassen, den Skill zu aktivieren, seine detaillierten Anweisungen zu laden und diese zu befolgen, um Ihre Anfrage abzuschließen.
...
Skills können zusätzliche Dateien enthalten, auf die Caret nur bei Bedarf zugreift:
...
Caret liest diese Dateien mit `read_file`, wenn die Anweisungen darauf verweisen. Scripts können direkt ausgeführt werden, wobei nur der Output in den Kontext gelangt (nicht der Code des Scripts selbst).
...
|---------|---------|-------------|
| **Rules** | Definiert, wie Caret sich verhalten soll | Immer (oder kontextabhängig) |
...
**Skills** sind Fachwissen, das Caret automatisch aktiviert, wenn es relevant ist (wie Datenanalyse-Kenntnisse, wenn Sie mit CSV-Dateien arbeiten).
