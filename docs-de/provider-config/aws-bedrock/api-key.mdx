...
description: "Richten Sie AWS Bedrock mit Caret unter Verwendung von Bedrock API Keys ein. Die einfachste Einrichtung für einzelne Entwickler, um auf Frontier-Modelle zuzugreifen."
---
...
-   **Caret:** Eine VS Code Extension, die als Coding-Assistent fungiert, indem sie KI-Modelle integriert – sie unterstützt Entwickler bei der Code-Generierung, beim Debugging und bei der Datenanalyse.
...
-   **Modell-Auflistung in Caret:** Die minimalen Berechtigungen (`bedrock:InvokeModel`, `bedrock:InvokeModelWithResponseStream`) reichen aus, damit Caret ein Modell *verwenden* kann, wenn Sie die Modell-ID direkt in den Einstellungen von Caret angeben. Wenn Sie darauf angewiesen sind, dass Caret verfügbare Bedrock-Modelle dynamisch auflistet, benötigen Sie möglicherweise zusätzliche Berechtigungen wie `bedrock:ListFoundationModels`.
-   **AWS Marketplace-Abonnements:** Für Modelle von Drittanbietern (z. B. Anthropic Claude) gewährt Ihnen die Policy **`AmazonBedrockLimitedAccess`** die erforderlichen Berechtigungen zum Abonnieren über den AWS Marketplace. Es muss kein expliziter Zugriff aktiviert werden. Für Anthropic-Modelle müssen Sie dennoch ein First Time Use (FTU) Formular über die Console einreichen. Wenn Sie im Caret-Chat die Meldung erhalten `[ERROR] Failed to process response: Model use case details have not been submitted for this account. Fill out the Anthropic use case details form before using the model.`, öffnen Sie den [Playground in der AWS Bedrock Console](https://console.aws.amazon.com/bedrock/home?#/text-generation-playground), wählen Sie ein beliebiges Anthropic-Modell aus und füllen Sie das Formular aus (eventuell müssen Sie zuerst einen Prompt senden).
...
### Schritt 3: Die Caret VS Code Extension konfigurieren

#### 3.1 Caret installieren und öffnen

1. **VS Code installieren:**\
   Download von der [VS Code Website](https://code.visualstudio.com/).
2. **Die Caret Extension installieren:**
    - Öffnen Sie VS Code.
    - Gehen Sie zum Extensions Marketplace (`Strg+Umschalt+X` oder `Cmd+Umschalt+X`).
    - Suchen Sie nach **Caret** und installieren Sie es.

#### 3.2 Caret-Einstellungen konfigurieren

1. **Caret-Einstellungen öffnen:**
...
Indem Sie diese Schritte befolgen, können Sie AWS Bedrock schnell in die Caret VS Code Extension integrieren, um Ihre Entwicklung zu beschleunigen:
...
3. **Caret in VS Code konfigurieren:** Installieren und richten Sie Caret mit Ihrem AWS API-Key ein und wählen Sie ein passendes Modell.
...
_Dieser Leitfaden wird aktualisiert, wenn sich AWS Bedrock und Caret weiterentwickeln. Beziehen Sie sich für aktuelle Praktiken immer auf die neueste Dokumentation und interne Richtlinien._