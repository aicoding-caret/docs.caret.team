...
description: "Nutzen Sie das Vercel AI Gateway in Careti, um über 100 Modelle über einen einzigen Endpoint mit Routing, Retries und Spend-Observability zu erreichen."
---

Vercel AI Gateway bietet Ihnen eine einzige API, um auf Modelle von vielen Providern zuzugreifen. Sie wechseln einfach per model id, ohne SDKs austauschen oder mehrere Keys verwalten zu müssen. Careti lässt sich direkt integrieren, sodass Sie ein Gateway-Modell im Dropdown-Menü auswählen, es wie jeden anderen Provider nutzen und die Token- sowie Cache-Nutzung im Stream sehen können.
...
1. Öffnen Sie die Careti-Einstellungen
2. Wählen Sie **Vercel AI Gateway** als API Provider aus
3. Fügen Sie Ihren Gateway API Key ein
4. Wählen Sie ein Modell aus der Liste. Careti ruft den Katalog automatisch ab. Sie können auch eine genaue ID einfügen.
...
Das Gateway unterstützt eine große und sich ständig ändernde Auswahl an Modellen. Careti ruft die Liste über die Gateway API ab und speichert sie lokal zwischen. Den aktuellen Katalog finden Sie unter https://vercel.com/ai-gateway/models
...
- Multi-Modell-Evaluierungen – tauschen Sie in Careti nur die Model-ID aus und vergleichen Sie Latenz und Output-Tokens
