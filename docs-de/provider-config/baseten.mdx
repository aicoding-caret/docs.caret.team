---
title: "Baseten"
description: "Erfahren Sie, wie Sie die Model APIs von Baseten mit Caret konfigurieren und nutzen. Greifen Sie auf modernste Open-Source-Modelle mit Enterprise-Grade-Performance, Zuverlässigkeit und wettbewerbsfähigen Preisen zu."
---

<Note>
Dies ist ein Basisdokument für Caret. Es basiert auf dem Caret v3.38.1 Merge-Stand. Falls es spezifische Richtlinien für Caret gibt (erlaubte/blockierte Modelle, regionale Einschränkungen, Authentifizierung/Routing), werden diese im Text mit `<Note>` gekennzeichnet.
</Note>

<Note>
Optimierung des Provider Setup: Gemäß `caret-docs/features/f09-enhanced-provider-setup.md` kann die Validierung der Provider-Einstellungen und die UX in Caret verbessert werden. Beachten Sie, dass sich erlaubte oder blockierte Modelle je nach Account-/Organisationsrichtlinien oder bei Anwendung des Caret-Routers ändern können.
</Note>
...
Caret unterstützt alle aktuellen Modelle unter den Baseten Model APIs, einschließlich:
...
-   **Dynamische Modell-Updates:** Caret ruft automatisch die neueste Modellliste von Baseten ab und gewährleistet so den Zugriff auf neue Modelle direkt nach deren Veröffentlichung.
