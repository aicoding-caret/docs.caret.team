...
description: "Vollständiger Leitfaden zur Lösung von Terminal-Integrationsproblemen in Caret"
---

Dieser Leitfaden hilft Ihnen bei der Behebung von Terminal-Integrationsproblemen in Caret. Die Terminal-Integration ist entscheidend, damit Caret Befehle ausführen und deren Ausgabe lesen kann, was es ermöglicht, Fehler, Testergebnisse und Befehlsantworten zu verstehen.

<Tip>
	Wenn Sie Terminal-Probleme haben, versuchen Sie, in den Caret-Einstellungen unter „Terminal Settings“ zu einer einfacheren Shell wie `bash` zu wechseln.
...
```mermaid
graph TD
    A[Terminal-Problem] --> B{Kann Caret Befehle ausführen?}
    B -->|Nein| C[Shell-Integration nicht verfügbar]
    B -->|Ja| D{Kann Caret die Ausgabe sehen?}
...
-   Befehle werden ausgeführt, aber Caret kann die Ausgabe nicht lesen
-   Das Terminal funktioniert manuell einwandfrei, aber nicht mit Caret
...
-   **Zu bash wechseln**
    ...
    1. Öffnen Sie die Caret Settings
...
-   **PowerShell 7 verwenden**
    ...
    2. Öffnen Sie die Caret Settings
...
-   Caret meldet im Chat: „[Command is running but producing no output]“
-   Befehle werden abgeschlossen, aber Caret sieht die Ergebnisse nicht
...
-   **Shell-Integration-Timeout erhöhen**
    ...
    1. Klicken Sie innerhalb von Caret auf die Schaltfläche **Settings** in der oberen rechten Ecke des Chat-Fensters
...
-   Empfehlen Sie der AI, stattdessen die Dateiausgabe zu verwenden
    1. Weisen Sie Caret im Chat oder über Caret-Regeln an, `command > output.txt` zu verwenden, bevor die Datei(en) gelesen werden

<Tip>
	Diese Kategorie von Problemen ist in den neuesten Caret-Versionen nur teilweise gelöst. Wenn Sie also weiterhin damit konfrontiert sind, erstellen Sie ein GitHub-Issue, falls es sich um ein dauerhaftes Problem handelt.
</Tip>
...
-   Docker-Builds werden in Caret nie abgeschlossen
-   Fortschrittsbalken verbrauchen tausende Tokens
-   Die Caret-Schaltfläche „Proceed while running“ funktioniert im Chat nicht ordnungsgemäß

<Tip>
	Diese Kategorie von Problemen wurde in den neuesten Caret-Versionen gelöst. Sollten Sie dennoch auf Probleme stoßen, erstellen Sie bitte ein GitHub-Issue dazu.
</Tip>

## Terminal-Einstellungen erklärt

Greifen Sie in Caret darauf zu, indem Sie auf das Zahnrad-Symbol klicken und zum Bereich „Terminal Settings“ navigieren:

### Default Terminal Profile

-   **Funktion**: Wählt aus, welche Shell Caret für Befehle verwendet
...
-   **Funktion**: Wie lange Caret wartet, bis das Terminal bereit ist
-   **Standard**: 4 Sekunden
-   **Wann zu erhöhen**:
...
-   **Funktion**: Begrenzt, wie viele Zeilen Caret aus der Terminal-Ausgabe liest
-   **Standard**: 500 Zeilen
-   **Wann anzupassen**:
...
3. Wählen Sie „WSL Bash“ als Terminal-Profil in Caret

#### Pfad-Probleme

Windows-Pfadprobleme:

1. Verwenden Sie Vorwärts-Slashes in Caret: `C:/Users/...`
...
1. Installieren Sie Caret auf der Remote-Maschine, nicht lokal
...
1. Installieren Sie Caret im Container
...
3. Wählen Sie „Trace“
4. Prüfen Sie das Output-Panel → „Caret“ auf Terminal-Logs
...
### Warum erstellt Caret so viele Terminals?

Wenn die Shell-Integration fehlschlägt, kann Caret Terminals nicht sicher wiederverwenden (da sie möglicherweise noch langlaufende Prozesse ausführen). Aktivieren Sie die Shell-Integration oder passen Sie die Einstellung für die Terminal-Wiederverwendung an.

### Kann ich meine benutzerdefinierte Shell verwenden (nushell, xonsh, etc.)?

Caret unterstützt offiziell bash, zsh, fish und PowerShell. Benutzerdefinierte Shells funktionieren möglicherweise, werden aber nicht garantiert. Verwenden Sie bash als Fallback.
...
Eine funktionierende Integration zeigt die Befehlsausgabe im Chat von Caret an. Eine fehlgeschlagene Integration zeigt „Shell Integration Unavailable“ oder „[Command is running but producing no output]“.
...
2. **Problem melden**:
    - Verwenden Sie `/reportbug` in den Caret GitHub Issues
