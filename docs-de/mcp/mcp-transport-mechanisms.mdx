...
description: "Erfahren Sie mehr über die zwei primären Transportmechanismen für die Kommunikation zwischen Caret und MCP-Servern: Standard Input/Output (STDIO) und Server-Sent Events (SSE). Jeder hat spezifische Eigenschaften, Vorteile und Anwendungsfälle."
---

Model Context Protocol (MCP) unterstützt zwei primäre Transportmechanismen für die Kommunikation zwischen Caret und MCP-Servern: Standard Input/Output (STDIO) und Server-Sent Events (SSE). Jeder hat spezifische Eigenschaften, Vorteile und Anwendungsfälle.
...
1. Der Client (Caret) startet einen MCP-Server als Child-Process
...
-   **Lokalität**: Läuft auf derselben Maschine wie Caret
...
-   Single-Client-Szenarien (eine Caret-Instanz pro Server)
...
1. Der Client (Caret) verbindet sich über einen HTTP-GET-Request mit dem SSE-Endpoint des Servers
...
-   **Remote-Zugriff**: Kann auf einer anderen Maschine als Ihre Caret-Instanz gehostet werden
...
STDIO-Server laufen lokal auf derselben Maschine wie Caret, was mehrere wichtige Auswirkungen hat:
...
-   **Ausführung**: Startet und stoppt mit Caret (Child-Process-Lifecycle)
...
-   Bei Bedarf von Caret gestartet werden
-   Keine Netzwerkkonfiguration erfordern
-   Zusammen mit Caret oder über einen Package Manager installiert werden müssen
...
## Konfiguration von Transporten in Caret

Detaillierte Informationen zur Konfiguration von STDIO- und SSE-Transporten in Caret, einschließlich Beispielen, finden Sie unter [Configuring MCP Servers](/german/mcp/configuring-mcp-servers).