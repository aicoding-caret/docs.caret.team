---
title: "MCP Überblick"
description: "Erfahren Sie mehr über Model Context Protocol (MCP) Server, ihre Funktionen und wie Caret beim Erstellen und Verwenden helfen kann. MCP standardisiert, wie Anwendungen LLMs Kontext bereitstellen, und fungiert wie ein USB-C-Anschluss für AI-Anwendungen."
---
...
Caret enthält keine vorinstallierten MCP Server. Sie müssen diese separat finden und installieren.
...
-   **Caret Marketplace:** Installieren Sie einen aus Carets [MCP Marketplace](/german/mcp/mcp-marketplace)
-   **Fragen Sie Caret:** Sie können Caret bitten, Ihnen beim Finden oder Erstellen von MCP Servern zu helfen
...
## Integration mit Caret

Caret vereinfacht das Erstellen und die Nutzung von MCP Servern durch seine AI-Fähigkeiten.

### MCP Server erstellen

-   **Verständnis natürlicher Sprache:** Weisen Sie Caret in natürlicher Sprache an, einen MCP Server zu erstellen, indem Sie dessen Funktionen beschreiben. Caret wird Ihre Anweisungen interpretieren und den erforderlichen Code generieren.
-   **Server klonen und bauen:** Caret kann bestehende MCP Server-Repositories von GitHub klonen und diese automatisch bauen.
-   **Konfigurations- und Abhängigkeitsmanagement:** Caret kümmert sich um Konfigurationsdateien, Umgebungsvariablen und Abhängigkeiten.
-   **Fehlerbehebung und Debugging:** Caret hilft dabei, Fehler während der Entwicklung zu identifizieren und zu beheben.

### MCP Server verwenden

-   **Tool-Ausführung:** Caret integriert sich nahtlos in MCP Server und ermöglicht es Ihnen, deren definierte Tools auszuführen.
-   **Kontextbezogene Interaktionen:** Caret kann intelligent vorschlagen, relevante Tools basierend auf dem Gesprächskontext zu verwenden.
-   **Dynamische Integrationen:** Kombinieren Sie mehrere MCP Server-Funktionen für komplexe Aufgaben. Zum Beispiel könnte Caret einen GitHub-Server verwenden, um Daten abzurufen, und einen Notion-Server, um einen formatierten Bericht zu erstellen.
