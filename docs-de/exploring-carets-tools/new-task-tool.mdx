...
description: "Erfahren Sie mehr über das Tool 'new_task' in Careti, mit dem komplexe Aufgaben in kleinere, überschaubare Abschnitte unterteilt werden können, und wie es in Kombination mit '.agents/context' zur Automatisierung von Arbeitsabläufen eingesetzt werden kann."
---
...
1. **Das Tool `new_task`:**
    - **Funktion:** Ermöglicht es Careti, nach Genehmigung durch den Benutzer die aktuelle Aufgabensitzung zu beenden und sofort eine neue zu starten.
    - **Kontext-Vorabladen:** Entscheidend ist, dass Careti diese neue Aufgabensitzung mit spezifischem Kontext **vorladen** kann, der innerhalb des `<context>`-Blocks des Tools bereitgestellt wird. Dieser Kontext kann alles sein, was Careti oder eine `.agents/context`-Datei definieren – Zusammenfassungen, Code-Snippets, nächste Schritte, Projektstatus usw.
2. **Bewusstsein für das Kontextfenster:**
    - **Verfolgung:** Careti verfolgt intern den Prozentsatz des verfügbaren Kontextfensters, der während einer Aufgabe aktuell verwendet wird.
    - **Sichtbarkeit:** Diese Informationen sind in den `environment_details` sichtbar, die Careti in seinem Prompt zur Verfügung gestellt werden.

#### Verwenden des Slash-Befehls `/newtask`

Als schnelle Alternative zu Careti, das das Tool `newtask` vorschlägt oder komplexe Regeln definiert, können Sie den Prozess direkt mit einem Slash-Befehl initiieren.

-   **Wie:** Geben Sie einfach `/newtask` in das Chat-Eingabefeld ein.
-   **Aktion:** Careti schlägt die Erstellung einer neuen Aufgabe vor und schlägt in der Regel Kontext basierend auf der aktuellen Sitzung vor (ähnlich dem Standardverhalten bei Verwendung des Tools). Sie erhalten weiterhin den `ask_followup_question`-Prompt, um den Kontext zu bestätigen und möglicherweise zu ändern, bevor die neue Aufgabe erstellt wird.
-   **Vorteil:** Bietet eine schnelle, vom Benutzer initiierte Möglichkeit, die `new_task`-Funktionalität für verzweigende Erkundungen oder die Verwaltung langer Sitzungen zu nutzen, ohne darauf zu warten, dass Careti sie vorschlägt.
...
-   **Intelligente Haltepunkte:** Weisen Sie Careti über Regeln an, logische Haltepunkte zu finden (z. B. nach Abschluss einer Funktion oder eines Tests) _nachdem_ ein Kontextschwellenwert überschritten wurde, um sauberere Übergaben zu gewährleisten.
-   **Strukturierte Aufgabenzerlegung:** Verwenden Sie den Plan-Modus, um Teilaufgaben zu definieren, und verwenden Sie dann `.agents/context`, damit Careti nach Abschluss jeder Teilaufgabe automatisch eine neue Aufgabe über `new_task` erstellt und den Kontext für die _nächste_ Teilaufgabe vorlädt.
...
1. **Auslösererkennung (regelbasiert):** Careti überwacht Übergabepunkte, die in den Regeln definiert sind (z. B. Kontextnutzung > 50 %, Aufgabenabschluss).
2. **Benutzerbestätigung:** Careti verwendet `ask_followup_question`, um die Erstellung einer neuen Aufgabe vorzuschlagen und zeigt oft den beabsichtigten, durch die Regeln definierten Kontext an.
...
3. **Benutzerkontrolle:** Sie können die Erstellung der neuen Aufgabe genehmigen, ablehnen oder Careti bitten, den Kontext zu ändern, bevor die neue Aufgabe erstellt wird.
4. **Kontextverpackung (Tool `new_task`):** Wenn die Erstellung genehmigt wurde, verwendet Careti `new_task` und verpackt den Kontext gemäß der durch `.agents/context` vorgeschriebenen Struktur.
...
-   **Intelligente Haltepunkte:** Weisen Sie Careti an, logische Haltepunkte zu finden (z. B. nach Abschluss einer Funktion oder eines Tests) _nachdem_ ein Kontextschwellenwert überschritten wurde, um sauberere Übergaben zu gewährleisten.
-   **Strukturierte Aufgabenzerlegung:** Verwenden Sie den Plan-Modus, um Teilaufgaben zu definieren, und verwenden Sie dann `.agents/context`, damit Careti nach Abschluss jeder Teilaufgabe automatisch eine neue Aufgabe über `new_task` erstellt.
...
-   **Benutzergenehmigung:** Sie haben immer die Kontrolle, genehmigen die Übergabe und haben die Möglichkeit, den Kontext zu ändern, den Careti weiterführen soll.
