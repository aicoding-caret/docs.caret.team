...
<Note>
Dies ist ein auf Caret basierendes Dokument. Es folgt dem Caret v3.38.1 Merge-Stand. Falls es Caret-spezifische Richtlinien gibt (Context-Beschränkungen nach unterstützten/blockierten Modellen, Authentifizierung/Routing), werden diese im Text mit `<Note>` gekennzeichnet.
</Note>
...
2. **Dateiinhalte** – Alle Dateien, die du geteilt hast oder die Caret gelesen hat
3. **Tool-Outputs** – Ergebnisse von ausgeführten Befehlen
4. **System-Prompts** – Anweisungen von Caret (minimaler Einfluss)
...
Caret kann lange Konversationen automatisch zusammenfassen:
...
**A:** Caret zeigt den Token-Verbrauch im Interface an. Achten Sie auf die Kontext-Anzeige, wenn sie sich den Limits nähert.

### F: Was passiert, wenn ich das Kontext-Limit überschreite?
**A:** Caret wird entweder:
