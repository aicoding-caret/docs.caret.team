...
New models drop constantly, so this guide focuses on what's working well with <BrandName /> right now. We'll keep it updated as the landscape shifts.

<Callout type="tip">
**New to model selection?** Start with [Module 2 of <BrandName />'s Learning Path](https://caret.bot/learn) for a comprehensive guide to choosing and configuring models.
</Callout>

## What is an AI Model?

Think of an AI model as the "brain" that powers <BrandName />. When you ask <BrandName /> to write code, fix bugs, or refactor your project, it's the model that actually understands your request and generates the response.
...
- **You choose which model <BrandName /> uses** like picking between different experts for different tasks
- **Models are accessed via API providers** - companies like Anthropic, OpenAI, and OpenRouter host these models

**Why it matters:** The model you choose directly impacts <BrandName />'s capabilities, response quality, speed, and cost. A premium model might handle complex refactoring beautifully but cost more, while a budget model works great for routine tasks at a fraction of the price.

## How to Select a Model in <BrandName />

Follow these 5 simple steps to get <BrandName /> up and running with your preferred AI model:

### Step 1: Open <BrandName /> Settings

First, you need to access <BrandName />'s configuration panel.

**Two ways to open settings:**
- **Quick method**: Click the **gear icon (⚙️)** in the top-right corner of <BrandName />'s chat interface
- **Command palette**: Press **Cmd/Ctrl + Shift + P** → type "<BrandName />: Open Settings"
...
|----------|----------|-------|
| **<BrandName />** | Easiest setup | No API keys needed, access to multiple models including stealth models |
...
See the [full provider list](/en/provider-config/openrouter) for more options including Cerebras, Vertex AI, Azure, and more.

<Info>
**Recommended for beginners:** Start with **<BrandName />** as your provider - no API key management needed, instant access to multiple models, and occasional free inferencing through partner providers.
</Info>
...
#### If you selected **<BrandName />** as your provider:

- **No API key needed!** Simply sign in with your <BrandName /> account
- Click the **Sign In** button when prompted
- You'll be redirected to [app.caret.bot](https://app.caret.bot) to authenticate
...
1. **Visit your provider's website to get an API key:**
   ...
   - **Others**: See [Provider Setup Guide](/en/provider-config/openrouter)

2. **Generate a new API key** on the provider's website

3. **Copy the API key** to your clipboard

4. **Paste your key** in the **"API Key"** field in <BrandName /> settings
...
## Current Top Models {#current-top-models}
...
### Step 5: Start Using <BrandName />

**Congratulations! You're all set up.** Here's how to start coding with <BrandName />:

1. **Type your request** in the <BrandName /> chat box
...
- [**Model Comparison & Pricing**](/en/model-config/model-comparison) - Complete pricing tables and performance benchmarks
- [**Context Window Guide**](/en/model-config/context-windows) - Understanding and optimizing context usage
...
Check [OpenRouter's <BrandName /> usage stats](https://openrouter.ai/apps?url=https%3A%2F%2Fcline.bot%2F) to see real usage patterns from the community.
