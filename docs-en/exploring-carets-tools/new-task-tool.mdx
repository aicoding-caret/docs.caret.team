...
1. **The `new_task` Tool:**
    ...
    - **Context Preloading:** Crucially, Caret can **preload** this new task session with specific context provided within the tool's `<context>` block. This context can be anything Caret or a `.clinerules` file defines – summaries, code snippets, next steps, project state, etc.
...
While the core capabilities exist by default, the true power, automation, and customization emerge when you combine `new_task` and context awareness with custom workflows defined in `.clinerules`. This allows you to precisely control _when_ and _how_ Caret manages context and task continuity.
...
-   **Structured Task Decomposition:** Use Plan Mode to define subtasks, then use `.clinerules` to have Caret automatically create a new task via `new_task` upon completing each subtask, preloading the context for the _next_ subtask.
