...
<BrandName /> includes a powerful internal tool, `new_task`, designed to help manage workflow continuity and context preservation, especially during complex or long-running tasks. This tool, combined with <BrandName />'s awareness of its own context window usage and the flexibility of `.agents/context`, enables sophisticated strategies for breaking down work and ensuring seamless transitions between task sessions.
...
1. **The `new_task` Tool:**
    - **Function:** Allows <BrandName />, upon user approval, to end the current task session and immediately start a new one.
    - **Context Preloading:** Crucially, <BrandName /> can **preload** this new task session with specific context provided within the tool's `<context>` block. This context can be anything <BrandName /> or a `.agents/context` file defines – summaries, code snippets, next steps, project state, etc.
2. **Context Window Awareness:**
    - **Tracking:** <BrandName /> internally tracks the percentage of its available context window currently being used during a task.
    - **Visibility:** This information is visible in the `environment_details` provided to <BrandName /> in its prompt.

#### Using the `/newtask` Slash Command

As a quick alternative to <BrandName /> suggesting the `newtask` tool or defining complex rules, you can directly initiate the process using a Slash Command.

-   **How:** Simply type `/newtask` in the chat input field.
-   **Action:** <BrandName /> will propose creating a new task, typically suggesting context based on the current session (similar to its default behavior when using the tool). You will still get the `ask_followup_question` prompt to confirm and potentially modify the context before the new task is created.
-   **Benefit:** Provides a fast, user-initiated way to leverage the `new_task` functionality for branching explorations or managing long sessions without waiting for <BrandName /> to suggest it.

<Note>
	For more details on using the `/newtask` slash command, see the [New Task Command](/en/features/slash-commands/new-task)
	documentation.
</Note>
...
-   **Tool Availability:** The `new_task` tool exists, and <BrandName /> _can_ choose to use it.
-   **Context Awareness:** <BrandName /> _is_ aware of its context usage percentage.
-   **No Automatic Trigger:** <BrandName /> **will not** automatically initiate a task handoff _solely_ based on context usage reaching a specific percentage (like 50%). The decision to suggest using `new_task` comes from the AI model's reasoning based on the overall task progress and prompt instructions.
-   **Basic Context Preloading:** If `new_task` is used without specific rules defining the `<context>` block structure, <BrandName /> will attempt to preload relevant information based on its current understanding (e.g., a basic summary of progress and next steps), but this may be less comprehensive than a rule-driven approach.

#### The Power of `.agents/context`: Enabling Custom Workflows

While the core capabilities exist by default, the true power, automation, and customization emerge when you combine `new_task` and context awareness with custom workflows defined in `.agents/context`. This allows you to precisely control _when_ and _how_ <BrandName /> manages context and task continuity.
...
-   **Intelligent Breakpoints:** Instruct <BrandName /> via rules to find logical stopping points (e.g., after completing a function or test) _after_ a context threshold is passed, ensuring cleaner handoffs.
-   **Structured Task Decomposition:** Use Plan Mode to define subtasks, then use `.agents/context` to have <BrandName /> automatically create a new task via `new_task` upon completing each subtask, preloading the context for the _next_ subtask.
...
1. **Trigger Identification (Rule-Based):** <BrandName /> monitors for handoff points defined in the rules (e.g., context usage > 50%, task completion).
2. **User Confirmation:** <BrandName /> uses `ask_followup_question` to propose creating a new task, often showing the intended context defined by the rules.
...
3. **User Control:** You can approve, deny, or ask <BrandName /> to modify the context before the new task is created.
4. **Context Packaging (`new_task` Tool):** If approved, <BrandName /> uses `new_task`, packaging the context according to the structure mandated by the `.agents/context`.
...
-   **Intelligent Breakpoints:** Instruct <BrandName /> to find logical stopping points (e.g., after completing a function or test) _after_ a context threshold is passed, ensuring cleaner handoffs.
-   **Structured Task Decomposition:** Use Plan Mode to define subtasks, then use `.agents/context` to have <BrandName /> automatically create a new task via `new_task` upon completing each subtask.
...
-   **Context System Support:** Use `new_task` context blocks as the primary handoff method across sessions, aligned with `AGENTS.md`/`.agents/context` and `work-logs/`.
...
-   **User Approval:** You always have control, approving the handoff and having the chance to modify the context <BrandName /> proposes to carry forward.
