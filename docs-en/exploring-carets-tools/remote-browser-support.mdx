...
description: "Remote browser support allows Careti to utilize a remote Chrome instance, leveraging authentication tokens and session cookies relevant to certain web development test cases."
---

The Remote Browser feature in <BrandName /> allows the AI assistant to interact with web content directly through a controlled browser instance. This enables several powerful capabilities:
...
## Remote Browser in <BrandName />

### What is Remote Browser?

Remote Browser allows <BrandName /> to view and interact with websites directly. This feature enables <BrandName /> to:
...
You can ask <BrandName /> to use the browser with simple instructions:
...
<BrandName /> can only use one browser at a time. If you want to visit a different website, you can either:

-   Ask <BrandName /> to navigate to a new URL within the same browser session
-   Ask <BrandName /> to close the current browser and open a new one

#### Browser Must Be Closed Before Using Other Tools

If you want <BrandName /> to edit files or run commands after using the browser, you must first ask it to close the browser:
...
The browser has a fixed viewport size (900x600 pixels by default), similar to a small laptop screen. <BrandName /> will share screenshots after each action so you can see exactly what it sees.

#### Console Logs

<BrandName /> captures browser console logs, which can be helpful for debugging web applications. These logs are included with each screenshot.

### Common Use Cases

-   **Web Development**: Test your websites and web applications
-   **UI/UX Review**: Get feedback on website design and usability
-   **Content Research**: Have <BrandName /> browse websites to gather information
...
-   **If the browser seems stuck**: Ask <BrandName /> to close the browser and try again
...
#### Configure <BrandName /> in VS Code:

1. Open VS Code settings
2. Search for "<BrandName />: Chrome Executable Path"
3. Set the value to the path of your Chrome executable (e.g., `C:\Program Files\Google\Chrome\Application\chrome.exe`)

<BrandName /> should now be able to use the Remote Browser feature from within WSL.
