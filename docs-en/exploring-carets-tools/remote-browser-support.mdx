...
The Remote Browser feature in Careti allows the AI assistant to interact with web content directly through a controlled browser instance. This enables several powerful capabilities:
...
Remote Browser allows Careti to view and interact with websites directly. This feature enables Careti to:
...
You can ask Careti to use the browser with simple instructions:
...
Careti can only use one browser at a time. If you want to visit a different website, you can either:

-   Ask Careti to navigate to a new URL within the same browser session
-   Ask Careti to close the current browser and open a new one

#### Browser Must Be Closed Before Using Other Tools

If you want Careti to edit files or run commands after using the browser, you must first ask it to close the browser:
...
The browser has a fixed viewport size (900x600 pixels by default), similar to a small laptop screen. Careti will share screenshots after each action so you can see exactly what it sees.

#### Console Logs

Careti captures browser console logs, which can be helpful for debugging web applications. These logs are included with each screenshot.

### Common Use Cases

-   **Web Development**: Test your websites and web applications
-   **UI/UX Review**: Get feedback on website design and usability
-   **Content Research**: Have Careti browse websites to gather information
...
-   **If the browser seems stuck**: Ask Careti to close the browser and try again
...
#### Configure Careti in VS Code:

1. Open VS Code settings
2. Search for "Careti: Chrome Executable Path"
3. Set the value to the path of your Chrome executable (e.g., `C:\Program Files\Google\Chrome\Application\chrome.exe`)

Careti should now be able to use the Remote Browser feature from within WSL.
