...
To access the MCP Servers interface in Careti:

1. Click on the Careti icon in the VSCode sidebar
2. Open the menu (⋮) in the top right corner of the Careti panel
...
4. Careti will attempt to connect to the server and display the connection status

> **Note**: When connecting to a remote server, ensure you trust the source, as MCP servers can execute code in your environment.

#### Transport Types

Careti supports two transport protocols for remote MCP servers:
...
6. You'll be redirected back to Careti automatically
7. The server will connect and show a green status dot

Once authenticated, your credentials are securely stored and the server will reconnect automatically when you reload Careti. You won't need to authenticate again unless you delete the server or your credentials expire.
...
> **Warning**: The following third-party marketplaces are listed for informational purposes only. Careti does not endorse, verify, or take responsibility for any servers listed on these marketplaces. These servers are cloud-hosted services that process your requests and may have access to data you share with them. Always review privacy policies and terms of use before connecting to third-party services.
...
2. **Request Timeout**:
    - Set how long Careti should wait for server responses
...
-   **Enabled**: Careti can use the server's tools and resources
...
-   **Redirect errors**: Verify you're using the latest version of Careti - older versions may not support OAuth
-   **Reset authentication**: Delete the server and re-add it to start fresh with a new OAuth flow

### Advanced Configuration

For advanced users, Careti stores MCP server configurations in a JSON file that can be modified:
...
Once connected, Careti can use the tools and resources provided by the MCP server. When Careti suggests using an MCP tool:

1. A tool approval prompt will appear (unless auto-approved)
2. Review the tool details and parameters before approving
3. The tool will execute and return results to Careti
...
5. Sign in to Vercel in your browser and authorize Careti
6. Return to Careti - the server will automatically connect
7. Vercel's tools (deploy, logs, projects) are now available to Careti!

Your Vercel authentication persists across sessions, so you won't need to re-authenticate each time you use Careti.
