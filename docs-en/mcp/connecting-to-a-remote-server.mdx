...
description: "The Model Context Protocol (MCP) allows Careti to communicate with external servers that provide additional tools and resources to extend its capabilities. This guide explains how to add and connect to remote MCP servers through the MCP Servers interface."
---
...
To access the MCP Servers interface in <BrandName />:

1. Click on the <BrandName /> icon in the VSCode sidebar
2. Open the menu (⋮) in the top right corner of the <BrandName /> panel
...
4. <BrandName /> will attempt to connect to the server and display the connection status

> **Note**: When connecting to a remote server, ensure you trust the source, as MCP servers can execute code in your environment.

#### Transport Types

<BrandName /> supports two transport protocols for remote MCP servers:
...
6. You'll be redirected back to <BrandName /> automatically
7. The server will connect and show a green status dot

Once authenticated, your credentials are securely stored and the server will reconnect automatically when you reload <BrandName />. You won't need to authenticate again unless you delete the server or your credentials expire.
...
> **Warning**: The following third-party marketplaces are listed for informational purposes only. <BrandName /> does not endorse, verify, or take responsibility for any servers listed on these marketplaces. These servers are cloud-hosted services that process your requests and may have access to data you share with them. Always review privacy policies and terms of use before connecting to third-party services.
...
2. **Request Timeout**:
    - Set how long <BrandName /> should wait for server responses
...
-   **Enabled**: <BrandName /> can use the server's tools and resources
...
-   **Redirect errors**: Verify you're using the latest version of <BrandName /> - older versions may not support OAuth
-   **Reset authentication**: Delete the server and re-add it to start fresh with a new OAuth flow

### Advanced Configuration

For advanced users, <BrandName /> stores MCP server configurations in a JSON file that can be modified:
...
Once connected, <BrandName /> can use the tools and resources provided by the MCP server. When <BrandName /> suggests using an MCP tool:

1. A tool approval prompt will appear (unless auto-approved)
2. Review the tool details and parameters before approving
3. The tool will execute and return results to <BrandName />
...
5. Sign in to Vercel in your browser and authorize <BrandName />
6. Return to <BrandName /> - the server will automatically connect
7. Vercel's tools (deploy, logs, projects) are now available to <BrandName />!

Your Vercel authentication persists across sessions, so you won't need to re-authenticate each time you use <BrandName />.
