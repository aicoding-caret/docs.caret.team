...
description: "Learn about the two primary transport mechanisms for communication between Careti and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases."
---

Model Context Protocol (MCP) supports two primary transport mechanisms for communication between <BrandName /> and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (<BrandName />) spawns an MCP server as a child process
...
-   **Locality**: Runs on the same machine as <BrandName />
...
-   Single-client scenarios (one <BrandName /> instance per server)
...
1. The client (<BrandName />) connects to the server's SSE endpoint via HTTP GET request
...
-   **Remote Access**: Can be hosted on a different machine from your <BrandName /> instance
...
STDIO servers run locally on the same machine as <BrandName />, which has several important implications:
...
-   **Execution**: Starts and stops with <BrandName /> (child process lifecycle)
...
-   Start when needed by <BrandName />
-   Not require network configuration
-   Need to be installed alongside <BrandName /> or via a package manager
...
## Configuring Transports in <BrandName />

For detailed information on configuring STDIO and SSE transports in <BrandName />, including examples, see [Configuring MCP Servers](/en/mcp/configuring-mcp-servers).
