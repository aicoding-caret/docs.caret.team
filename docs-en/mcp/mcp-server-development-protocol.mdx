...
description: "This protocol is designed to streamline the development process of building MCP servers with Careti."
---

> **Build and share your MCP servers with the world.** Once you've created a great MCP server, submit it to the [<BrandName /> MCP Marketplace](https://github.com/caret/mcp-marketplace) to make it discoverable and one-click installable by thousands of developers.

## What Are MCP Servers?

Model Context Protocol (MCP) servers extend AI assistants like <BrandName /> by giving them the ability to:
...
The heart of effective MCP server development is following a structured protocol. This protocol is implemented through a `.agents/context` file that lives at the **root** of your MCP working directory (/Users/your-name/Documents/Careti/MCP).

### Using `.agents/context` Files

A `.agents/context` file is a special configuration that <BrandName /> reads automatically when working in the directory where it's placed. These files:

-   Configure <BrandName />'s behavior and enforce best practices
-   Switch <BrandName /> into a specialized MCP development mode
...
When this `.agents/context` file is present in your working directory, <BrandName /> will:
...
First, add a `.agents/context` file to the root of your MCP working directory using the protocol above. This file configures <BrandName /> to use the MCP development protocol when working in this folder.

### 2. Start a Chat with a Clear Description

Begin your <BrandName /> chat by clearly describing what you want to build. Be specific about:
...
<BrandName /> will automatically start in PLAN MODE, guiding you through the planning process:
...
One of the most effective ways to help <BrandName /> build your MCP server is to share official API documentation right at the start:
...
Providing comprehensive API details (endpoints, authentication, data structures) significantly improves <BrandName />'s ability to implement an effective MCP server.

## Understanding the Two Modes

### PLAN MODE

In this collaborative phase, you work with <BrandName /> to design your MCP server:
...
Once planning is complete, <BrandName /> helps implement the server:
...
Resources let your MCP servers expose data to <BrandName /> without executing code. They're perfect for providing context like files, API responses, or database records that <BrandName /> can reference during conversations.
...
Resources make your MCP servers more context-aware, allowing <BrandName /> to access specific information without requiring you to copy/paste. For more information, refer to the [official documentation](https://modelcontextprotocol.io/docs/concepts/resources).
