...
> **Build and share your MCP servers with the world.** Once you've created a great MCP server, submit it to the [Careti MCP Marketplace](https://github.com/caret/mcp-marketplace) to make it discoverable and one-click installable by thousands of developers.

## What Are MCP Servers?

Model Context Protocol (MCP) servers extend AI assistants like Careti by giving them the ability to:
...
A `.agents/context` file is a special configuration that Careti reads automatically when working in the directory where it's placed. These files:

-   Configure Careti's behavior and enforce best practices
-   Switch Careti into a specialized MCP development mode
...
When this `.agents/context` file is present in your working directory, Careti will:
...
First, add a `.agents/context` file to the root of your MCP working directory using the protocol above. This file configures Careti to use the MCP development protocol when working in this folder.

### 2. Start a Chat with a Clear Description

Begin your Careti chat by clearly describing what you want to build. Be specific about:
...
Careti will automatically start in PLAN MODE, guiding you through the planning process:
...
One of the most effective ways to help Careti build your MCP server is to share official API documentation right at the start:
...
Providing comprehensive API details (endpoints, authentication, data structures) significantly improves Careti's ability to implement an effective MCP server.

## Understanding the Two Modes

### PLAN MODE

In this collaborative phase, you work with Careti to design your MCP server:
...
Once planning is complete, Careti helps implement the server:
...
Resources let your MCP servers expose data to Careti without executing code. They're perfect for providing context like files, API responses, or database records that Careti can reference during conversations.
...
Resources make your MCP servers more context-aware, allowing Careti to access specific information without requiring you to copy/paste. For more information, refer to the [official documentation](https://modelcontextprotocol.io/docs/concepts/resources).
