...
The heart of effective MCP server development is following a structured protocol. This protocol is implemented through a `.clinerules` file that lives at the **root** of your MCP working directory (/Users/your-name/Documents/Caret/MCP).

### Using `.clinerules` Files

A `.clinerules` file is a special configuration that Caret reads automatically when working in the directory where it's placed. These files:
...
When this `.clinerules` file is present in your working directory, Caret will:
...
First, add a `.clinerules` file to the root of your MCP working directory using the protocol above. This file configures Caret to use the MCP development protocol when working in this folder.
