---
title: "MCP Made Easy"
description: "Learn how to use the MCP Marketplace to discover, install, and configure MCP servers that enhance Caret's capabilities with additional tools and resources."
---

## What's an MCP Server?

MCP servers are specialized extensions that enhance Caret's capabilities. They enable Caret to perform additional tasks like fetching web pages, processing images, accessing APIs, and much more.
...
-   In Caret, click the "Extensions" button (square icon) in the top toolbar
...
-   If the server requires an API key (most do), Caret will guide you through:
...
-   Caret will show confirmation when installation is complete
-   Check the server status in Caret's MCP settings UI

### 5. Using Your New Server

-   After successful installation, Caret will automatically integrate the server's capabilities
-   You'll see new tools and resources available in Caret's system prompt
-   Simply ask Caret to use the capabilities of your new server
-   Example: "Search the web for recent React updates using Perplexity"

**Corporate Users:** If you're using Caret in a corporate environment, ensure you have permission to install third-party MCP servers according to your organization's security policies.
...
Utilize a `.agents/context` file or custom instructions to support intelligent MCP server activation through keyword-based triggers, making Caret's tool selection more intuitive and context-aware.
...
Add this to your `.agents/context` file or to your custom instructions to make Caret's MCP server selection more intuitive and context-aware.
