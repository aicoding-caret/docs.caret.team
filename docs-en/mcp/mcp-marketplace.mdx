---
title: "MCP Made Easy"
description: "Learn how to use the MCP Marketplace to discover, install, and configure MCP servers that enhance Careti's capabilities with additional tools and resources."
---

## What's an MCP Server?

MCP servers are specialized extensions that enhance <BrandName />'s capabilities. They enable <BrandName /> to perform additional tasks like fetching web pages, processing images, accessing APIs, and much more.
...
-   In <BrandName />, click the "Extensions" button (square icon) in the top toolbar
...
-   If the server requires an API key (most do), <BrandName /> will guide you through:
...
-   <BrandName /> will show confirmation when installation is complete
-   Check the server status in <BrandName />'s MCP settings UI

### 5. Using Your New Server

-   After successful installation, <BrandName /> will automatically integrate the server's capabilities
-   You'll see new tools and resources available in <BrandName />'s system prompt
-   Simply ask <BrandName /> to use the capabilities of your new server
-   Example: "Search the web for recent React updates using Perplexity"

**Corporate Users:** If you're using <BrandName /> in a corporate environment, ensure you have permission to install third-party MCP servers according to your organization's security policies.
...
-   The server code is cloned/installed to `/Users/<username>/Documents/Careti/MCP/`
...
-   <BrandName /> detects the configuration change
-   <BrandName /> launches your server as a separate process
-   Communication is established via stdio or HTTP

### 4. Integration with <BrandName />

-   Your server's capabilities are added to <BrandName />'s system prompt
-   Tools become available via `use_mcp_tool` commands
-   Resources become available via `access_mcp_resource` commands
-   <BrandName /> can now use these capabilities when prompted by the user
...
-   Try removing and reinstalling the server (remove from both the `cline_mcp_settings.json` file and the `/Users/<username>/Documents/Careti/MCP/` directory)
-   Use a terminal and run the command with its arguments directly. This will allow you to see the same errors that <BrandName /> is seeing

## MCP Server Rules

<BrandName /> is already aware of your active MCP servers and what they are for, but when you have a lot of MCP servers enabled, it can be useful to define when to use each server.

Utilize a `.agents/context` file or custom instructions to support intelligent MCP server activation through keyword-based triggers, making <BrandName />'s tool selection more intuitive and context-aware.

### How MCP Rules Work

MCP Rules group your connected MCP servers into functional categories and define trigger keywords that activate them automatically when detected in your conversations with <BrandName />.
...
-   **Contextual Tool Selection**: <BrandName /> selects appropriate tools based on conversation context
...
When you write "Can you scrape this website?", <BrandName /> detects "scrape" and "website" as triggers, automatically selecting web-related MCP servers.
...
Add this to your `.agents/context` file or to your custom instructions to make <BrandName />'s MCP server selection more intuitive and context-aware.
