...
MCP servers are specialized extensions that enhance Careti's capabilities. They enable Careti to perform additional tasks like fetching web pages, processing images, accessing APIs, and much more.
...
-   In Careti, click the "Extensions" button (square icon) in the top toolbar
...
-   If the server requires an API key (most do), Careti will guide you through:
...
-   Careti will show confirmation when installation is complete
-   Check the server status in Careti's MCP settings UI

### 5. Using Your New Server

-   After successful installation, Careti will automatically integrate the server's capabilities
-   You'll see new tools and resources available in Careti's system prompt
-   Simply ask Careti to use the capabilities of your new server
-   Example: "Search the web for recent React updates using Perplexity"

**Corporate Users:** If you're using Careti in a corporate environment, ensure you have permission to install third-party MCP servers according to your organization's security policies.
...
-   Careti detects the configuration change
-   Careti launches your server as a separate process
-   Communication is established via stdio or HTTP

### 4. Integration with Careti

-   Your server's capabilities are added to Careti's system prompt
-   Tools become available via `use_mcp_tool` commands
-   Resources become available via `access_mcp_resource` commands
-   Careti can now use these capabilities when prompted by the user
...
-   Use a terminal and run the command with its arguments directly. This will allow you to see the same errors that Careti is seeing

## MCP Server Rules

Careti is already aware of your active MCP servers and what they are for, but when you have a lot of MCP servers enabled, it can be useful to define when to use each server.

Utilize a `.agents/context` file or custom instructions to support intelligent MCP server activation through keyword-based triggers, making Careti's tool selection more intuitive and context-aware.

### How MCP Rules Work

MCP Rules group your connected MCP servers into functional categories and define trigger keywords that activate them automatically when detected in your conversations with Careti.
...
-   **Contextual Tool Selection**: Careti selects appropriate tools based on conversation context
...
When you write "Can you scrape this website?", Careti detects "scrape" and "website" as triggers, automatically selecting web-related MCP servers.
...
Add this to your `.agents/context` file or to your custom instructions to make Careti's MCP server selection more intuitive and context-aware.
