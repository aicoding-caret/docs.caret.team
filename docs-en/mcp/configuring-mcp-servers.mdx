...
Utilizing MCP servers will increase your token usage. Careti offers the ability to restrict or disable MCP server functionality as desired.

1. Click the "MCP Servers" icon in the top navigation bar of the Careti extension.
2. Select the "Configure" tab, and then Click the "Advanced MCP Settings" link at the bottom of that pane.
3. Careti will open a new settings window. find `Careti>Mcp:Mode` and make your selection from the dropdown menu.
...
1. Click the MCP Servers icon at the top navigation bar of the Careti pane.
...
_Example of MCP Server config in Careti (STDIO Transport)_
...
After configuring an MCP server, Careti will automatically detect available tools and resources. To use them:

1. Type your request in Careti's conversation window
2. Careti will identify when an MCP tool can help with your task
