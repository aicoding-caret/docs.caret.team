...
Utilizing MCP servers will increase your token usage. <BrandName /> offers the ability to restrict or disable MCP server functionality as desired.

1. Click the "MCP Servers" icon in the top navigation bar of the <BrandName /> extension.
2. Select the "Configure" tab, and then Click the "Advanced MCP Settings" link at the bottom of that pane.
3. <BrandName /> will open a new settings window. find `<BrandName />>Mcp:Mode` and make your selection from the dropdown menu.
...
1. Click the MCP Servers icon at the top navigation bar of the <BrandName /> pane.
...
_Example of MCP Server config in <BrandName /> (STDIO Transport)_
...
For more in-depth information about how STDIO transport works, see [MCP Transport Mechanisms](/en/mcp/mcp-transport-mechanisms).
...
For more in-depth information about how SSE transport works, see [MCP Transport Mechanisms](/en/mcp/mcp-transport-mechanisms).
...
After configuring an MCP server, <BrandName /> will automatically detect available tools and resources. To use them:

1. Type your request in <BrandName />'s conversation window
2. <BrandName /> will identify when an MCP tool can help with your task
