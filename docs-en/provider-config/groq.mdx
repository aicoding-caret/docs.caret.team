...
3.  **Create a Key:** Navigate to the API Keys section and create a new API key. Give your key a descriptive name (e.g., "<BrandName />").
4.  **Copy the Key:** Copy the API key immediately. You will not be able to see it again. Store it securely.

### Supported Models

<BrandName /> supports the following Groq models:
