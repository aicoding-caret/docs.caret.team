...
# <BrandName /> Context System (Memory Bank Replacement)

<BrandName /> **fully replaces** the Cline Memory Bank approach.  
Instead, <BrandName /> manages project context through **`AGENTS.md` + `.agents/context` + `caret-docs` + `work-logs`**.
...
## Why <BrandName /> Replaced Memory Bank (<BrandName /> Advantages)
...
| Memory Bank | <BrandName /> Replacement |
| --- | --- |
...
- [<BrandName /> Rules](/en/features/caret-rules)
