...
Welcome to the Caret Prompting Guide! This guide will equip you with the knowledge to write effective prompts and custom instructions, maximizing your productivity with Caret.

## .clineignore File Guide

### Overview

The `.clineignore` file is a project-level configuration file that tells Caret which files and directories to ignore when analyzing your codebase. Similar to `.gitignore`, it uses pattern matching to specify which files should be excluded from Caret's context and operations.

### Purpose

-   **Reduce Noise**: Exclude auto-generated files, build artifacts, and other non-essential content
-   **Improve Performance**: Limit the amount of code Caret needs to process
-   **Focus Attention**: Direct Caret to relevant parts of your codebase
-   **Protect Sensitive Data**: Prevent Caret from accessing sensitive configuration files
...
-   **Starting a New Task:** "Caret, let's start a new task. Create `user-authentication.js`. We need to implement user login with JWT tokens. Here are the requirements…"
-   **Summarizing Previous Work:** "Caret, summarize what we did in the last user dashboard task. I want to capture the main features and outstanding issues. Save this to `cline_docs/user-dashboard-summary.md`."
