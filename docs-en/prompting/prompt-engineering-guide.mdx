...
The `.caretignore` file is a project-level configuration file that tells Caret which files and directories to ignore when analyzing your codebase. Similar to `.gitignore`, it uses pattern matching to specify which files should be excluded from Caret's context and operations.
...
-   **Summarizing Previous Work:** "Caret, summarize what we did in the last user dashboard task. I want to capture the main features and outstanding issues. Save this to `caret_docs/user-dashboard-summary.md`."
