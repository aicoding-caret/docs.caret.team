...
Your model is now ready to use within <BrandName />.

#### 3. Configure <BrandName />
...
Open VS Code and configure <BrandName />:

1. Click the <BrandName /> settings icon
...
For the best experience with <BrandName />, use **Qwen3 Coder 30B**. This model provides strong coding capabilities and reliable tool use for local development.
...
-   Start Ollama before using with <BrandName />
-   Keep Ollama running in background
-   First model download may take several minutes

### Enable Compact Prompts

For better performance with local models, enable compact prompts in <BrandName /> settings. This reduces the prompt size by 90% while maintaining core functionality.

Navigate to <BrandName /> Settings → Features → Use Compact Prompt and toggle it on.

### Troubleshooting

If <BrandName /> can't connect to Ollama:
