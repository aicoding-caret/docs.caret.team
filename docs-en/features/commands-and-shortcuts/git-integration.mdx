...
<BrandName />'s Git integration brings AI assistance directly to your version control workflow. Generate commit messages without leaving your editor.
...
2. Click the robot icon in the Source Control view or run the "Generate Commit Message with <BrandName />" command
3. <BrandName /> analyzes your changes and generates a descriptive commit message
...
<Tip>
	...
	Mentions](/en/features/at-mentions/git-mentions) documentation.
</Tip>

## How It Works

When you use <BrandName />'s commit message generation feature, here's what happens behind the scenes:

1. <BrandName /> retrieves the current Git diff using `getWorkingState()`
...
This process uses your current <BrandName /> API configuration, so the quality of the generated messages matches your chosen AI model.
...
1. When you trigger the command:
    - <BrandName /> gets the current diff
...
Next time you're struggling to write a good commit message, try using <BrandName />'s commit message generation. You'll save time and improve your version control workflow with AI assistance right where you need it.
