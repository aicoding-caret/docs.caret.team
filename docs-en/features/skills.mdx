...
title: "Skills System"
sidebarTitle: "Skills"
description: "Extend Caret's capabilities with reusable knowledge modules."
---
...
Skills are reusable instruction sets for specific tasks. Each skill packages detailed guidance, workflows, and additional resources, loading only when relevant to your request.

Unlike rules (which are always active), skills load **on-demand**. You can install dozens of skills without affecting context or performance because Caret only sees the skill name and description until it's actually needed.

## Why Skills?

Think about how you'd onboard a new team member: you wouldn't dump every document on them at once. You'd give them a brief overview, then point them to detailed guides when they're working on specific tasks.

Skills work the same way:
- **At startup**: Caret sees only a brief description of each skill
- **When triggered**: The full instructions for that skill are loaded
- **As needed**: Additional files bundled with skills are read only when referenced
...
The `SKILL.md` file has two parts: YAML frontmatter and instructions.
...
- `description`: Tells Caret when to use this skill (max 1024 characters)

The description is critical because it's how Caret decides whether to activate a skill. Be specific about what the skill does and when it should be used.
...
When a global skill and project skill have the same name, **global skills take precedence**. This lets you customize skills for your personal workflow while still using project defaults.

<Note>
**Caret Standardization**: Caret uses a dual-directory architecture.
- `.agents/` - For AI (token-optimized)
- `.users/` - For humans (detailed explanations)

Use the `/init` command to initialize the standard structure. See [Advanced Rules System](/en/caret-exclusive/knowledge-parity-system) and [AI-Developer Knowledge Parity](/en/caret-exclusive/knowledge-parity-system) for details.
</Note>
...
## How Caret Uses Skills

When you send a message, Caret sees a list of available skills with their descriptions. If your request matches a skill's description, Caret activates it using the `use_skill` tool, which loads the full instructions.
...
Asking "deploy this to AWS" would trigger Caret to activate the skill, load its detailed instructions, and follow them to complete your request.
...
Skills can include additional files that Caret accesses only when needed:
...
Caret reads these files using `read_file` when the instructions reference them. Scripts can be executed directly, with only the output entering the context (not the script code itself).
...
|---------|---------|-------------|
| **Rules** | Define how Caret should behave | Always (or contextually) |
...
**Skills** are expertise that Caret activates automatically when relevant (like data analysis knowledge when you're working with CSV files).
...
- [Advanced Rules System](/en/caret-exclusive/knowledge-parity-system) for project rule management
- [Hooks](/en/features/hooks) for injecting custom logic at key moments
- [Workflows](/en/features/slash-commands/workflows) for explicit task automation
