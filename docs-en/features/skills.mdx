...
title: "Skills System"
sidebarTitle: "Skills"
description: "Extend Careti's capabilities with reusable knowledge modules."
---
...
Unlike rules (which are always active), skills load **on-demand**. You can install dozens of skills without affecting context or performance because <BrandName /> only sees the skill name and description until it's actually needed.
...
- **At startup**: <BrandName /> sees only a brief description of each skill
...
- `description`: Tells <BrandName /> when to use this skill (max 1024 characters)

The description is critical because it's how <BrandName /> decides whether to activate a skill. Be specific about what the skill does and when it should be used.
...
<Note>
**<BrandName /> Standardization**: <BrandName /> uses a dual-directory architecture.
...
## How <BrandName /> Uses Skills

When you send a message, <BrandName /> sees a list of available skills with their descriptions. If your request matches a skill's description, <BrandName /> activates it using the `use_skill` tool, which loads the full instructions.
...
Asking "deploy this to AWS" would trigger <BrandName /> to activate the skill, load its detailed instructions, and follow them to complete your request.
...
Skills can include additional files that <BrandName /> accesses only when needed:
...
<BrandName /> reads these files using `read_file` when the instructions reference them. Scripts can be executed directly, with only the output entering the context (not the script code itself).
...
|---------|---------|-------------|
| **Rules** | Define how <BrandName /> should behave | Always (or contextually) |
...
**Skills** are expertise that <BrandName /> activates automatically when relevant (like data analysis knowledge when you're working with CSV files).
