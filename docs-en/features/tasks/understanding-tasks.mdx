...
description: "Learn what tasks are in Careti, how they work, and how to create effective prompts for better results."
---

<Note>
캐러티(<BrandName />) 기준 문서입니다. <BrandName /> v3.38.1 머지본을 따르며, 캐러티 전용 정책/제한(예: 작업 저장 위치, 체크포인트 정책, 인증/라우팅)이 있을 경우 본문에서 `<Note>`로 표시합니다.
</Note>
...
Most users interact with <BrandName /> through **tasks** - the fundamental unit of work that drives every coding session. Whether you're building a new feature, fixing a bug, refactoring code, or exploring a codebase, every interaction with <BrandName /> happens within the context of a task. A task represents a complete conversation and work session between you and the AI agent, created through **prompts** - the instructions you provide to tell <BrandName /> what you want to accomplish. Tasks serve as self-contained work sessions that capture your entire conversation with <BrandName />, including all the code changes, command executions, and decisions made along the way.

This approach ensures that your work is organized, traceable, and resumable. Each task maintains its own isolated context, allowing you to work on multiple projects simultaneously without confusion. The beauty of <BrandName />'s task system lies in its flexibility and persistence, providing a collaborative coding session where you provide the direction through prompts, and <BrandName /> executes your vision with precision.

### Key Characteristics

Each task in <BrandName />:
...
These features make <BrandName /> not just a coding tool, but a comprehensive development agent that understands the full lifecycle of your work.

## Creating Tasks with Prompts

Tasks begin with prompts - your instructions to <BrandName />. The quality of your results depends heavily on how you describe what you want.
...
Deep dive into **Module 1: "Prompting"** in [<BrandName /> Learn](https://clinelearn.com) to become an expert at creating effective prompts. The module covers:
...
<BrandName /> operates in two distinct modes that help structure your workflow:

- **Plan Mode**: For information gathering, discussing approaches, and creating strategies without making changes
- **Act Mode**: For actual implementation where <BrandName /> executes file modifications, runs commands, and uses tools
...
One of <BrandName />'s powerful features is the ability to resume interrupted tasks:
...
2. <BrandName /> loads the complete conversation
...
This context allows <BrandName /> to:
- Understand what has been done
- Maintain consistency in approach
- Resume work intelligently
- Learn from previous attempts

→ **[Learn more about Context Management](/en/getting-started/understanding-context-management)** to understand how <BrandName /> manages and optimizes context across tasks.

Understanding how tasks work is fundamental to using <BrandName /> effectively. With well-crafted prompts and an understanding of the task lifecycle, you can leverage <BrandName />'s full potential to accelerate your development workflow.
