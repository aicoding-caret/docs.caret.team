...
You can also have Caret create a rule for you by using the [`/newrule` slash command](/features/slash-commands/new-rule) in the chat.
...
Caret automatically processes **all Markdown files** inside the `.clinerules/` directory, combining them into a unified set of rules. The numeric prefixes (optional) help organize files in a logical sequence.
...
To make managing both single `.clinerules` files and the folder system even easier, Caret v3.13 introduces a dedicated popover UI directly accessible from the chat interface.
