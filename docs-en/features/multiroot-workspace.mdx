...
Careti's Multiroot feature works seamlessly with VSCode's multi-root workspaces, letting you manage multiple project folders in a single workspace.
...
Instead of being limited to one project folder, Careti can read files, write code, and run commands across all folders in your VSCode workspace. This is helpful when working with monorepos, microservices, or when you're working on related projects simultaneously.

### How it works

When you open multiple workspace folders in VSCode, Careti automatically:
...
2. **Start using Careti** - Careti will automatically detect all your workspace folders and interact with them as needed.
...
**Workspace detection**
- Careti detects all workspace folders when a task starts
...
- Careti attempts to intelligently determine which workspace a file belongs to
...
This syntax helps Careti resolve ambiguity when multiple workspaces contain similarly named files.
...
Ask Careti: *"Update the API endpoint in both web and mobile apps to match the new backend route"*
...
Careti can complete tasks spanning multiple workspaces:
...
When working with large multiroot workspaces, start in [Plan mode](/en/features/plan-and-act) to let Careti understand your project structure before making changes.
...
3. **Name folders clearly** so Careti can understand your project structure
