...
<Note>
캐러티(<BrandName />) 기준 문서입니다. <BrandName /> v3.38.1 머지본을 따르며, 캐러티 전용 정책(지원 OS/마이크 권한, 인증/라우팅, 음성 처리 제한)이 있을 경우 본문에서 `<Note>`로 표시합니다.
</Note>
...
Use Dictation constantly in [Plan mode](/en/features/plan-and-act) for rapid back-and-forth discussions. Instead of typing careful, structured prompts, think about a problem. <BrandName /> asks clarifying questions, respond immediately, and iterate until having a solid plan.
...
3. Sign into your <BrandName /> account when prompted
4. Install FFmpeg if you haven't already (<BrandName /> will guide you)
...
<Tip>
Dictation works with any AI model you've configured. The transcription happens through <BrandName />'s service, but your conversation continues with whatever model you're using.
</Tip>
...
If you don't have FFmpeg installed, <BrandName /> will automatically detect this and prompt you to install it with a single click.
...
- Respond to <BrandName />'s questions immediately
- Iterate on ideas without typing friction
- Think out loud while <BrandName /> listens
...
- <BrandName /> account with transcription credits
...
Voice transcription costs $0.006 per minute through your <BrandName /> account. For most users, this works out to pennies per session.
...
- Check if firewall is blocking <BrandName />'s servers
- Try signing out and back into your <BrandName /> account

**Authentication Issues**
- Sign out and back into your <BrandName /> account if you see authentication errors
