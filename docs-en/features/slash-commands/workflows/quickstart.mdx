...
<Steps>
  ...
  <Step title="Run the workflow">
    ...
    1. Open the <BrandName /> chat panel
    2. Type `/pr-review.md` followed by the PR number (e.g., `/pr-review.md 42`)
    3. <BrandName /> fetches PR details, analyzes the code, and shows findings

    <Tip>
      When <BrandName /> runs commands like `gh pr view`, it may pause to show output. Click **Proceed While Running** to let <BrandName /> continue.
    </Tip>
  </Step>
</Steps>
...
Look for repetitive tasks in your workflow and turn them into efficient <BrandName /> workflows.
