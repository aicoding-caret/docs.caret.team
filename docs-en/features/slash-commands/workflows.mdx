...
Workflows allow you to define a series of steps to guide <BrandName /> through a repetitive set of tasks, such as deploying a service or submitting a PR.

To invoke a workflow, type `/[workflow-name.md]` in the chat.

## How to Create and Use Workflows

Workflows live alongside [<BrandName /> Rules](/en/features/caret-rules). Creating one is straightforward:

<Frame>
	<img src="https://storage.googleapis.com/cline_public_images/docs/assets/workflows.png" alt="Workflows tab in Careti" />
</Frame>

1. Create a markdown file with clear instructions for the steps <BrandName /> should take
...
-   Leverage <BrandName />'s [built-in tools](/en/exploring-carets-tools/caret-tools-guide) like `ask_followup_question`, `read_file`, `search_files`, and `new_task`
...
3. Let <BrandName /> handle everything else

My workflow uses the `gh` command-line tool and <BrandName />'s built in `ask_followup_question` to:
...
-   If I say "yes," <BrandName /> automatically approves the PR with the `gh` command
