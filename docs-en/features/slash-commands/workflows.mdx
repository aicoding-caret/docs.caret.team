...
Workflows allow you to define a series of steps to guide Caret through a repetitive set of tasks, such as deploying a service or submitting a PR.

To invoke a workflow, type `/[workflow-name.md]` in the chat.

## How to Create and Use Workflows

Workflows live alongside [Caret Rules](/en/features/caret-rules). Creating one is straightforward:

<Frame>
	<img src="https://storage.googleapis.com/cline_public_images/docs/assets/workflows.png" alt="Workflows tab in Caret" />
</Frame>

1. Create a markdown file with clear instructions for the steps Caret should take
...
-   Leverage Caret's [built-in tools](/en/exploring-carets-tools/caret-tools-guide) like `ask_followup_question`, `read_file`, `search_files`, and `new_task`
-   Use command-line tools you already have installed like `gh` or `docker`
-   Reference external [MCP tool calls](/en/mcp/mcp-overview) like Slack or Whatsapp
...
3. Let Caret handle everything else

My workflow uses the `gh` command-line tool and Caret's built in `ask_followup_question` to:
...
-   If I say "yes," Caret automatically approves the PR with the `gh` command
...
> This is just one example of a workflow file. You can find more in our [prompts repository](https://github.com/caret/prompts) for inspiration.
