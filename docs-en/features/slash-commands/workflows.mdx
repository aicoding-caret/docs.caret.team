...
Workflows live alongside [Caret Rules](/features/caret-rules). Creating one is straightforward:
...
-   Leverage Caret's [built-in tools](/exploring-clines-tools/caret-tools-guide) like `ask_followup_question`, `read_file`, `search_files`, and `new_task`
