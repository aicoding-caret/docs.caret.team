...
`/reportbug` is an absolute lifesaver when you hit a weird issue with <BrandName />. Instead of having to remember all the details GitHub wants for a bug report, this command turns <BrandName /> into your personal bug reporting assistant.
...
-   <BrandName /> will guide you through all the details we need:
...
Last week I hit a weird bug where <BrandName /> kept timing out when reading large files. Instead of trying to remember all the GitHub template fields, I just typed `/reportbug` and <BrandName /> guided me through the whole process.

It asked me about what I was trying to do, what happened instead, and the exact steps that led to the issue. The best part was that it automatically included my OS version, <BrandName /> version, and all the technical details our devs would need.
