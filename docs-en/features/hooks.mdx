...
title: "Hooks"
sidebarTitle: "Hooks"
description: "Inject custom logic into Caret's workflow to validate operations, monitor tool usage, and shape AI decisions"
---

<Note>
캐럿(Caret) 기준 문서입니다. Caret v3.38.1 머지본을 따르며, 캐럿 전용 정책(허용 훅 스코프, 인증/라우팅, 로깅/텔레메트리 차이)이 있을 경우 본문에서 `<Note>`로 표시합니다.
</Note>
...
Hooks let you inject custom logic into Caret's workflow at key moments. Think of them as automated checkpoints where you can validate operations before they execute, monitor tool usage as it happens, and shape how Caret makes decisions.
...
Enabling hooks in Caret is straightforward. Here's what you need to do:

<Steps>
<Step title="Enable Hooks in Settings">
Open Caret settings and check the **"Enable Hooks"** checkbox.
...
- Create hooks in `~/Documents/Caret/Rules/Hooks/`
...
Start a task in Caret and verify your hook executes.
</Step>
</Steps>
...
The key is combining hooks with external tools. A hook can be the glue between Caret's workflow and the rest of your development ecosystem.


## Hook Types

Caret provides multiple hook types that let you tap into different stages of the AI workflow. They're organized into categories based on their trigger points and use cases.
...
These hooks monitor and enhance user communication with Caret. Use them to validate input, inject context, and track interaction patterns.

#### UserPromptSubmit

Runs when a user sends a message to Caret. Use it to validate input, inject context based on the prompt, and track interaction patterns.
...
Runs when a new task begins. Use it to detect project type, initialize tracking, and inject initial context that shapes how Caret approaches the work.
...
These hooks monitor internal Caret operations and system-level events. Use them to track context usage, log system behavior, and analyze performance patterns.
...
Your hook script can output logging or diagnostic information to stdout during execution, as long as the JSON response is the last thing written. Caret will parse only the final JSON object from stdout.
...
- Look for errors in VSCode's Output panel (Caret channel)
...
Hooks complement other Caret features:

- [Caret Rules](/en/features/caret-rules) define high-level guidance that hooks can enforce
- [Checkpoints](/en/features/checkpoints) let you roll back changes if a hook didn't catch an issue
- [Auto-Approve](/en/features/auto-approve) works well with hooks as safety nets for automated operations
