...
title: "Hooks"
sidebarTitle: "Hooks"
description: "Inject custom logic into Careti's workflow to validate operations, monitor tool usage, and shape AI decisions"
---

Hooks let you inject custom logic into <BrandName />'s workflow at key moments. Think of them as automated checkpoints where you can validate operations before they execute, monitor tool usage as it happens, and shape how <BrandName /> makes decisions.
...
Enabling hooks in <BrandName /> is straightforward. Here's what you need to do:

<Steps>
<Step title="Enable Hooks in Settings">
Open <BrandName /> settings and check the **"Enable Hooks"** checkbox.
...
- Create hooks in `~/Documents/.agents/hooks/`
...
Start a task in <BrandName /> and verify your hook executes.
</Step>
</Steps>
...
The key is combining hooks with external tools. A hook can be the glue between <BrandName />'s workflow and the rest of your development ecosystem.


## Hook Types

<BrandName /> provides multiple hook types that let you tap into different stages of the AI workflow. They're organized into categories based on their trigger points and use cases.
...
These hooks monitor and enhance user communication with <BrandName />. Use them to validate input, inject context, and track interaction patterns.

#### UserPromptSubmit

Runs when a user sends a message to <BrandName />. Use it to validate input, inject context based on the prompt, and track interaction patterns.
...
Runs when a new task begins. Use it to detect project type, initialize tracking, and inject initial context that shapes how <BrandName /> approaches the work.
...
These hooks monitor internal <BrandName /> operations and system-level events. Use them to track context usage, log system behavior, and analyze performance patterns.
...
Your hook script can output logging or diagnostic information to stdout during execution, as long as the JSON response is the last thing written. <BrandName /> will parse only the final JSON object from stdout.
...
- Look for errors in VSCode's Output panel (<BrandName /> channel)
...
Hooks complement other <BrandName /> features:

- [<BrandName /> Rules](/en/features/caret-rules) define high-level guidance that hooks can enforce
