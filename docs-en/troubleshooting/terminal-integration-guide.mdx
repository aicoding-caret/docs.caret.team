...
This guide helps you resolve terminal integration issues in Careti. Terminal integration is crucial for Careti to execute commands and read their output, enabling it to understand errors, test results, and command responses.

<Tip>
	If you're experiencing terminal issues, try switching to a simpler shell like `bash` in the Careti settings, under "Terminal Settings"
...
-   Commands execute but Careti can't read output
-   Terminal works fine manually but not with Careti
...
-   Careti states in chat: "[Command is running but producing no output]"
-   Commands complete but Careti doesn't see results
...
-   **Increase Shell Integration Timeout**
    ...
    1. Within Careti, left-click the **Settings** button in the top right-hand corner of the chat window
...
-   Recommend AI to use file output instead
    1. Tell Careti in chat or Careti rules, to use `command > output.txt` before reading the file/s

<Tip>
	This family of issues is only partially solved in the latest Careti versions, so if you still face this, create a GitHub issue
...
-   Docker builds never complete in Careti
-   Progress bars consume thousands of tokens
-   The Careti button "Proceed while running" doesn't work properly in chat

<Tip>
	This family of issues has been solved in latest Careti versions but if you still face any issues, then create a GitHub issue
	for this.
</Tip>

## Terminal Settings Explained

Access these in Careti by clicking the settings icon, and navigating to the "Terminal Settings" section:

### Default Terminal Profile

-   **What it does**: Selects which shell Careti uses for commands
...
-   **What it does**: How long Careti waits for the terminal to be ready
-   **Default**: 4 seconds
-   **When to increase**:
...
-   **What it does**: Limits how many lines Careti reads from terminal output
-   **Default**: 500 lines
-   **When to adjust**:
...
3. Select "WSL Bash" as terminal profile in Careti

#### Path Issues

Windows path problems:

1. Use forward slashes in Careti: `C:/Users/...`
...
1. Install Careti on the remote machine, not locally
...
1. Install Careti in the container
...
3. Choose "Trace"
4. Check Output panel → "Careti" for terminal logs
...
### Why does Careti create so many terminals?

When shell integration fails, Careti can't reuse terminals safely (they might be running long processes). Enable shell integration or adjust the terminal reuse setting.

### Can I use my custom shell (nushell, xonsh, etc.)?

Careti officially supports bash, zsh, fish, and PowerShell. Custom shells may work but aren't guaranteed. Use bash as a fallback.
...
Working integration shows command output in Careti's chat. Failed integration shows "Shell Integration Unavailable" or "[Command is running but producing no output]".
...
2. **Report the Issue**:
    - Use `/reportbug` in Careti github issues
