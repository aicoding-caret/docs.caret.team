...
description: "Complete guide to resolving terminal integration issues in Caret"
---

This guide helps you resolve terminal integration issues in Caret. Terminal integration is crucial for Caret to execute commands and read their output, enabling it to understand errors, test results, and command responses.

:::tip
	If you're experiencing terminal issues, try switching to a simpler shell like `bash` in the Caret settings, under "Terminal Settings"
...
-   Commands execute but Caret can't read output
-   Terminal works fine manually but not with Caret
...
-   Caret states in chat: "[Command is running but producing no output]"
-   Commands complete but Caret doesn't see results
...
-   **Increase Shell Integration Timeout**
    ...
    1. Within Caret, left-click the **Settings** button in the top right-hand corner of the chat window
...
-   Recommend AI to use file output instead
    1. Tell Caret in chat or Caret rules, to use `command > output.txt` before reading the file/s

:::tip
	This family of issues is only partially solved in the latest Caret versions, so if you still face this, create a GitHub issue
...
-   Docker builds never complete in Caret
-   Progress bars consume thousands of tokens
-   The Caret button "Proceed while running" doesn't work properly in chat

:::tip
	This family of issues has been solved in latest Caret versions but if you still face any issues, then create a GitHub issue
	for this.
:::

## Terminal Settings Explained

Access these in Caret by clicking the settings icon, and navigating to the "Terminal Settings" section:

### Default Terminal Profile

-   **What it does**: Selects which shell Caret uses for commands
...
-   **What it does**: How long Caret waits for the terminal to be ready
-   **Default**: 4 seconds
-   **When to increase**:
...
-   **What it does**: Limits how many lines Caret reads from terminal output
-   **Default**: 500 lines
-   **When to adjust**:
...
3. Select "WSL Bash" as terminal profile in Caret

#### Path Issues

Windows path problems:

1. Use forward slashes in Caret: `C:/Users/...`
...
1. Install Caret on the remote machine, not locally
...
1. Install Caret in the container
...
3. Choose "Trace"
4. Check Output panel → "Caret" for terminal logs
...
### Why does Caret create so many terminals?

When shell integration fails, Caret can't reuse terminals safely (they might be running long processes). Enable shell integration or adjust the terminal reuse setting.

### Can I use my custom shell (nushell, xonsh, etc.)?

Caret officially supports bash, zsh, fish, and PowerShell. Custom shells may work but aren't guaranteed. Use bash as a fallback.
...
Working integration shows command output in Caret's chat. Failed integration shows "Shell Integration Unavailable" or "[Command is running but producing no output]".
...
2. **Report the Issue**:
    - Use `/reportbug` in Caret github issues
