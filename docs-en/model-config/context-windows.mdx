...
<Note>
캐러티(Careti) 기준 문서입니다. Careti v3.38.1 머지본을 따르며, 캐러티 전용 정책(지원/차단 모델별 컨텍스트 제한, 인증/라우팅)이 있을 경우 본문에서 `<Note>`로 표시합니다.
</Note>
...
2. **File contents** - Any files you've shared or Careti has read
3. **Tool outputs** - Results from executed commands
4. **System prompts** - Careti's instructions (minimal impact)
...
Careti can automatically summarize long conversations:
...
**A:** Careti shows token usage in the interface. Watch for the context meter approaching limits.

### Q: What happens when I exceed the context limit?
**A:** Careti will either:
