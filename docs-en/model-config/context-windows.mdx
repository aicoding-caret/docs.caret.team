...
<Note>
캐러티(<BrandName />) 기준 문서입니다. <BrandName /> v3.38.1 머지본을 따르며, 캐러티 전용 정책(지원/차단 모델별 컨텍스트 제한, 인증/라우팅)이 있을 경우 본문에서 `<Note>`로 표시합니다.
</Note>
...
2. **File contents** - Any files you've shared or <BrandName /> has read
3. **Tool outputs** - Results from executed commands
4. **System prompts** - <BrandName />'s instructions (minimal impact)
...
<BrandName /> can automatically summarize long conversations:
...
**A:** <BrandName /> shows token usage in the interface. Watch for the context meter approaching limits.

### Q: What happens when I exceed the context limit?
**A:** <BrandName /> will either:
