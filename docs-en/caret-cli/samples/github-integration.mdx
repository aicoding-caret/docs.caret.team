...
<Note>
**New to Careti CLI?** This sample assumes you understand Careti CLI basics and have completed the [Installation Guide](https://docs.caret.bot/caret-cli/installation). If you're new to Careti CLI, we recommend starting with the [GitHub RCA sample](/en/caret-cli/samples/github-issue-rca) first, as it's simpler and will help you understand the fundamentals before setting up GitHub Actions.
</Note>

## The Workflow

Trigger Careti by mentioning `@caret` in any issue comment:

<Frame>
  <img src="https://storage.googleapis.com/cline_public_images/ss0a-comment.png" alt="Issue comment with @caret mention" width="600" />
</Frame>

Careti's automated analysis appears as a new comment, with insights drawn from your actual codebase:
...
- **Careti CLI knowledge** - Completed the [Installation Guide](https://docs.caret.bot/caret-cli/installation) and understand basic usage
...
start up an instance of the Careti CLI to dig into the issue, providing feedback
...
Now you're ready to supply Careti with the credentials it needs in a GitHub Action.
...
This analysis script calls Careti to execute a prompt on a GitHub issue,
...
3. **Installs** Careti CLI globally using npm
4. **Creates** a Careti instance using `caret instance new`
