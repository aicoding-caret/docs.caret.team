...
description: "Automatically respond to GitHub issues by mentioning @caret in comments using Careti CLI in GitHub Actions."
---
...
<Note>
**New to <BrandName /> CLI?** This sample assumes you understand <BrandName /> CLI basics and have completed the [Installation Guide](https://docs.caret.bot/caret-cli/installation). If you're new to <BrandName /> CLI, we recommend starting with the [GitHub RCA sample](/en/caret-cli/samples/github-issue-rca) first, as it's simpler and will help you understand the fundamentals before setting up GitHub Actions.
</Note>

## The Workflow

Trigger <BrandName /> by mentioning `@caret` in any issue comment:

<Frame>
  <img src="https://storage.googleapis.com/cline_public_images/ss0a-comment.png" alt="Issue comment with @caret mention" width="600" />
</Frame>

<BrandName />'s automated analysis appears as a new comment, with insights drawn from your actual codebase:
...
- **<BrandName /> CLI knowledge** - Completed the [Installation Guide](https://docs.caret.bot/caret-cli/installation) and understand basic usage
...
<Accordion title="Click to view the complete caret-responder.yml workflow">
```yaml
...
start up an instance of the <BrandName /> CLI to dig into the issue, providing feedback
...
2. Navigate to **Settings** → **Environment** and Add a new environment.
   ...
   Make sure to name it "caret-actions" so that it matches the `environment`
   value at the top of the `caret-responder.yml` file.
...
Now you're ready to supply <BrandName /> with the credentials it needs in a GitHub Action.
...
This analysis script calls <BrandName /> to execute a prompt on a GitHub issue,
...
3. **Installs** <BrandName /> CLI globally using npm
4. **Creates** a <BrandName /> instance using `caret instance new`
5. **Configures** authentication using `caret config set open-router-api-key=...
   --address ...`
