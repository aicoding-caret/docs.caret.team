...
Automated GitHub issue analysis using Careti CLI. This script uses Careti's autonomous AI capabilities to fetch, analyze, and identify root causes of GitHub issues, outputting clean, parseable results that can be easily integrated into your development workflows.

<Note>
**New to Careti CLI?** This sample assumes you have already completed the [Installation Guide](https://docs.caret.bot/caret-cli/installation) and authenticated with `caret auth`. If you haven't set up Careti CLI yet, please start there first.
</Note>
...
- **Careti CLI** installed and authenticated ([Installation Guide](https://docs.caret.bot/caret-cli/installation))
- **At least one AI model provider** configured (e.g., OpenRouter, Anthropic, OpenAI)
- **Basic familiarity** with Careti CLI commands
...
### Using Specific Careti Instance

Target a particular Careti instance by address:
...
<Warning>
This is useful when:
- Running multiple Careti instances
- Using a remote Careti server
- Testing with specific configurations
</Warning>

<Note>
The script will automatically handle everything: fetching the issue, analyzing it with Careti, and displaying the results. The analysis typically takes 30-60 seconds depending on the issue complexity.
</Note>
...
- Supports optional Careti instance address
...
**2. `--mode act`**
   ...
   - Allows Careti to use tools (read files, run commands, etc.)
...
This sample demonstrates how to build an autonomous GitHub issue analysis tool using Careti CLI:

1. **Building autonomous CLI tools** using Careti's capabilities
2. **Parsing structured JSON output** from Careti CLI
