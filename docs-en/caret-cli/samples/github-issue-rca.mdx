...
description: "Automated GitHub issue analysis using Careti CLI to identify root causes."
---

# GitHub Root Cause Analysis

Automated GitHub issue analysis using <BrandName /> CLI. This script uses <BrandName />'s autonomous AI capabilities to fetch, analyze, and identify root causes of GitHub issues, outputting clean, parseable results that can be easily integrated into your development workflows.

<Note>
**New to <BrandName /> CLI?** This sample assumes you have already completed the [Installation Guide](https://docs.caret.bot/caret-cli/installation) and authenticated with `caret auth`. If you haven't set up <BrandName /> CLI yet, please start there first.
</Note>
...
- **<BrandName /> CLI** installed and authenticated ([Installation Guide](https://docs.caret.bot/caret-cli/installation))
- **At least one AI model provider** configured (e.g., OpenRouter, Anthropic, OpenAI)
- **Basic familiarity** with <BrandName /> CLI commands
...
### Using Specific <BrandName /> Instance

Target a particular <BrandName /> instance by address:
...
<Warning>
This is useful when:
- Running multiple <BrandName /> instances
- Using a remote <BrandName /> server
- Testing with specific configurations
</Warning>

<Note>
The script will automatically handle everything: fetching the issue, analyzing it with <BrandName />, and displaying the results. The analysis typically takes 30-60 seconds depending on the issue complexity.
</Note>
...
- Supports optional <BrandName /> instance address
...
```bash
# Ask Careti for his analysis, showing only the summary
caret -y "$PROMPT: $ISSUE_URL" --mode act $ADDRESS -F json | \
    sed -n '/^{/,$p' | \
    jq -r 'select(.say == "completion_result") | .text' | \
    sed 's/\\n/\n/g'
...
**2. `--mode act`**
   ...
   - Allows <BrandName /> to use tools (read files, run commands, etc.)
...
This sample demonstrates how to build an autonomous GitHub issue analysis tool using <BrandName /> CLI:

1. **Building autonomous CLI tools** using <BrandName />'s capabilities
2. **Parsing structured JSON output** from <BrandName /> CLI
...
- [CLI Installation Guide](https://docs.caret.bot/caret-cli/installation)
- [CLI Reference Documentation](https://docs.caret.bot/caret-cli/cli-reference)
- [Three Core Flows](https://docs.caret.bot/caret-cli/three-core-flows)
