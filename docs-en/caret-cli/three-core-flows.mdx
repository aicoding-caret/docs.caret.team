...
description: "Learn the three ways to use Careti CLI: interactive mode, headless automation, and multi-instance parallelization"
---

Two concepts to understand:

**Task** - A single job for <BrandName /> to complete ("add tests to utils.js"). You describe what you want, <BrandName /> plans how to do it, then executes the plan. Tasks run on instances.

**Instance** - An independent <BrandName /> workspace. Each instance runs one task at a time. Create multiple instances to run multiple tasks that work on different parts of your project in parallel.

## 1. Interactive mode: Plan first, then act

Start here to see how <BrandName /> works. Interactive mode opens a chat session where you can review plans before execution.

```bash
caret
```

<BrandName /> opens an interactive session in your current directory. Type your task as a message. <BrandName /> enters Plan mode and proposes a step-by-step strategy.
...
<BrandName /> executes the approved steps—reading files, writing code, running commands. You maintain control throughout the process.
...
With the `-y` (YOLO) flag, <BrandName /> plans and executes autonomously without interactive chat. Perfect for CI, cron jobs, or scripts.
...
- **Interactive mode**: Best for exploring new problems, learning how <BrandName /> works, or when you want to review plans before execution
- **Headless single-shot**: Perfect for automation, CI/CD, and tasks where you trust <BrandName /> to execute without supervision
...
<Columns cols={2}>
  ...
  <Card title="Task management" icon="clipboard-check" href="/getting-started/task-management">
    Learn how <BrandName /> tracks and manages tasks, including saving and restoring state from checkpoints.
  </Card>
</Columns>
