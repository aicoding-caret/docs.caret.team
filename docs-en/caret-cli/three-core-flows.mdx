...
**Task** - A single job for Careti to complete ("add tests to utils.js"). You describe what you want, Careti plans how to do it, then executes the plan. Tasks run on instances.

**Instance** - An independent Careti workspace. Each instance runs one task at a time. Create multiple instances to run multiple tasks that work on different parts of your project in parallel.

## 1. Interactive mode: Plan first, then act

Start here to see how Careti works. Interactive mode opens a chat session where you can review plans before execution.

```bash
caret
```

Careti opens an interactive session in your current directory. Type your task as a message. Careti enters Plan mode and proposes a step-by-step strategy.
...
Careti executes the approved steps—reading files, writing code, running commands. You maintain control throughout the process.
...
With the `-y` (YOLO) flag, Careti plans and executes autonomously without interactive chat. Perfect for CI, cron jobs, or scripts.
...
- **Interactive mode**: Best for exploring new problems, learning how Careti works, or when you want to review plans before execution
- **Headless single-shot**: Perfect for automation, CI/CD, and tasks where you trust Careti to execute without supervision
...
<Columns cols={2}>
  ...
  <Card title="Task management" icon="clipboard-check" href="/getting-started/task-management">
    Learn how Careti tracks and manages tasks, including saving and restoring state from checkpoints.
  </Card>
</Columns>
