...
Git Mentions позволяют добавлять историю репозитория и изменения напрямую в диалог с Caret. Вы можете ссылаться на незафиксированные изменения с помощью `@git-changes` или на конкретные коммиты через `@[commit-hash]`.

Когда вы вводите `@` в чате, вы можете выбрать «Git Changes» из меню или ввести `@git-changes` вручную. Для выбора конкретных коммитов введите `@`, а затем хеш коммита (минимум 7 символов). Caret мгновенно увидит git status, diff-ы, сообщения коммитов и другую важную информацию.
...
Это предоставляет Caret полную информацию о коммите, включая сообщение коммита, автора, дату и полный diff. Затем Caret сможет проанализировать, что именно изменилось и как это может повлиять на другие части codebase.
...
Это показывает Caret все ваши незафиксированные изменения, включая новые файлы, измененные файлы и их diff-ы. После этого Caret сможет просмотреть ваши правки и дать отзыв по реализации.
...
В следующий раз, когда будете работать с изменениями кода или изучать проблему, попробуйте использовать Git Mentions вместо того, чтобы вручную описывать или копировать изменения. Вы получите более точную помощь, так как Caret увидит, что именно изменилось и в каком контексте.
...
1. Когда вы отправляете сообщение, Caret обнаруживает паттерн `@git-changes` в тексте.
...
1. Когда вы отправляете сообщение, Caret обнаруживает символ `@`, за которым следует хеш коммита.
