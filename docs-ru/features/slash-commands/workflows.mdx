...
Workflows позволяют определять последовательность шагов, чтобы направлять Caret при выполнении повторяющихся задач, таких как развертывание сервиса или отправка PR.

Чтобы вызвать workflow, введите `/[workflow-name.md]` в чате.

## Как создавать и использовать Workflows

Workflows находятся рядом с [Caret Rules](/russian/features/caret-rules). Создать их очень просто:

<Frame>
	<img src="https://storage.googleapis.com/cline_public_images/docs/assets/workflows.png" alt="Вкладка Workflows в Caret" />
</Frame>

1. Создайте Markdown-файл с четкими инструкциями по шагам, которые должен выполнить Caret.
...
-   Использовать [встроенные инструменты](/russian/exploring-carets-tools/caret-tools-guide) Caret, такие как `ask_followup_question`, `read_file`, `search_files` и `new_task`.
...
2. Вставляю номер PR
3. Позволяю Caret сделать все остальное

Мой workflow использует инструмент командной строки `gh` и встроенную функцию Caret `ask_followup_question`, чтобы:
...
-   Если я отвечаю «да», Caret автоматически одобряет PR с помощью команды `gh`
