...
Workflows позволяют определять последовательность шагов, чтобы направлять Careti при выполнении повторяющихся задач, таких как развертывание сервиса или отправка PR.

Чтобы вызвать workflow, введите `/[workflow-name.md]` в чате.

## Как создавать и использовать Workflows

Workflows находятся рядом с [Careti Rules](/russian/features/caret-rules). Создать их очень просто:

<Frame>
	<img src="https://storage.googleapis.com/cline_public_images/docs/assets/workflows.png" alt="Вкладка Workflows в Careti" />
</Frame>

1. Создайте Markdown-файл с четкими инструкциями по шагам, которые должен выполнить Careti.
2. Сохраните его с расширением `.md` в вашей директории workflows.
3. Чтобы запустить workflow, просто введите `/`, а затем имя файла workflow.
4. Укажите необходимые параметры по запросу.

Настоящая сила заключается в том, как вы структурируете файлы workflow. Вы можете:

-   Использовать [встроенные инструменты](/russian/exploring-carets-tools/caret-tools-guide) Careti, такие как `ask_followup_question`, `read_file`, `search_files` и `new_task`.
-   Использовать уже установленные инструменты командной строки, такие как `gh` или `docker`.
-   Обращаться к внешним [вызовам инструментов MCP](/russian/mcp/mcp-overview), таким как Slack или Whatsapp.
-   Связывать несколько действий в определенную последовательность.

## Пример из реальной практики

Я создал workflow для PR Review, который уже экономит мне массу времени.
...
Раньше, когда я получал новый PR для ревью, я собирал контекст вручную: изучал описание PR, проверял diff, просматривал связанные файлы и только потом формировал мнение. Теперь я просто:

1. Ввожу `/pr-review.md` в чате
2. Вставляю номер PR
3. Позволяю Careti сделать все остальное

Мой workflow использует инструмент командной строки `gh` и встроенную функцию Careti `ask_followup_question`, чтобы:

-   Получить описание PR и комментарии
-   Изучить diff
-   Проверить окружающие файлы для понимания контекста
-   Проанализировать потенциальные проблемы
-   Спросить меня, можно ли одобрить PR, если все выглядит хорошо, с обоснованием того, почему он должен быть одобрен
-   Если я отвечаю «да», Careti автоматически одобряет PR с помощью команды `gh`

Это превратило мой процесс ревью PR из ручной многоэтапной операции в одну команду, которая дает мне все необходимое для принятия взвешенного решения.

> Это всего лишь один пример файла workflow. Вы можете найти больше примеров в нашем [репозитории промптов](https://github.com/caret/prompts) для вдохновения.

## Создание собственных Workflows

Прелесть Workflows в том, что они полностью настраиваются под ваши нужды. Вы можете создавать их для любых повторяющихся задач:

-   Для релизов можно создать workflow, который собирает все объединенные PR, создает changelog и обновляет версии.
-   Настройка новых проектов идеально подходит для Workflows. Просто запустите одну команду, чтобы создать структуру папок, установить зависимости и настроить конфигурации.
-   Нужно создать отчет? Создайте workflow, который собирает статистику из разных источников и форматирует ее именно так, как вам нравится. Вы даже можете визуализировать данные с помощью библиотеки графиков, а затем сделать из этого презентацию с помощью чего-то вроде [slidev](https://sli.dev/).
-   Вы даже можете использовать Workflows для отправки сообщений вашей команде через MCP-сервер, например Slack или Whatsapp, после отправки PR.

С Workflows предел — только ваше воображение. Настоящий потенциал раскрывается, когда вы замечаете те самые раздражающие повторяющиеся задачи, которые выполняете постоянно.

Если вы можете описать процесс как «сначала я делаю X, затем Y, затем Z» — это идеальный кандидат для workflow.

Начните с чего-то небольшого, что вас беспокоит, превратите это в workflow и продолжайте совершенствовать его. Вы будете поражены тем, какую часть вашего рабочего дня можно автоматизировать таким образом.