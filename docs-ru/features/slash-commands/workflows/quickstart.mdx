...
description: "Пошаговое руководство по созданию вашего первого Caret workflow"
---
...
<Steps>
  ...
  <Step title="Запустите workflow">
    ...
    1. Откройте панель чата Caret
    2. Введите `/pr-review.md`, а затем номер PR (например, `/pr-review.md 42`)
    3. Caret получит детали PR, проанализирует код и покажет результаты

    <Tip>
      Когда Caret выполняет команды, такие как `gh pr view`, он может приостановиться, чтобы показать вывод. Нажмите **Proceed While Running**, чтобы Caret продолжил.
    </Tip>
  </Step>
</Steps>
...
Ищите повторяющиеся задачи в вашем workflow и превращайте их в эффективные Caret workflows.
