...
---
title: "Обзор MCP"
description: "Узнайте о серверах Model Context Protocol (MCP), их возможностях и о том, как Caret может помочь в их создании и использовании. MCP стандартизирует способ, которым приложения предоставляют контекст для LLM, действуя как порт USB-C для AI-приложений."
---
...
Caret не поставляется с предустановленными MCP-серверами. Вам нужно будет найти и установить их отдельно.
...
-   **Caret Marketplace:** Установите один из Caret [MCP Marketplace](/russian/mcp/mcp-marketplace)
-   **Ask Caret:** Вы можете попросить Caret помочь вам найти или создать MCP-серверы
...
Caret упрощает создание и использование MCP-серверов благодаря своим AI-возможностям.

### Создание MCP-серверов

-   **Natural language understanding:** Поручите Caret на естественном языке создать MCP-сервер, описав его функциональные возможности. Caret интерпретирует ваши инструкции и сгенерирует необходимый code.
-   **Cloning and building servers:** Caret может клонировать существующие репозитории MCP-серверов из GitHub и автоматически их собирать.
-   **Configuration and dependency management:** Caret обрабатывает configuration files, environment variables и dependencies.
-   **Troubleshooting and debugging:** Caret помогает выявлять и устранять ошибки во время разработки.

### Использование MCP-серверов

-   **Tool execution:** Caret беспрепятственно интегрируется с MCP-серверами, позволяя вам выполнять их определенные tools.
-   **Context-aware interactions:** Caret может интеллектуально предлагать использование соответствующих tools на основе контекста разговора.
-   **Dynamic integrations:** Объедините возможности нескольких MCP-серверов для выполнения сложных задач. Например, Caret может использовать GitHub server для получения данных и Notion server для создания отформатированного отчета.
