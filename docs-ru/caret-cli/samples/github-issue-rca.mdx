...
description: "Автоматизированный анализ issues на GitHub с использованием Careti CLI для определения первопричин."
---

# Анализ первопричин GitHub

Автоматизированный анализ issues на GitHub с использованием Careti CLI. Этот скрипт использует автономные AI-возможности Careti для получения, анализа и определения первопричин issues на GitHub, выводя чистые, легко анализируемые результаты, которые можно легко интегрировать в ваши процессы разработки.

<Note>
**Впервые используете Careti CLI?** Этот пример предполагает, что вы уже выполнили [Руководство по установке](https://docs.caret.bot/russian/caret-cli/installation) и аутентифицировались с помощью `caret auth`. Если вы еще не настроили Careti CLI, пожалуйста, начните с этого.
</Note>
...
- **Careti CLI** установлен и аутентифицирован ([Руководство по установке](https://docs.caret.bot/russian/caret-cli/installation))
- **Как минимум один провайдер AI-моделей** сконфигурирован (например, OpenRouter, Anthropic, OpenAI)
- **Базовое знакомство** с командами Careti CLI
...
### Использование конкретного экземпляра Careti

Укажите конкретный экземпляр Careti по адресу:
...
<Warning>
Это полезно, когда:
- Запущено несколько экземпляров Careti
- Используется удаленный сервер Careti
- Тестируется с определенными конфигурациями
</Warning>

<Note>
Скрипт автоматически обработает все: получение issue, анализ с помощью Careti и отображение результатов. Анализ обычно занимает 30-60 секунд в зависимости от сложности issue.
</Note>
...
- Поддерживает необязательный адрес экземпляра Careti
...
**2. `--mode act`**
   ...
   - Позволяет Careti использовать инструменты (читать файлы, выполнять команды и т. д.)
...
Этот пример демонстрирует, как создать автономный инструмент анализа issues на GitHub с использованием Careti CLI:

1. **Создание автономных инструментов CLI** с использованием возможностей Careti
2. **Парсинг структурированного JSON-вывода** из Careti CLI
