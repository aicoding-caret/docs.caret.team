...
description: "Изучите три способа использования Careti CLI: интерактивный режим, автоматизация без участия пользователя и параллелизация с использованием нескольких экземпляров"
---

Два понятия, которые необходимо понимать:

**Task** - Единая задача, которую Careti должен выполнить ("добавить тесты в utils.js"). Вы описываете, что хотите, Careti планирует, как это сделать, а затем выполняет план. Задачи выполняются на instances.

**Instance** - Независимая рабочая область Careti. Каждый instance выполняет одну задачу за раз. Создайте несколько instances для параллельного выполнения нескольких tasks, работающих с разными частями вашего проекта.

## 1. Интерактивный режим: Сначала планируйте, потом действуйте

Начните здесь, чтобы увидеть, как работает Careti. Интерактивный режим открывает сеанс чата, где вы можете просматривать планы перед выполнением.

```bash
caret
```

Careti открывает интерактивный сеанс в вашем текущем каталоге. Введите свою задачу в виде сообщения. Careti переходит в режим Plan и предлагает пошаговую стратегию.
...
Careti выполняет утвержденные шаги — чтение файлов, написание кода, выполнение команд. Вы сохраняете контроль на протяжении всего процесса.
...
С флагом `-y` (YOLO) Careti планирует и выполняет автономно без интерактивного чата. Идеально подходит для CI, cron jobs или скриптов.
...
- **Интерактивный режим**: Лучше всего подходит для изучения новых проблем, изучения того, как работает Careti, или когда вы хотите просмотреть планы перед выполнением
- **Headless single-shot**: Идеально подходит для автоматизации, CI/CD и tasks, где вы доверяете Careti выполнять их без присмотра
...
<Columns cols={2}>
  ...
  <Card title="Task management" icon="clipboard-check" href="/russian/getting-started/task-management">
    Узнайте, как Careti отслеживает tasks и управляет ими, включая сохранение и восстановление состояния из checkpoints.
  </Card>
</Columns>
```