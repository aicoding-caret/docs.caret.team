...
description: "Полный справочник по командам Careti CLI, включая конфигурацию, управление экземплярами и командами задач"
---

Полный справочник по командам Careti CLI. Используйте его для подробной документации по всем командам, опциям и конфигурации.
...
Полная страница руководства для Careti CLI:

```
CARET(1)                         User Commands                        CARET(1)

NAME
       caret - управление и взаимодействие с AI-агентами кодирования Careti
...
DESCRIPTION
       ...
       агентами кодирования Careti. Careti - это автономный AI-агент, который
       может читать, писать и выполнять код в ваших проектах. Он работает
       через клиент-серверную архитектуру, где Careti Core работает как
       ...
       одному и тому же экземпляру Careti Core, обеспечивая плавную передачу
       задач между средами.
...
AGENT BEHAVIOR
       Careti работает в двух основных режимах:

       ACT MODE
              Careti активно использует инструменты для выполнения задач. Он
              может читать файлы, писать код, выполнять команды, использовать
              headless browser и многое другое. Это режим по умолчанию для
              выполнения задач.

       PLAN MODE
              Careti собирает информацию и создает подробный план перед
...
INSTANT TASK OPTIONS
       ...
       -o, --oneshot
              Полный автономный режим. Careti выполняет задачу и останавливается
...
COMMANDS
   ...
   Instance Management
       Экземпляры Careti Core — это независимые процессы агента, которые могут
       ...
       caret i n [-d|--default]
              Создать новый экземпляр Careti Core. Используйте --default, чтобы
       ...
       caret i l
              Перечислить все работающие экземпляры Careti Core с их адресами и
              статусом.
       ...
       caret i k address [-a|--all]
              Завершить экземпляр Careti Core. Используйте --all, чтобы
              завершить все работающие экземпляры.

   Task Management
       Задачи представляют собой отдельные рабочие элементы, которые выполняет
       Careti. Задачи поддерживают историю разговоров, контрольные точки и
       настройки.
       ...
       caret t [-a|--address ADDR]
              ...
              указывает, какой экземпляр Careti Core использовать (например,
              localhost:50052).
       ...
       caret t c
              ...
              вести двусторонний разговор с Careti.

       caret task send [message] [options]

       caret t s [message] [options]
              Отправить сообщение Careti. Если сообщение не предоставлено,
              считывает из stdin. Параметры:

              -a, --approve
                     Утвердить предложенное действие Careti

              -d, --deny
                     Отклонить предложенное действие Careti
...
NOTES & EXAMPLES
   ...
   Instance Management
       Управление несколькими экземплярами Careti:
...
ARCHITECTURE
       Careti работает по трехуровневой архитектуре:

       Presentation Layer
              Пользовательские интерфейсы (CLI, VSCode, JetBrains), которые
              подключаются к Careti Core через gRPC
       ...
       Host Provider Layer
              ...
              shell API), которые Careti Core использует для взаимодействия с
              хост-системой
...
AUTHORS
       Careti разработан Careti Bot Inc. и сообществом open source.

COPYRIGHT
       Copyright © 2025 Careti Bot Inc. Лицензировано в соответствии с
...
Переопределите расположение каталога Careti по умолчанию:
