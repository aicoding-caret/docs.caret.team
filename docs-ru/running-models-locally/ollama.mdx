---
title: "Ollama"
description: "Краткое руководство по настройке Ollama для локального запуска моделей ИИ в Caret."
---

<Note>
Это документация для Caret. Она основана на версии Caret v3.38.1. Если существуют специфические политики Caret (поддерживаемые локальные среды исполнения, аутентификация/маршрутизация, ограничения моделей), они будут отмечены в тексте как `<Note>`.
</Note>

### Предварительные условия

-   Компьютер под управлением Windows, macOS или Linux
-   Установленное расширение Caret в VS Code
...
Теперь ваша модель готова к использованию в Caret.
...
Откройте VS Code и настройте Caret:

1. Нажмите на иконку настроек Caret
...
Для наилучшей работы с Caret используйте **Qwen3 Coder 30B**. Эта модель обладает отличными навыками написания кода и надежно работает с инструментами при локальной разработке.
...
-   Запустите Ollama перед использованием Caret
...
Для повышения производительности при работе с локальными моделями включите компактные промпты в настройках Caret. Это уменьшит размер промпта на 90%, сохраняя при этом основные функции.

Перейдите в Caret Settings → Features → Use Compact Prompt и включите этот параметр.

### Устранение неполадок

Если Caret не может подключиться к Ollama:
