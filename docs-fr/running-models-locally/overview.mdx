...
<Note>
Ceci est un document de référence pour Caret. Il suit la version fusionnée de Caret v3.38.1. Si des politiques spécifiques à Caret (runtime local supporté, authentification/routage, limitations des modèles) s'appliquent, elles seront indiquées par une balise `<Note>` dans le texte.
</Note>

## Exécuter des modèles localement avec Caret

Exécutez Caret entièrement hors ligne avec des modèles réellement performants sur votre propre matériel. Pas de coûts d'API, pas de données quittant votre machine, pas de dépendance à Internet.

Les modèles locaux ont atteint un tournant où ils sont désormais exploitables pour un travail de développement réel. Ce guide couvre tout ce que vous devez savoir pour exécuter Caret avec des modèles locaux.
...
| 64GB | Qwen3 Coder 30B | 8-bit | Fonctionnalités Caret complètes |
...
Après des tests approfondis, **Qwen3 Coder 30B** est le modèle le plus fiable sous la barre des 70B paramètres pour Caret :
...
- **Fiabilité éprouvée** - Résultats cohérents avec le format d'outils de Caret
...
La plupart des modèles de moins de 30B paramètres (7B-20B) échouent avec Caret car ils :
...
- Passez à bash dans Caret Settings → Terminal → Default Terminal Profile
...
Les modèles locaux avec Caret sont désormais véritablement exploitables. Bien qu'ils n'égalent pas les API cloud de premier plan en termes de vitesse, ils offrent une confidentialité totale, un coût nul et une capacité hors ligne. Avec une configuration appropriée et le bon matériel, Qwen3 Coder 30B peut gérer efficacement la plupart des tâches de codage.
