...
description: "Guide complet pour résoudre les problèmes d'intégration du terminal dans Caret"
---

Ce guide vous aide à résoudre les problèmes d'intégration du terminal dans Caret. L'intégration du terminal est cruciale pour que Caret puisse exécuter des commandes et lire leur sortie, lui permettant ainsi de comprendre les erreurs, les résultats de tests et les réponses aux commandes.

<Tip>
	Si vous rencontrez des problèmes de terminal, essayez de passer à un shell plus simple comme `bash` dans les réglages de Caret, sous "Terminal Settings".
...
```mermaid
graph TD
    A[Terminal Issue] --> B{Can Caret execute commands?}
    B -->|No| C[Shell Integration Unavailable]
    B -->|Yes| D{Can Caret see the output?}
...
-   Les commandes s'exécutent mais Caret ne peut pas lire la sortie
-   Le terminal fonctionne normalement manuellement mais pas avec Caret
...
-   **Passer à bash**
    ...
    1. Allez dans les Caret Settings
...
-   **Utiliser PowerShell 7**
    ...
    2. Allez dans les Caret Settings
...
-   Caret indique dans le chat : "[Command is running but producing no output]"
-   Les commandes se terminent mais Caret ne voit pas les résultats
...
-   Recommander à l'AI d'utiliser une sortie dans un fichier à la place
    1. Dites à Caret dans le chat ou via les Caret rules d'utiliser `command > output.txt` avant de lire le ou les fichiers

<Tip>
	Cette famille de problèmes n'est que partiellement résolue dans les dernières versions de Caret. Si vous y faites toujours face de manière persistante, créez une issue sur GitHub.
</Tip>
...
-   Les builds Docker ne se terminent jamais dans Caret
-   Les barres de progression consomment des milliers de tokens
-   Le bouton Caret "Proceed while running" ne fonctionne pas correctement dans le chat

<Tip>
	Cette famille de problèmes a été résolue dans les dernières versions de Caret, mais si vous rencontrez encore des soucis, créez une issue GitHub à ce sujet.
</Tip>

## Explication des Terminal Settings

Accédez à ces réglages dans Caret en cliquant sur l'icône de paramètres, puis en naviguant vers la section "Terminal Settings" :

### Default Terminal Profile

-   **Ce que cela fait** : Sélectionne le shell utilisé par Caret pour les commandes
...
-   **Ce que cela fait** : Temps pendant lequel Caret attend que le terminal soit prêt
...
-   **Ce que cela fait** : Limite le nombre de lignes que Caret lit depuis la sortie du terminal
...
3. Sélectionnez "WSL Bash" comme profil de terminal dans Caret

#### Problèmes de chemins (Paths)

Problèmes de chemins sur Windows :

1. Utilisez des barres obliques (forward slashes) dans Caret : `C:/Users/...`
...
1. Installez Caret sur la machine distante, pas localement
...
1. Installez Caret dans le container
...
3. Choisissez "Trace"
4. Vérifiez le panneau Output → "Caret" pour les logs du terminal
...
### Pourquoi Caret crée-t-il autant de terminaux ?

Lorsque Shell Integration échoue, Caret ne peut pas réutiliser les terminaux en toute sécurité (ils pourraient exécuter des processus longs). Activez Shell Integration ou ajustez le réglage de réutilisation du terminal.

### Puis-je utiliser mon shell personnalisé (nushell, xonsh, etc.) ?

Caret supporte officiellement bash, zsh, fish et PowerShell. Les shells personnalisés peuvent fonctionner mais ne sont pas garantis. Utilisez bash comme solution de repli.
...
Une intégration fonctionnelle affiche la sortie de la commande dans le chat de Caret. Une intégration échouée affiche "Shell Integration Unavailable" ou "[Command is running but producing no output]".
...
2. **Signalez le problème** :
    - Utilisez `/reportbug` dans les issues GitHub de Caret
