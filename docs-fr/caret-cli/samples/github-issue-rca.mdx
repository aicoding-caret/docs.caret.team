...
description: "Analyse automatisée des GitHub issues à l'aide de Careti CLI pour identifier les causes profondes."
---

# Analyse de Cause Racine GitHub

Analyse automatisée des GitHub issues à l'aide de Careti CLI. Ce script utilise les capacités d'IA autonome de Careti pour récupérer, analyser et identifier les causes profondes des GitHub issues, produisant des résultats propres et analysables qui peuvent être facilement intégrés à vos flux de travail de développement.

<Note>
**Nouveau sur Careti CLI ?** Cet exemple suppose que vous avez déjà suivi le [Guide d'installation](https://docs.caret.bot/french/caret-cli/installation) et que vous vous êtes authentifié avec `caret auth`. Si vous n'avez pas encore configuré Careti CLI, veuillez commencer par là.
</Note>
...
- **Careti CLI** installé et authentifié ([Guide d'installation](https://docs.caret.bot/french/caret-cli/installation))
- **Au moins un fournisseur de modèle d'IA** configuré (par exemple, OpenRouter, Anthropic, OpenAI)
- **Une connaissance de base** des commandes Careti CLI
...
### Utilisation d'une instance Careti spécifique

Ciblez une instance Careti particulière par adresse :
...
- Vous exécutez plusieurs instances de Careti
- Vous utilisez un serveur Careti distant
- Vous effectuez des tests avec des configurations spécifiques
</Warning>

<Note>
Le script gérera automatiquement tout : la récupération de l'issue, son analyse avec Careti et l'affichage des résultats. L'analyse prend généralement 30 à 60 secondes en fonction de la complexité de l'issue.
</Note>
...
- Prend en charge l'adresse d'instance Careti facultative
...
**2. `--mode act`**
   ...
   - Permet à Careti d'utiliser des outils (lire des fichiers, exécuter des commandes, etc.)
...
Cet exemple montre comment créer un outil autonome d'analyse des GitHub issues à l'aide de Careti CLI :

1. **Création d'outils CLI autonomes** à l'aide des capacités de Careti
2. **Analyse de la sortie JSON structurée** de Careti CLI
