...
description: "Répondez automatiquement aux *issues* GitHub en mentionnant @caret dans les commentaires en utilisant Caret CLI dans GitHub Actions."
---
...
<Note>
**Nouveau sur Caret CLI ?** Cet exemple suppose que vous comprenez les bases de Caret CLI et que vous avez terminé le [Guide d'installation](https://docs.caret.bot/french/caret-cli/installation). Si vous êtes novice en Caret CLI, nous vous recommandons de commencer par l'[exemple GitHub RCA](/french/caret-cli/samples/github-issue-rca) en premier, car il est plus simple et vous aidera à comprendre les fondamentaux avant de configurer GitHub Actions.
</Note>

## Le Workflow

Déclenchez Caret en mentionnant `@caret` dans n'importe quel commentaire d'*issue* :

<Frame>
  <img src="https://storage.googleapis.com/cline_public_images/ss0a-comment.png" alt="Issue comment with @caret mention" width="600" />
</Frame>

L'analyse automatisée de Caret apparaît sous forme de nouveau commentaire, avec des informations tirées de votre base de code réelle :
...
- **Connaissance de Caret CLI** - Avoir terminé le [Guide d'installation](https://docs.caret.bot/french/caret-cli/installation) et comprendre l'utilisation de base
...
Le *workflow* recherchera les *issues* nouvelles ou mises à jour, vérifiera les mentions `@caret`, puis démarrera une instance de Caret CLI pour examiner l'*issue*, en fournissant une réponse sous forme de commentaire à l'*issue*.
...
Vous êtes maintenant prêt à fournir à Caret les informations d'identification dont il a besoin dans une GitHub Action.
...
Ce script d'analyse appelle Caret pour exécuter une invite sur une *issue* GitHub, en résumant la sortie pour remplir la réponse à l'*issue*.
...
3. **Installe** Caret CLI globalement en utilisant `npm`
4. **Crée** une instance de Caret en utilisant `caret instance new`
