...
title: "Hooks"
sidebarTitle: "Hooks"
description: "Injectez une logique personnalisée dans le workflow de Caret pour valider les opérations, surveiller l'utilisation des outils et orienter les décisions de l'IA"
---

Les Hooks vous permettent d'injecter une logique personnalisée dans le workflow de Caret à des moments clés. Considérez-les comme des points de contrôle automatisés où vous pouvez valider les opérations avant leur exécution, surveiller l'utilisation des outils en temps réel et influencer la manière dont Caret prend des décisions.
...
L'activation des hooks dans Caret est simple. Voici ce que vous devez faire :

<Steps>
<Step title="Activer les Hooks dans les Paramètres">
Ouvrez les paramètres de Caret et cochez la case **"Enable Hooks"**.
...
Démarrez une tâche dans Caret et vérifiez que votre hook s'exécute.
</Step>
</Steps>
...
La clé est de combiner les hooks avec des outils externes. Un hook peut être le lien entre le workflow de Caret et le reste de votre écosystème de développement.


## Types de Hooks

Caret propose plusieurs types de hooks qui vous permettent de vous brancher sur différentes étapes du workflow de l'IA. Ils sont organisés en catégories basées sur leurs points de déclenchement et leurs cas d'utilisation.
...
Ces hooks surveillent et améliorent la communication de l'utilisateur avec Caret. Utilisez-les pour valider les entrées, injecter du contexte et suivre les schémas d'interaction.

#### UserPromptSubmit

S'exécute lorsqu'un utilisateur envoie un message à Caret. Utilisez-le pour valider l'entrée, injecter du contexte basé sur le prompt et suivre les schémas d'interaction.
...
S'exécute lorsqu'une nouvelle tâche commence. Utilisez-le pour détecter le type de projet, initialiser le suivi et injecter le contexte initial qui oriente l'approche de Caret pour le travail.
...
Ces hooks surveillent les opérations internes de Caret et les événements au niveau du système. Utilisez-les pour suivre l'utilisation du contexte, journaliser le comportement du système et analyser les schémas de performance.
...
Votre script de hook peut afficher des informations de journalisation ou de diagnostic sur stdout pendant l'exécution, à condition que la réponse JSON soit la dernière chose écrite. Caret n'analysera que le dernier objet JSON de stdout.
...
- Recherchez les erreurs dans le panneau Output de VS Code (canal Caret)
...
Les Hooks complètent d'autres fonctionnalités de Caret :

- [Caret Rules](/french/features/caret-rules) définissent des directives de haut niveau que les hooks peuvent appliquer
