...
description: "Étendez les capacités de Caret avec des modules de connaissances réutilisables."
---
...
Contrairement aux règles (qui sont toujours actives), les compétences sont chargées **à la demande**. Vous pouvez installer des dizaines de compétences sans affecter le contexte ou les performances, car Caret ne voit que le nom et la description de la compétence tant qu'elle n'est pas réellement nécessaire.
...
- **Au démarrage** : Caret ne voit qu'une brève description de chaque compétence
...
Instructions détaillées que Caret doit suivre lorsque cette compétence est activée.
...
- `description` : Indique à Caret quand utiliser cette compétence (1024 caractères maximum)

La description est essentielle, car c'est ainsi que Caret décide s'il faut activer une compétence. Soyez précis sur ce que fait la compétence et quand elle doit être utilisée.
...
<Note>
**Standardisation Caret** : Caret utilise une architecture à double répertoire.
...
## Comment Caret utilise les compétences

Lorsque vous envoyez un message, Caret voit une liste des compétences disponibles avec leurs descriptions. Si votre demande correspond à la description d'une compétence, Caret l'active à l'aide de l'outil `use_skill`, qui charge les instructions complètes.
...
Demander "deploy this to AWS" inciterait Caret à activer la compétence, à charger ses instructions détaillées et à les suivre pour mener à bien votre demande.
...
Les compétences peuvent inclure des fichiers supplémentaires auxquels Caret n'accède qu'en cas de besoin :
...
Caret lit ces fichiers à l'aide de `read_file` lorsque les instructions les référencent. Les scripts peuvent être exécutés directement, avec uniquement la sortie entrant dans le contexte (pas le code de script lui-même).
...
|---------|---------|-------------|
| **Règles** | Définir le comportement de Caret | Toujours (ou contextuellement) |
...
Les **compétences** sont une expertise que Caret active automatiquement lorsqu'elle est pertinente (comme les connaissances en analyse de données lorsque vous travaillez avec des fichiers CSV).
