```
---
title: "Désactiver les pagers de terminal pendant les sessions Caret"
description: "Rendre la sortie CLI non interactive lorsque Caret exécute des commandes en détectant la variable d'environnement CLINE_ACTIVE et en désactivant les pagers comme less."
---

De nombreux outils CLI (comme Git) utilisent un pager tel que `less` pour une sortie interactive et défilable. Lorsque Caret exécute des commandes dans votre terminal, cette interactivité devient gênante : le pager peut se bloquer sur la première page et interrompre la progression. Vous pouvez configurer votre shell de sorte que lorsqu'un terminal est lancé par Caret, les pagers soient désactivés et que la sortie soit diffusée normalement.

## Comment ça marche

Caret définit une variable d'environnement pour les terminaux qu'il ouvre pour exécuter des commandes :

- `CLINE_ACTIVE` : non vide lorsque le shell s'exécute sous Caret

Vous pouvez détecter cette variable dans votre fichier de démarrage de shell et ajuster les variables d'environnement ou les alias uniquement pour les sessions exécutées par Caret. Cela permet de conserver vos terminaux interactifs normaux inchangés.
...
- Ouvrez une tâche dans Caret qui exécute des commandes de terminal et vérifiez :
...
- Vous pouvez également remplacer la pagination via des alias de shell limités aux sessions Caret en utilisant des options plutôt que des variables d'environnement :
...
- Si vous préférez les variables d'environnement, de nombreuses CLI respectent également une variable de pager générique ou spécifique à l'outil :
  ...
  - Pages de manuel : `MANPAGER=cat` (généralement pas nécessaire pour les commandes pilotées par Caret)
