...
Les Workflows vous permettent de définir une série d'étapes pour guider Careti à travers un ensemble de tâches répétitives, comme le déploiement d'un service ou la soumission d'une PR.

Pour invoquer un workflow, tapez `/[workflow-name.md]` dans le chat.

## Comment créer et utiliser des Workflows

Les Workflows vivent aux côtés des [Careti Rules](/french/features/caret-rules). En créer un est très simple :

<Frame>
	<img src="https://storage.googleapis.com/cline_public_images/docs/assets/workflows.png" alt="Workflows tab in Careti" />
</Frame>

1. Créez un fichier markdown avec des instructions claires sur les étapes que Careti doit suivre
...
-   Exploiter les [built-in tools](/french/exploring-carets-tools/caret-tools-guide) de Careti comme `ask_followup_question`, `read_file`, `search_files` et `new_task`
...
3. Laisser Careti s'occuper du reste

Mon workflow utilise l'outil en ligne de commande `gh` et l'outil intégré `ask_followup_question` de Careti pour :
...
-   Si je dis "oui", Careti approuve automatiquement la PR avec la commande `gh`
