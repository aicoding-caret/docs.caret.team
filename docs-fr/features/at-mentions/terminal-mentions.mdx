...
description: "Mentionnez votre sortie de terminal directement dans vos conversations avec Caret."
---

La mention de terminal vous permet d'intégrer directement la sortie de votre terminal dans votre conversation avec Caret. Tapez simplement `@terminal` et Caret peut voir la sortie récente de votre terminal.

Lorsque vous tapez `@` dans le chat, sélectionnez "Terminal" dans le menu ou tapez simplement `@terminal` directement. Caret verra immédiatement la sortie récente de votre terminal actif, y compris les messages d'erreur, les journaux de construction ou les résultats de commandes.
...
Cela donne à Caret la sortie complète du terminal avec tout son formatage intact. Caret peut ensuite analyser les messages d'erreur, les stack traces et le contexte environnant pour fournir une aide plus précise.
...
Cette approche donne à Caret tout ce dont il a besoin - la sortie d'erreur exacte, le code client et l'implémentation du serveur - le tout sans que j'aie à copier quoi que ce soit manuellement.

La prochaine fois que vous rencontrez des problèmes avec la sortie de commandes ou des erreurs de construction, essayez d'utiliser `@terminal` au lieu de copier la sortie. Vous obtiendrez une aide plus précise, car Caret peut voir le contexte complet du terminal avec un formatage approprié.
...
1. Lorsque vous envoyez votre message, Caret détecte le modèle `@terminal` dans votre texte
...
-   Messages "Shell Integration Unavailable" dans le chat Caret
-   Commandes s'exécutant mais sortie non visible par Caret
