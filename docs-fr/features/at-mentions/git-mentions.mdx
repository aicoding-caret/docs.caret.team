...
description: "Les mentions Git vous permettent d'intégrer l'historique et les modifications de votre dépôt directement dans votre conversation avec Caret."
---

Les mentions Git vous permettent d'intégrer l'historique et les modifications de votre repository directement dans votre conversation avec Caret. Vous pouvez référencer les modifications non validées avec `@git-changes` ou des commits spécifiques avec `@[commit-hash]`.

Lorsque vous tapez `@` dans le chat, vous pouvez sélectionner "Git Changes" dans le menu ou taper `@git-changes` directement. Pour les commits spécifiques, tapez `@` suivi du hash du commit (au moins 7 caractères). Caret verra immédiatement l'état git, les diffs, les messages de commit et d'autres informations pertinentes.
...
Cela donne à Caret les informations complètes du commit, y compris le message de commit, l'auteur, la date et le diff complet. Caret peut alors analyser exactement ce qui a changé et comment cela pourrait affecter d'autres parties du codebase.
...
Cela montre à Caret toutes vos modifications non validées, y compris les nouveaux fichiers, les fichiers modifiés et leurs diffs. Caret peut alors examiner vos modifications et vous fournir des commentaires sur votre implémentation.
...
La prochaine fois que vous travaillerez avec des modifications de code ou que vous enquêterez sur des problèmes, essayez d'utiliser les mentions git au lieu de décrire ou de copier manuellement les modifications. Vous obtiendrez une aide plus précise car Caret peut voir exactement ce qui a changé et dans quel contexte.
...
1. Lorsque vous envoyez votre message, Caret détecte le motif `@git-changes` dans votre texte
...
1. Lorsque vous envoyez votre message, Caret détecte le `@` suivi d'un motif de hachage de commit
