...
description: "Découvrez ce que sont les tâches dans Caret, comment elles fonctionnent et comment créer des invites efficaces pour de meilleurs résultats."
---

<Note>
Ceci est la documentation de référence Caret. Elle suit la version fusionnée de Caret v3.38.1 et, le cas échéant, les politiques/restrictions spécifiques à Caret (par exemple, l'emplacement de sauvegarde des tâches, la politique de points de contrôle, l'authentification/le routage) sont indiquées par `<Note>` dans le corps du texte.
</Note>

<Note>
Historique des entrées/compte : la fonctionnalité d'historique/de restauration des entrées de Caret peut fonctionner conformément à `caret-docs/features/f10-input-history-system.md`, et les informations de compte/organisation peuvent être gérées conjointement.
</Note>

## Que sont les tâches ?

La plupart des utilisateurs interagissent avec Caret par le biais de **tâches**, l'unité de travail fondamentale qui anime chaque session de codage. Que vous construisiez une nouvelle fonctionnalité, que vous corrigiez un bug, que vous refactorisiez du code ou que vous exploriez une base de code, chaque interaction avec Caret se déroule dans le contexte d'une tâche. Une tâche représente une conversation complète et une session de travail entre vous et l'agent d'IA, créée par le biais d'**invites** (prompts) : les instructions que vous fournissez pour indiquer à Caret ce que vous voulez accomplir. Les tâches servent de sessions de travail autonomes qui capturent l'intégralité de votre conversation avec Caret, y compris toutes les modifications de code, les exécutions de commandes et les décisions prises en cours de route.

Cette approche garantit que votre travail est organisé, traçable et résumable. Chaque tâche conserve son propre contexte isolé, ce qui vous permet de travailler sur plusieurs projets simultanément sans confusion. La beauté du système de tâches de Caret réside dans sa flexibilité et sa persistance, offrant une session de codage collaborative où vous donnez la direction par le biais d'invites, et Caret exécute votre vision avec précision.

### Caractéristiques principales

Chaque tâche dans Caret :
...
Ces fonctionnalités font de Caret non seulement un outil de codage, mais aussi un agent de développement complet qui comprend l'ensemble du cycle de vie de votre travail.

## Création de tâches avec des invites (Prompts)

Les tâches commencent par des invites (prompts) : vos instructions à Caret. La qualité de vos résultats dépend fortement de la façon dont vous décrivez ce que vous voulez.
...
Plongez au cœur du **Module 1 : « Prompting »** dans [Caret Learn](https://clinelearn.com) pour devenir un expert dans la création d'invites efficaces. Le module couvre :
...
Caret fonctionne selon deux modes distincts qui permettent de structurer votre flux de travail :
...
- **Mode Act** : Pour la mise en œuvre proprement dite où Caret exécute des modifications de fichiers, exécute des commandes et utilise des outils
...
L'une des fonctionnalités puissantes de Caret est la possibilité de reprendre les tâches interrompues :
...
2. Caret charge l'intégralité de la conversation
...
Ce contexte permet à Caret de :
- Comprendre ce qui a été fait
- Maintenir la cohérence de l'approche
- Reprendre le travail intelligemment
- Tirer des leçons des tentatives précédentes

→ **[En savoir plus sur la gestion du contexte](/french/getting-started/understanding-context-management)** pour comprendre comment Caret gère et optimise le contexte entre les tâches.

Comprendre le fonctionnement des tâches est essentiel pour utiliser Caret efficacement. Avec des invites bien conçues et une compréhension du cycle de vie des tâches, vous pouvez exploiter tout le potentiel de Caret pour accélérer votre flux de travail de développement.
