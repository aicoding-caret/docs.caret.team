...
description: "Configurer et gérer les serveurs MCP dans Careti."
---

## Mode d'inclusion globale des serveurs MCP

L'utilisation des serveurs MCP augmentera votre consommation de tokens. Careti offre la possibilité de restreindre ou de désactiver la fonctionnalité des serveurs MCP selon vos besoins.

1. Cliquez sur l'icône "MCP Servers" dans la barre de navigation supérieure de l'extension Careti.
2. Sélectionnez l'onglet "Configure", puis cliquez sur le lien "Advanced MCP Settings" au bas de ce panneau.
3. Careti ouvrira une nouvelle fenêtre de paramètres. Recherchez `Careti>Mcp:Mode` et faites votre sélection dans le menu déroulant.
...
1. Cliquez sur l'icône MCP Servers dans la barre de navigation supérieure du panneau Careti.
...
_Exemple de configuration de serveur MCP dans Careti (transport STDIO)_
...
Après avoir configuré un serveur MCP, Careti détectera automatiquement les outils et ressources disponibles. Pour les utiliser :

1. Tapez votre requête dans la fenêtre de conversation de Careti
2. Careti identifiera quand un outil MCP peut vous aider dans votre tâche
