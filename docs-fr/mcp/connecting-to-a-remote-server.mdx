...
description: "Le Model Context Protocol (MCP) permet à Caret de communiquer avec des serveurs externes qui fournissent des outils et des ressources supplémentaires pour étendre ses capacités. Ce guide explique comment ajouter et se connecter à des serveurs MCP distants via l'interface MCP Servers."
---
...
Pour accéder à l'interface MCP Servers dans Caret :

1. Cliquez sur l'icône Caret dans la barre latérale de VS Code
2. Ouvrez le menu (⋮) dans le coin supérieur droit du panneau Caret
...
4. Caret tentera de se connecter au serveur et affichera l'état de la connexion
...
Caret prend en charge deux protocoles de transport pour les serveurs MCP distants :
...
6. Vous serez automatiquement redirigé vers Caret
7. Le serveur se connectera et affichera un point d'état vert

Une fois authentifiées, vos informations d'identification sont stockées en toute sécurité et le serveur se reconnectera automatiquement lorsque vous rechargerez Caret. Vous n'aurez pas besoin de vous authentifier à nouveau, sauf si vous supprimez le serveur ou si vos informations d'identification expirent.
...
> **Warning**: Les places de marché tierces suivantes sont répertoriées à titre d'information uniquement. Caret n'approuve, ne vérifie ni n'assume aucune responsabilité pour les serveurs répertoriés sur ces places de marché. Ces serveurs sont des services hébergés dans le cloud qui traitent vos requêtes et peuvent avoir accès aux données que vous partagez avec eux. Consultez toujours les politiques de confidentialité et les conditions d'utilisation avant de vous connecter à des services tiers.
...
2. **Request Timeout** :
    - Définissez le temps pendant lequel Caret doit attendre les réponses du serveur
...
-   **Enabled** : Caret peut utiliser les outils et les ressources du serveur
...
-   **Erreurs de redirection** : Vérifiez que vous utilisez la dernière version de Caret - les anciennes versions peuvent ne pas prendre en charge OAuth
-   **Réinitialiser l'authentification** : Supprimez le serveur et rajoutez-le pour repartir à zéro avec un nouveau flux OAuth

### Configuration avancée

Pour les utilisateurs avancés, Caret stocke les configurations du serveur MCP dans un fichier JSON qui peut être modifié :
...
Une fois connecté, Caret peut utiliser les outils et les ressources fournis par le serveur MCP. Lorsque Caret suggère d'utiliser un outil MCP :
...
3. L'outil s'exécutera et renverra les résultats à Caret
...
5. Connectez-vous à Vercel dans votre navigateur et autorisez Caret
6. Retournez à Caret - le serveur se connectera automatiquement
7. Les outils de Vercel (deploy, logs, projects) sont maintenant disponibles pour Caret !

Votre authentification Vercel persiste d'une session à l'autre, vous n'aurez donc pas besoin de vous réauthentifier à chaque fois que vous utilisez Caret.
