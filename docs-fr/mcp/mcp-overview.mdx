...
---
title: "Aperçu de MCP"
description: "Découvrez les serveurs Model Context Protocol (MCP), leurs capacités et comment Caret peut vous aider à les construire et à les utiliser. MCP standardise la manière dont les applications fournissent du contexte aux LLM, agissant comme un port USB-C pour les applications d'IA."
---
...
Caret n'est livré avec aucun serveur MCP préinstallé. Vous devrez les trouver et les installer séparément.
...
-   **Caret Marketplace :** Installez-en un depuis le [MCP Marketplace](/french/mcp/mcp-marketplace) de Caret
-   **Demander à Caret :** Vous pouvez demander à Caret de vous aider à trouver ou à créer des serveurs MCP
...
## Intégration avec Caret

Caret simplifie la construction et l'utilisation des serveurs MCP grâce à ses capacités d'IA.

### Construction de serveurs MCP

-   **Compréhension du langage naturel :** Demandez à Caret en langage naturel de construire un serveur MCP en décrivant ses fonctionnalités. Caret interprétera vos instructions et générera le code nécessaire.
-   **Clonage et construction de serveurs :** Caret peut cloner des référentiels de serveurs MCP existants depuis GitHub et les construire automatiquement.
-   **Configuration et gestion des dépendances :** Caret gère les fichiers de configuration, les variables d'environnement et les dépendances.
-   **Dépannage et débogage :** Caret aide à identifier et à résoudre les erreurs pendant le développement.

### Utilisation des serveurs MCP

-   **Exécution des outils :** Caret s'intègre de manière transparente aux serveurs MCP, vous permettant d'exécuter leurs outils définis.
-   **Interactions tenant compte du contexte :** Caret peut suggérer intelligemment d'utiliser les outils pertinents en fonction du contexte de la conversation.
-   **Intégrations dynamiques :** Combinez plusieurs capacités de serveurs MCP pour des tâches complexes. Par exemple, Caret pourrait utiliser un serveur GitHub pour obtenir des données et un serveur Notion pour créer un rapport formaté.
