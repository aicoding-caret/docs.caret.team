...
description: "Découvrez les deux principaux mécanismes de transport pour la communication entre les serveurs Caret et MCP : Standard Input/Output (STDIO) et Server-Sent Events (SSE). Chacun a des caractéristiques, des avantages et des cas d’utilisation distincts."
---

Model Context Protocol (MCP) prend en charge deux principaux mécanismes de transport pour la communication entre les serveurs Caret et MCP : Standard Input/Output (STDIO) et Server-Sent Events (SSE). Chacun a des caractéristiques, des avantages et des cas d’utilisation distincts.
...
1. Le client (Caret) génère un serveur MCP en tant que processus enfant
...
-   **Localité** : s’exécute sur la même machine que Caret
...
-   Les scénarios à client unique (une instance Caret par serveur)
...
1. Le client (Caret) se connecte au point de terminaison SSE du serveur via une requête HTTP GET
...
-   **Accès à distance** : peut être hébergé sur une machine différente de votre instance Caret
...
Les serveurs STDIO s’exécutent localement sur la même machine que Caret, ce qui a plusieurs implications importantes :
...
-   **Exécution** : démarre et s’arrête avec Caret (cycle de vie du processus enfant)
...
-   Démarrerait lorsque Caret en a besoin
-   Ne nécessiterait pas de configuration réseau
-   Devrait être installé avec Caret ou via un gestionnaire de packages
...
## Configuration des transports dans Caret

Pour obtenir des informations détaillées sur la configuration des transports STDIO et SSE dans Caret, y compris des exemples, consultez [Configuration des serveurs MCP](/french/mcp/configuring-mcp-servers).
```