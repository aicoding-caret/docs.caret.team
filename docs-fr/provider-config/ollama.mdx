...
4.  **Configurer Careti :**
    -   Ouvrez la barre latérale de Careti (généralement indiquée par l'icône Careti).
    ...
    -   (Optionnel) Configurez la Model context size dans les paramètres Advanced de Careti. Cela aide Careti à gérer efficacement sa fenêtre de contexte avec votre modèle Ollama personnalisé.
...
-   **Capacité hors ligne :** Après avoir téléchargé un modèle, vous pouvez utiliser Careti avec ce modèle même sans connexion Internet.
-   **Suivi de l'utilisation des tokens :** Careti suit l'utilisation des tokens pour les modèles accédés via Ollama, vous permettant de surveiller votre consommation.
