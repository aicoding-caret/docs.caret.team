...
description: "Utilisez Vercel AI Gateway dans Caret pour accéder à plus de 100 modèles depuis un seul endpoint avec du routage, des retries et une observabilité des dépenses."
---

Vercel AI Gateway vous offre une API unique pour accéder aux modèles de nombreux fournisseurs. Vous changez de modèle par son model id sans changer de SDK ni jongler avec plusieurs clés. Caret s'intègre directement afin que vous puissiez choisir un modèle Gateway dans le menu déroulant, l'utiliser comme n'importe quel autre fournisseur, et visualiser l'utilisation des tokens et du cache dans le stream.
...
4. Choisissez un modèle dans la liste. Caret récupère automatiquement le catalogue. Vous pouvez également coller un id exact
...
La gateway supporte un ensemble large et évolutif de modèles. Caret récupère la liste via l'API Gateway et la met en cache localement. Pour consulter le catalogue actuel, voir https://vercel.com/ai-gateway/models
...
- Évaluations multi-modèles - changez uniquement le model id dans Caret et comparez la latence et les tokens de sortie
