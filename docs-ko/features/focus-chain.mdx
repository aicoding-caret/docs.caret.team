...
<BrandName particle="of" /> 포커스 체인은 장기 작업을 위한 자동 TODO 관리 기능입니다. LLM에 전달되는 컨텍스트를 지속적으로 관리하며, 여러 번의 컨텍스트 리셋에서도 진행 상황을 유지합니다.
...
1. <BrandName /> 설정(⚙️) → **Features**에서 **Enable Focus Chain** 활성화  
2. **Remind <BrandName /> Interval** 값 조정(기본 6)  
3. 새 작업 시작 → TODO 자동 생성/관리 시작

| 설정 | 기본값 | 범위 | 설명 |
|---------|---------|-------|-------------|
| Enable Focus Chain | Disabled | On/Off | 향상된 진행 추적 활성화 |
| Remind <BrandName /> Interval | 6 | 1-100 messages | TODO 갱신 알림 주기 |
...
4. **Save**: <BrandName /> automatically detects and uses your updates
...
Focus Chain works seamlessly with <BrandName />'s [Plan/Act mode](/ko/features/plan-and-act):
...
Still stuck? Use the [/reportbug](/ko/features/slash-commands/report-bug) command in <BrandName /> to get help.
