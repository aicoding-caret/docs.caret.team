...
title: "워크플로 빠른 시작"
sidebarTitle: "빠른 시작"
description: "첫 캐러티 워크플로를 만드는 단계별 가이드"
---
...
<Steps>
  ...
  <Step title="워크플로 실행">
    ...
    1. <BrandName /> 채팅 패널 열기
    2. `/pr-review.md` 뒤에 PR 번호를 입력 (예: `/pr-review.md 42`)
    3. <BrandName particle="subject" /> PR 정보를 가져오고 분석한 뒤 결과를 보여줌

    <Tip>
      `gh pr view` 같은 명령 실행 후 <BrandName particle="subject" /> 멈추면 **Proceed While Running** 버튼을 눌러 계속 진행하세요.
    </Tip>
  </Step>
</Steps>
