...
워크플로는 [Caret 규칙](/ko/features/caret-rules)과 함께 존재합니다. 생성하는 것은 간단합니다.
...
-   `ask_followup_question`, `read_file`, `search_files`, `new_task`와 같은 Cline의 [내장 도구](/ko/exploring-carets-tools/caret-tools-guide) 활용
-   `gh` 또는 `docker`와 같이 이미 설치된 명령줄 도구 사용
-   Slack 또는 Whatsapp과 같은 외부 [MCP 도구 호출](/ko/mcp/mcp-overview) 참조
-   특정 순서로 여러 작업을 함께 연결
...
> 이것은 워크플로 파일의 한 예시일 뿐입니다. 영감을 얻기 위해 [프롬프트 저장소](https://github.com/caret/prompts)에서 더 많은 것을 찾을 수 있습니다.
