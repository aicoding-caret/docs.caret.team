...
title: "훅"
sidebarTitle: "훅"
description: "Caret 워크플로우에 커스텀 로직을 주입해 작업 검증, 도구 사용 모니터링, AI 결정 흐름을 제어합니다."
---
...
훅(Hooks)은 Caret 워크플로우의 특정 시점에 커스텀 로직을 주입해 작업을 검증하거나 도구 사용을 모니터링하고, AI의 의사결정에 영향을 줄 수 있게 해줍니다.
...
Caret에서 훅을 활성화하는 방법은 간단합니다. 다음 단계를 따라주세요:

<Steps>
<Step title="설정에서 훅 활성화">
Caret 설정에서 **"Enable Hooks"** 체크박스를 활성화합니다.
...
**개인/조직 공통 훅:**
- `~/Documents/Caret/Rules/Hooks/`에 훅 생성
- 모든 워크스페이스에 자동 적용

**프로젝트별 훅:**
- 프로젝트 루트의 `.agents/hooks/`에 훅 생성
- 해당 워크스페이스에만 적용
- 버전 관리에 포함해 팀과 공유 가능
</Step>
...
<Step title="훅 테스트">
Caret에서 작업을 시작하고 훅이 실행되는지 확인합니다.
</Step>
</Steps>
...
훅은 Caret 워크플로우와 외부 생태계를 연결하는 접착제 역할을 합니다.


## 훅 타입

Caret는 AI 워크플로우의 서로 다른 단계에서 사용할 수 있는 여러 훅 타입을 제공합니다. 훅 타입은 트리거 지점과 용도에 따라 분류됩니다.
...
훅 실행 중 stdout으로 로그/진단 정보를 출력할 수 있지만, **마지막 출력은 반드시 JSON**이어야 합니다. Caret는 stdout의 마지막 JSON 객체만 파싱합니다.
...
- 훅 파일에 문법 오류가 없는지 확인
- VS Code 출력 패널(Caret 채널)의 에러 확인
...
### 컨텍스트가 동작에 반영되지 않음
컨텍스트 수정은 현재가 아닌 **다음 AI 결정**에 반영됩니다. AI의 현재 동작은 이전 "API Request..." 블록을 기준으로 결정되며, `contextModification`은 다음 "API Request..." 블록에 주입됩니다. 즉, 즉시 효과가 필요하다면 PreToolUse 훅에서 검증 후 `cancel: true`를 반환해 Caret의 진행을 차단해야 합니다.
...
훅은 다른 Caret 기능과 함께 사용할 때 더 강력합니다:

- [Caret 규칙](/ko/features/caret-rules)으로 상위 가이드를 정의하고 훅으로 강제할 수 있습니다
