...
description: "Caret가 명령을 실행할 때 less 같은 페이저를 비활성화해 비대화형 출력으로 만들기"
---

많은 CLI 도구(Git 등)는 `less` 같은 페이저를 사용해 스크롤 가능한 출력 방식을 제공합니다. Caret가 터미널 명령을 실행할 때 이 인터랙션이 방해가 될 수 있으며, 페이저가 첫 페이지에서 멈춰 작업이 진행되지 않기도 합니다. 이때 Caret가 실행한 터미널에서는 페이저가 비활성화되도록 셸 설정을 추가할 수 있습니다.

## 동작 방식

Caret는 명령을 실행하는 터미널에 환경 변수를 설정합니다:

- `CLINE_ACTIVE` — Caret가 실행한 셸이면 값이 비어 있지 않습니다.

셸 시작 파일에서 이 변수를 감지해 Caret 세션에서만 환경 변수나 alias를 바꿀 수 있습니다. 그러면 평소 사용하던 터미널은 그대로 유지됩니다.
...
- Caret에서 터미널 명령을 실행한 뒤 다음을 확인하세요:
