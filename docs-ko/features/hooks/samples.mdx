...
title: "샘플"
sidebarTitle: "샘플"
description: "초급부터 고급까지 난이도별 실전 Hook 예시"
---

이 문서는 난이도별로 정리한 **완전한 Hook 예시**를 제공합니다. 각 예시는 동작 가능한 코드, 상세 설명, 활용 시점을 함께 담고 있습니다.

## 샘플 활용 방법

각 샘플은 다음을 목표로 합니다:
- **바로 복사해서 사용 가능**: 그대로 쓰거나 출발점으로 활용
- **학습용**: 난이도가 점진적으로 올라가며 Hook 개념을 이해
- **실전용**: 실제 개발 워크플로의 문제를 해결

경험 수준에 맞는 샘플부터 시작해 점차 고급 패턴으로 확장해 보세요.

---

## 초급 예시

Hooks를 처음 시작할 때 적합한 예시입니다. 단순한 로직으로 핵심 개념을 보여줍니다.
...
# coding standards and best practices into the AI context. This helps Caret understand
...
# Return the context to guide Caret's behavior
if [[ -n "$context" ]]; then
...
**핵심 개념:**
- `input=$(cat)`로 Hook 입력 읽기
- 파일 시스템 체크로 프로젝트 유형 감지
- 컨텍스트 주입으로 AI 동작 유도
- `jq`로 JSON 출력 구성
...
# users about system resource issues that may be slowing down Caret's operations.
#
...
if (( execution_time > 3000 )); then
  ...
  mkdir -p "$HOME/.caret_logs"

  # Log the slow operation
  echo "$(date -Iseconds): SLOW OPERATION - $tool_name took ${execution_time}ms" >> "$HOME/.caret_logs/performance.log"
...
외부 도구 연동, 패턴 매칭, 구조화된 로깅 등 고급 개념을 포함합니다.
...
외부 시스템 연동, 비동기 처리, 복잡한 상태 관리를 다루는 고급 패턴입니다.
...
reports_dir="$HOME/.caret_reports"
...
activity_log="$HOME/.caret_user_activity/$(date +%Y-%m-%d).log"
...
# CI/CD systems. Enables seamless integration of Caret operations into enterprise workflows.
#
...
if [[ -n "$triggered_workflows" ]]; then
  ...
  if [[ -n "$slack_webhook" ]]; then
    slack_payload=$(jq -n \
      ...
      '{
        text: ("🔧 Caret modified `" + $file + "` in " + $workspace),
        color: "good",
        fields: [{
...
각 Hook 타입을 별도 파일로 두면 동시에 사용할 수 있습니다:
...
# 여러 Hook 생성
touch .agents/hooks/PreToolUse
touch .agents/hooks/PostToolUse
touch .agents/hooks/TaskStart

# 실행 권한 부여
chmod +x .agents/hooks/*
```

### 환경 변수 설정

외부 통합을 위해 환경 변수를 설정합니다:
...
export CI_WEBHOOK_URL="https://your-ci-system.com/hooks/caret"
```

### Hook 테스트

입력을 직접 흘려보내면서 Hook을 테스트할 수 있습니다:
...
이 샘플들은 Hooks를 실전 워크플로에 적용하기 위한 탄탄한 출발점입니다. 팀의 환경과 요구에 맞게 변형해 사용하세요.
