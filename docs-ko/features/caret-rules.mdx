...
Careti 규칙을 사용하면 Caret에 시스템 수준 지침을 제공할 수 있습니다. 프로젝트 또는 모든 대화에 대해 전역적으로 컨텍스트와 기본 설정을 포함하는 **지속적인 규칙**이라고 생각하면 됩니다.

<Note>
캐러티 Agent Standardization: Caret은 `.agents/context` 폴더를 기본 규칙 소스(SoT)로 사용하며, `AGENTS.md` 파일도 지원합니다. `/init` 명령으로 표준 구조를 쉽게 생성할 수 있습니다.
</Note>
...
| 운영 체제 | 기본 위치 | 참고 |
|---|---|---|
| **Windows** | `Documents\Careti\Rules` | 시스템 문서 폴더 사용 |
| **macOS** | `~/Documents/Careti/Rules` | 사용자 문서 폴더 사용 |
| **Linux/WSL** | `~/Documents/Careti/Rules` | 일부 시스템에서는 `~/Careti/Rules`로 대체될 수 있습니다. |

> **Linux/WSL 사용자 참고**: `~/Documents/Careti/Rules`에서 전역 규칙을 찾을 수 없으면 `~/Careti/Rules`도 확인하세요. 문서 디렉토리 유무에 따라 위치가 달라질 수 있습니다.
...
Caret는 Careti 규칙과 별도로 [AGENTS.md](https://agents.md/) 표준도 지원합니다. 워크스페이스 루트에 `AGENTS.md`가 있을 때 자동으로 감지합니다. 이를 통해 다른 AI 코딩 도구에서도 동일한 규칙 파일을 재사용할 수 있습니다.
...
Careti v3.13은 채팅 인터페이스에서 바로 접근 가능한 **규칙 관리 팝오버 UI**를 제공합니다. 이 UI를 통해:
