...
캐러티 규칙을 사용하면 캐러티에 시스템 수준 지침을 제공할 수 있습니다. 프로젝트 또는 모든 대화에 대해 전역적으로 컨텍스트와 기본 설정을 포함하는 **지속적인 규칙**이라고 생각하면 됩니다.

<Note>
캐러티 Agent Standardization: 캐러티는 `.agents/context` 폴더를 기본 규칙 소스(SoT)로 사용하며, `AGENTS.md` 파일도 지원합니다. `/init` 명령으로 표준 구조를 쉽게 생성할 수 있습니다.
</Note>
...
| 운영 체제 | 기본 위치 | 참고 |
|---|---|---|
| **Windows** | `Documents\캐러티\Rules` | 시스템 문서 폴더 사용 |
...
채팅에서 [`/newrule` 슬래시 명령](/ko/features/slash-commands/new-rule)을 사용해 캐러티가 규칙 파일을 생성하도록 할 수도 있습니다.
...
캐러티의 시스템 프롬프트는 사용자가 편집할 수 없지만, 규칙 파일을 통해 AI의 동작을 세밀하게 제어할 수 있습니다. 프롬프트 엔지니어링 모범 사례는 [Anthropic 가이드](https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/overview)를 참고하세요.

### AGENTS.md 표준 지원

캐러티는 캐러티 규칙과 별도로 [AGENTS.md](https://agents.md/) 표준도 지원합니다. 워크스페이스 루트에 `AGENTS.md`가 있을 때 자동으로 감지합니다. 이를 통해 다른 AI 코딩 도구에서도 동일한 규칙 파일을 재사용할 수 있습니다.
...
캐러티는 `.agents/context/` 디렉토리 내의 **모든 Markdown 파일**을 자동으로 결합해 하나의 규칙 세트로 사용합니다. 숫자 접두사(선택)는 논리적 순서를 잡는 데 도움이 됩니다.
...
폴더 시스템은 캐러티 규칙을 정적 문서에서 팀 컨텍스트에 맞게 진화하는 지식 시스템으로 바꿔줍니다.

### 토글 가능한 팝오버로 규칙 관리

캐러티 v3.13은 채팅 인터페이스에서 바로 접근 가능한 **규칙 관리 팝오버 UI**를 제공합니다. 이 UI를 통해:
