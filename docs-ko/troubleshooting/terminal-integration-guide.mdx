...
:::tip
	터미널 문제가 발생하는 경우 Caret 설정의 "터미널 설정"에서 `bash`와 같은 더 간단한 셸로 전환해 보세요.
...
-   Caret 채팅에 "[명령이 실행 중이지만 출력이 없습니다]" 메시지 표시
...
-   AI에게 대신 파일 출력을 사용하도록 권장
    1. Caret 채팅 또는 Caret 규칙에서 파일을 읽기 전에 `command > output.txt`를 사용하도록 지시

:::tip
	이러한 종류의 문제는 최신 Caret 버전에서 부분적으로만 해결되었으므로, 여전히 문제가 발생하는 경우 GitHub 이슈를 생성하세요.
	지속적인 문제인 경우.
:::
...
-   Caret 버튼 "실행 중 계속"이 채팅에서 제대로 작동하지 않음

:::tip
	이러한 종류의 문제는 최신 Caret 버전에서 해결되었지만, 여전히 문제가 발생하는 경우 GitHub 이슈를 생성하세요.
	이 문제에 대해.
:::
...
작동하는 통합은 Caret 채팅에 명령 출력을 표시합니다. 실패한 통합은 "셸 통합 사용 불가" 또는 "[명령이 실행 중이지만 출력이 없습니다]"를 표시합니다.
...
2. **문제 보고**:
    - Caret GitHub 이슈에서 `/reportbug` 사용
    - 디버그 정보 포함
    - 시도한 해결책 언급
