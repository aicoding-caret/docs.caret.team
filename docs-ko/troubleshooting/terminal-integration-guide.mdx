...
description: "Caret에서 터미널 통합 문제를 해결하기 위한 완벽한 가이드"
---

이 가이드는 Caret에서 터미널 통합 문제를 해결하는 데 도움이 됩니다. 터미널 통합은 Caret이 명령을 실행하고 출력을 읽어 오류, 테스트 결과 및 명령 응답을 이해할 수 있도록 하는 데 중요합니다.

:::tip
	터미널 문제가 발생하는 경우 Careti 설정의 "터미널 설정"에서 `bash`와 같은 더 간단한 셸로 전환해 보세요.
...
```mermaid
graph TD
    A[터미널 문제] --> B{Caret이 명령을 실행할 수 있습니까?}
    B -->|아니요| C[셸 통합 사용 불가]
    B -->|예| D{Caret이 출력을 볼 수 있습니까?}
    D -->|아니요| E[출력 캡처 실패]
    D -->|예| F{출력이 손상되었습니까?}
    F -->|예| G[문자 필터링 문제]
    F -->|아니요| H{명령이 중단됩니까?}
...
-   메시지: "셸 통합 사용 불가"
-   명령은 실행되지만 Caret이 출력을 읽을 수 없음
-   터미널은 수동으로 잘 작동하지만 Caret에서는 작동하지 않음
...
-   Careti 채팅에 "[명령이 실행 중이지만 출력이 없습니다]" 메시지 표시
-   명령은 완료되지만 Caret이 결과를 볼 수 없음
...
-   **셸 통합 시간 초과 증가**
    ...
    1. Caret에서 채팅 창 오른쪽 상단 모서리에 있는 **설정** 버튼을 왼쪽 클릭
...
-   AI에게 대신 파일 출력을 사용하도록 권장
    1. Careti 채팅 또는 Careti 규칙에서 파일을 읽기 전에 `command > output.txt`를 사용하도록 지시

:::tip
	이러한 종류의 문제는 최신 Careti 버전에서 부분적으로만 해결되었으므로, 여전히 문제가 발생하는 경우 GitHub 이슈를 생성하세요.
	지속적인 문제인 경우.
:::
...
-   Docker 빌드가 Caret에서 완료되지 않음
-   진행률 표시줄이 수천 개의 토큰을 소비
-   Careti 버튼 "실행 중 계속"이 채팅에서 제대로 작동하지 않음

:::tip
	이러한 종류의 문제는 최신 Careti 버전에서 해결되었지만, 여전히 문제가 발생하는 경우 GitHub 이슈를 생성하세요.
	이 문제에 대해.
:::

## 터미널 설정 설명

Caret에서 설정 아이콘을 클릭하고 "터미널 설정" 섹션으로 이동하여 액세스하세요.

### 기본 터미널 프로필

-   **기능**: Caret이 명령에 사용하는 셸 선택
-   **변경 시기**: 기본 셸에서 셸 통합 문제가 발생하는 경우
-   **권장**: - macOS: bash (zsh에 문제가 있는 경우) - Windows: PowerShell 7 - Linux: bash

### 셸 통합 시간 초과

-   **기능**: Caret이 터미널이 준비될 때까지 기다리는 시간
-   **기본값**: 4초
-   **증가 시기**:
...
3. Caret에서 터미널 프로필로 "WSL Bash" 선택

#### 경로 문제

Windows 경로 문제:

1. Caret에서 슬래시 사용: `C:/Users/...`
...
### Caret이 왜 그렇게 많은 터미널을 생성합니까?

셸 통합이 실패하면 Caret은 터미널을 안전하게 재사용할 수 없습니다(오래 실행되는 프로세스가 있을 수 있음). 셸 통합을 활성화하거나 터미널 재사용 설정을 조정하세요.

### 사용자 지정 셸(nushell, xonsh 등)을 사용할 수 있습니까?

Caret은 bash, zsh, fish 및 PowerShell을 공식적으로 지원합니다. 사용자 지정 셸은 작동할 수 있지만 보장되지는 않습니다. bash를 대체 셸로 사용하세요.
...
작동하는 통합은 Careti 채팅에 명령 출력을 표시합니다. 실패한 통합은 "셸 통합 사용 불가" 또는 "[명령이 실행 중이지만 출력이 없습니다]"를 표시합니다.
...
2. **문제 보고**:
    - Careti GitHub 이슈에서 `/reportbug` 사용
    - 디버그 정보 포함
    - 시도한 해결책 언급
