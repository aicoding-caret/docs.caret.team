---
title: "MCP를 쉽게"
description: "Caret의 기능을 추가 도구 및 리소스로 향상시키는 MCP 서버를 검색, 설치 및 구성하기 위해 MCP 마켓플레이스를 사용하는 방법을 알아보십시오."
---
...
-   서버 코드는 `/Users/<사용자 이름>/Documents/Careti/MCP/`에 복제/설치됩니다.
-   종속성이 설치됩니다.
...
-   서버를 제거하고 다시 설치해 보십시오(`cline_mcp_settings.json` 파일과 `/Users/<사용자 이름>/Documents/Careti/MCP/` 디렉토리 모두에서 제거).
...
키워드 기반 트리거를 통해 지능적인 MCP 서버 활성화를 지원하도록 `.agents/context` 파일 또는 사용자 지정 지침을 활용하여 Cline의 도구 선택을 더 직관적이고 컨텍스트 인식적으로 만드십시오.
...
Cline의 MCP 서버 선택을 더 직관적이고 컨텍스트 인식적으로 만들려면 이를 `.agents/context` 파일 또는 사용자 지정 지침에 추가하십시오.
