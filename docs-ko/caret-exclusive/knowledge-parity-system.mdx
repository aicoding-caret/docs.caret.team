...
description: AI가 팀의 규칙과 표준을 정확히 따르도록 보장하는 Caret의 핵심 시스템을 소개합니다.
---

# ^ AI 조직 지식 및 규칙 표준화

Caret은 AI가 **팀의 코딩 규칙과 표준을 정확히 이해하고 따르도록** 설계되었습니다. 이 시스템을 통해 AI는 단순한 코드 생성 도구가 아닌, 프로젝트 맥락을 이해하는 진정한 팀원으로 기능합니다.
...
Caret은 **AI와 사람이 읽는 문서의 목적이 다르다**는 점을 인정합니다. 이를 위해 두 개의 디렉토리를 사용합니다:
...
Caret은 **지식 원자화(Atomic Knowledge)** 방식을 사용합니다. 거대한 단일 문서 대신, 지식을 최소 단위(Atom)로 쪼개고 필요할 때 조합합니다.
...
Caret는 **`.agents/context/`** 폴더를 단일 규칙 소스로 사용합니다. 이를 통해 AI가 프로젝트 전체에서 일관된 표준을 따릅니다.
...
| **전역 규칙** | `Documents/Careti/Rules` | 모든 프로젝트에 적용 |
...
자세한 규칙 작성 방법은 [Careti 규칙 기능](/ko/features/caret-rules) 문서를 참고하세요.

## 관련 문서

- [Careti 규칙 기능](/ko/features/caret-rules) - 규칙 작성 가이드
