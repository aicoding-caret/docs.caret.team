---
title: "Baseten"
description: "캐러티에서 Baseten Model APIs를 설정하고 사용하는 방법을 안내합니다."
---

<Note>
<BrandName />(<BrandName />) 기준 문서입니다. <BrandName particle="topic" /> 프로바이더 설정 검증 및 UX가 강화되어 있습니다. 계정/조직 정책에 따라 허용 모델이 달라질 수 있습니다.
</Note>
...
3. **키 생성:** 새 API 키를 생성하고 식별 가능한 이름(예: "<BrandName />")을 지정합니다.
4. **키 복사:** API 키를 즉시 복사하여 안전하게 보관합니다.

### 지원 모델

<BrandName particle="topic" /> Baseten Model APIs의 최신 모델을 지원합니다:
...
### <BrandName particle="from" /> 설정

1. **<BrandName /> 설정 열기:** <BrandName /> 패널의 설정 아이콘(⚙️) 클릭
...
- **동적 모델 업데이트:** <BrandName particle="subject" /> Baseten 최신 모델 리스트를 자동 갱신합니다.
