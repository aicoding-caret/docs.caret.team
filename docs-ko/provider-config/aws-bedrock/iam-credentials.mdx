...
-   **Caret:** AI 모델과 통합되는 VS Code 확장으로, 코드 생성/디버깅/분석을 지원합니다.
...
-   **Caret의 모델 목록:** 최소 권한(`bedrock:InvokeModel`, `bedrock:InvokeModelWithResponseStream`)만 있어도 Caret 설정에 모델 ID를 직접 입력하면 사용할 수 있습니다. Caret이 동적으로 Bedrock 모델 목록을 불러오려면 `bedrock:ListFoundationModels` 같은 추가 권한이 필요할 수 있습니다.
...
위 단계를 따르면 기업 환경에서 AWS Bedrock을 Caret VS Code 확장에 안전하게 연결할 수 있습니다.
...
3. **Caret 설정:** VS Code에서 Caret을 설치하고 AWS 자격 증명을 설정합니다.
