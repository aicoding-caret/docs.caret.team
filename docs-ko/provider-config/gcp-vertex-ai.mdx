...
-   **캐러티 설정 열기:**\
    캐러티 확장 프로그램 내의 설정 ⚙️ 아이콘을 클릭합니다.
-   **API 공급자 설정:**\
...
3. **VS Code 다시 시작:**\
   캐러티 확장 프로그램이 새 자격 증명을 인식하도록 VS Code를 다시 시작해야 합니다.
...
4. **환경 변수 설정:**
    ...
    - 이는 Google Cloud 클라이언트 라이브러리(및 캐러티)가 이 키를 사용하도록 지시합니다.
...
다음 단계를 따르면 엔터프라이즈 팀은 GCP Vertex AI를 캐러티 VS Code 확장 프로그램과 안전하게 통합하여 **Claude 3.5 Sonnet v2**의 기능을 활용할 수 있습니다.
