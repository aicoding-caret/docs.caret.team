...
description: "조직 인프라 전반에 커스텀 MCP 서버를 배포/관리합니다"
---

원격 MCP 서버는 커스텀 통합을 배포하고 내부 시스템에 연결하면서, 엔터프라이즈 보안/거버넌스/확장성을 유지할 수 있게 해줍니다. 마켓플레이스 서버와 달리 구현과 배포를 **완전히 제어**할 수 있습니다.

## 원격 서버의 장점

<CardGroup cols={2}>
  <Card title="커스텀 통합" icon="code">
    내부 API, 사내 시스템, 전용 워크플로 통합 구현
  </Card>
  
  <Card title="인프라 제어" icon="server">
    네트워크/보안을 포함한 전체 인프라 제어
  </Card>
  ...
  <Card title="확장형 아키텍처" icon="chart-line">
    자동 확장, 로드 밸런싱, 다중 리전 HA
  </Card>
</CardGroup>
...
```mermaid
graph TB
    A[Caret Enterprise] --> B[MCP Gateway]
    B --> C[Load Balancer]
...
<Tabs>
<TabItem value="terraform" title="Terraform">
```hcl
# main.tf
...
<TabItem value="ansible" title="Ansible">
```yaml
...
compliance:
  ...
  audit:
    ...
    log_responses: false  # 민감 데이터 응답은 로그 제외
    retention: "7y"
...
### 보안
1. **인증**: JWT/mTLS/OIDC 적용
2. **인가**: RBAC로 세밀한 접근 제어
3. **입력 검증**: 모든 입력 검증 및 정제
4. **출력 정제**: 데이터 유출 방지
5. **네트워크 보안**: TLS/방화벽/네트워크 정책 적용

### 성능
1. **연결 풀링**: DB/API 연결 재사용
2. **캐싱**: 지능형 캐시 적용
3. **비동기 처리**: 논블로킹 I/O
4. **리소스 제한**: CPU/메모리 제한 설정
5. **부하 테스트**: 실제 부하 시뮬레이션

### 안정성
1. **헬스 체크**: 상태 모니터링 구현
2. **서킷 브레이커**: 장애 시 빠른 실패
3. **재시도**: 지수 백오프 적용
4. **우아한 종료**: 종료 신호 처리
5. **데이터 일관성**: 데이터 무결성 유지
...
- [ ] 현실적인 부하 테스트 완료
- [ ] 재해 복구/백업 절차 문서화
- [ ] 헬스 체크 및 readiness probe 구성
- [ ] 리소스 제한/스케일링 정책 정의
- [ ] 감사 로깅/컴플라이언스 요구 충족
- [ ] 문서 업데이트 및 팀 교육 완료

원격 MCP 서버는 엔터프라이즈 수준의 보안과 신뢰성을 유지하면서 어떤 시스템과도 통합할 수 있는 유연성을 제공합니다. 인프라에 맞는 배포 옵션부터 시작해 규모에 따라 확장하세요.
