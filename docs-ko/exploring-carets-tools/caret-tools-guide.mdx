...
캐러티는 다음과 같은 작업을 수행할 수 있는 AI 비서입니다:
...
3. **변경 사항 검토**
    - 캐러티는 변경 내용을 적용하기 전에 diff를 보여줍니다
...
최신 구현 상세는 [캐러티 저장소](https://github.com/caret/caret/blob/main/src/core/prompts/system-prompt/tools)에서 전체 소스 코드를 확인하세요.

캐러티는 다양한 작업을 위해 아래 도구에 접근할 수 있습니다:
...
3. **MCP 도구**
    ...
    - 사용자가 직접 MCP 도구를 만들어 캐러티가 접근하도록 구성할 수 있음
    - 예시: 캐러티가 일기예보를 조회하는 날씨 API 도구를 사용하도록 설정
