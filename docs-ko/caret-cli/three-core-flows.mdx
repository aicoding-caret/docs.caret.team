...
**작업(Task)** - 캐러티가 수행해야 하는 단일 작업입니다("utils.js에 테스트 추가" 등). 사용자가 원하는 내용을 설명하면 캐러티가 계획을 세우고 실행합니다. 작업은 인스턴스에서 실행됩니다.

**인스턴스(Instance)** - 독립적인 캐러티 작업 공간입니다. 각 인스턴스는 한 번에 하나의 작업만 실행합니다. 여러 인스턴스를 만들어 프로젝트의 다른 영역을 병렬로 처리할 수 있습니다.

## 1. 대화형 모드: 계획 후 실행

캐러티가 어떻게 동작하는지 확인하려면 여기서 시작하세요. 대화형 모드는 실행 전에 계획을 검토할 수 있는 채팅 세션을 엽니다.

```bash
caret
```

캐러티는 현재 디렉터리에서 대화형 세션을 엽니다. 작업을 메시지로 입력하면 Plan 모드로 들어가 단계별 전략을 제안합니다.
...
캐러티는 승인된 단계(파일 읽기, 코드 작성, 명령 실행 등)를 수행합니다. 전체 과정에서 사용자가 제어권을 유지합니다.
...
`-y`(YOLO) 플래그를 사용하면 캐러티가 대화 없이 계획과 실행을 자동으로 수행합니다. CI, 크론 작업, 스크립트에 적합합니다.
...
- **대화형 모드**: 새로운 문제를 탐색하거나 캐러티 동작을 익힐 때, 또는 실행 전 계획을 검토하고 싶을 때
...
<Columns cols={2}>
  ...
  <Card title="작업 관리" icon="clipboard-check" href="/ko/getting-started/task-management">
    캐러티의 작업 저장/복원과 체크포인트 관리 방식을 소개합니다.
  </Card>
</Columns>
