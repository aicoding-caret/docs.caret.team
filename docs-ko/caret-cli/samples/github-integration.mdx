...
AI로 GitHub 이슈 분석을 자동화하세요. 이슈 댓글에 `@caret`을 멘션하면 캐러티가 파일을 읽고 코드를 분석해 실행 가능한 인사이트를 제공합니다. 모든 과정은 GitHub Actions에서 자동으로 수행됩니다.

<Note>
**캐러티 CLI가 처음이신가요?** 이 샘플은 캐러티 CLI 기본을 이해하고 [설치 가이드](/ko/caret-cli/installation)를 완료했다고 가정합니다. 처음이라면 더 간단한 [GitHub RCA 샘플](/ko/caret-cli/samples/github-issue-rca)부터 시작하는 것을 권장합니다.
</Note>

## 워크플로

이슈 댓글에서 `@caret`을 멘션하면 캐러티가 트리거됩니다:

<Frame>
  <img src="https://storage.googleapis.com/cline_public_images/ss0a-comment.png" alt="Issue comment with @caret mention" width="600" />
</Frame>

캐러티의 자동 분석 결과가 새 댓글로 게시됩니다:
...
- **캐러티 CLI 지식** - [설치 가이드](/ko/caret-cli/installation) 완료 및 기본 사용법 숙지
...
이 워크플로는 새로 생성되거나 수정된 이슈를 확인하고 `@caret` 멘션을 감지한 뒤, 캐러티 CLI 인스턴스를 띄워 이슈를 분석하고 결과를 댓글로 남깁니다.
...
이제 GitHub Action에서 캐러티가 사용할 자격 증명을 제공할 준비가 끝났습니다.
...
이 분석 스크립트는 캐러티에게 GitHub 이슈를 분석하도록 요청하고, 응답을 요약하여 댓글로 게시하는 데 필요한 출력만 추출합니다.
...
3. npm으로 캐러티 CLI를 전역 **설치**
4. `caret instance new`로 캐러티 인스턴스 **생성**
