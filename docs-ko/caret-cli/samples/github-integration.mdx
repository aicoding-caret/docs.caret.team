...
description: "GitHub Actions에서 Careti CLI를 사용해 @caret 멘션 이슈에 자동 응답하기"
---
...
<Note>
**Careti CLI가 처음이신가요?** 이 샘플은 Careti CLI 기본을 이해하고 [설치 가이드](/ko/caret-cli/installation)를 완료했다고 가정합니다. 처음이라면 더 간단한 [GitHub RCA 샘플](/ko/caret-cli/samples/github-issue-rca)부터 시작하는 것을 권장합니다.
</Note>
...
- **Careti CLI 지식** - [설치 가이드](/ko/caret-cli/installation) 완료 및 기본 사용법 숙지
...
이 워크플로는 새로 생성되거나 수정된 이슈를 확인하고 `@caret` 멘션을 감지한 뒤, Careti CLI 인스턴스를 띄워 이슈를 분석하고 결과를 댓글로 남깁니다.
...
1. `@caret` 멘션 감지
2. Careti CLI 인스턴스 생성
3. 분석 스크립트 다운로드
4. act 모드 + yolo로 이슈 분석
5. Careti 분석 결과를 댓글로 게시
...
3. npm으로 Careti CLI를 전역 **설치**
4. `caret instance new`로 Careti 인스턴스 **생성**
