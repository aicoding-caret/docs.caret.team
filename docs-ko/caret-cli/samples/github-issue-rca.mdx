...
description: "Caret CLI로 GitHub 이슈를 자동 분석해 근본 원인을 찾습니다."
---

# GitHub 근본 원인 분석

Caret CLI를 사용한 GitHub 이슈 자동 분석 샘플입니다. Caret의 자율 AI 기능으로 이슈를 가져오고 분석해 근본 원인을 찾으며, 워크플로우에 쉽게 통합할 수 있는 깔끔한 출력 결과를 제공합니다.

<Note>
**Caret CLI가 처음이신가요?** 이 샘플은 [설치 가이드](/ko/caret-cli/installation)를 완료하고 `caret auth`로 인증했다고 가정합니다. 아직 Caret CLI를 설정하지 않았다면 먼저 설치를 완료하세요.
</Note>
...
- **Caret CLI** 설치 및 인증 완료 ([설치 가이드](/ko/caret-cli/installation))
...
특정 Caret 인스턴스 주소를 지정해 분석할 수 있습니다:
...
이 샘플은 Caret CLI로 GitHub 이슈 분석 도구를 구축하는 방법을 보여줍니다:

1. Caret의 기능을 활용한 **자율 CLI 도구** 구축
2. Caret CLI **JSON 출력 파싱**
