...
<BrandName /> CLI를 사용한 GitHub 이슈 자동 분석 샘플입니다. <BrandName particle="of" /> 자율 AI 기능으로 이슈를 가져오고 분석해 근본 원인을 찾으며, 워크플로우에 쉽게 통합할 수 있는 깔끔한 출력 결과를 제공합니다.

<Note>
**<BrandName /> CLI가 처음이신가요?** 이 샘플은 [설치 가이드](/ko/caret-cli/installation)를 완료하고 `caret auth`로 인증했다고 가정합니다. 아직 <BrandName /> CLI를 설정하지 않았다면 먼저 설치를 완료하세요.
</Note>
...
- **<BrandName /> CLI** 설치 및 인증 완료 ([설치 가이드](/ko/caret-cli/installation))
- **최소 1개의 AI 모델 프로바이더** 설정 (예: OpenRouter, Anthropic, OpenAI)
- **<BrandName /> CLI 기본 사용법** 이해
...
특정 <BrandName /> 인스턴스 주소를 지정해 분석할 수 있습니다:
...
<Warning>
다음 상황에서 유용합니다:
- 여러 <BrandName /> 인스턴스를 실행 중인 경우
- 원격 <BrandName /> 서버를 사용하는 경우
- 특정 구성으로 테스트하려는 경우
</Warning>

<Note>
스크립트가 이슈를 가져오고 <BrandName particle="to" /> 분석한 뒤 결과를 표시하는 모든 작업을 자동으로 처리합니다. 이슈 복잡도에 따라 보통 30~60초가 소요됩니다.
</Note>
...
이 샘플은 <BrandName /> CLI로 GitHub 이슈 분석 도구를 구축하는 방법을 보여줍니다:

1. <BrandName particle="of" /> 기능을 활용한 **자율 CLI 도구** 구축
2. <BrandName /> CLI **JSON 출력 파싱**
