---
title: "CLI 레퍼런스"
description: "설정, 인스턴스 관리, 작업 명령까지 캐러티 CLI 전체 명령어 참조"
---

<BrandName /> CLI의 전체 명령어 레퍼런스입니다. 모든 명령, 옵션, 설정에 대한 상세 문서를 확인할 수 있습니다.
...
<BrandName /> CLI의 전체 매뉴얼 페이지 예시입니다(영문 출력 그대로 표시):

```
CARET(1)                         User Commands                        CARET(1)

NAME
       caret - orchestrate and interact with 캐러티 AI coding agents
...
DESCRIPTION
       ...
       caret is a command-line interface for orchestrating multiple 캐러티 AI
       coding agents.  캐러티 is an autonomous AI agent who can read, write,
       and execute code across your projects.  He operates through a
       client-server architecture where 캐러티 Core runs as a standalone
       ...
       the same 캐러티 Core instance, enabling seamless task handoff between
       environments.
...
AGENT BEHAVIOR
       캐러티 operates in two primary modes:

       ACT MODE
              캐러티 actively uses tools to accomplish tasks.  He can read
              files, write code, execute commands, use a headless browser, and
              more.  This is the default mode for task execution.

       PLAN MODE
              캐러티 gathers information and creates a detailed plan before
...
INSTANT TASK OPTIONS
       ...
       -o, --oneshot
              Full autonomous mode.  캐러티 completes the task and stops
...
COMMANDS
   ...
   Instance Management
       캐러티 Core instances are independent agent processes that can run in
       ...
       caret i n [-d|--default]
              Spawn a new 캐러티 Core instance.  Use --default to set it as
              the default instance for subsequent commands.

       caret instance list

       caret i l
              List all running 캐러티 Core instances with their addresses and
              status.
       ...
       caret i k address [-a|--all]
              Terminate a 캐러티 Core instance.  Use --all to kill all running
              instances.

   Task Management
       Tasks represent individual work items that 캐러티 executes.  Tasks
       ...
       caret t [-a|--address ADDR]
              ...
              which 캐러티 Core instance to use (e.g., localhost:50052).
       ...
       caret t c
              ...
              back-and-forth conversation with 캐러티.

       caret task send [message] [options]

       caret t s [message] [options]
              Send a message to 캐러티.  If no message is provided, reads from
              stdin.  Options:

              -a, --approve
                     Approve 캐러티's proposed action

              -d, --deny
                     Deny 캐러티's proposed action
...
ARCHITECTURE
       캐러티 operates on a three-layer architecture:

       Presentation Layer
              User interfaces (CLI, VSCode, JetBrains) that connect to 캐러티
              Core via gRPC
       ...
       Host Provider Layer
              ...
              shell APIs) that 캐러티 Core uses to interact with the host
              system
...
AUTHORS
       캐러티 is developed by the 캐러티 Bot Inc. and the open source community.

COPYRIGHT
       Copyright © 2025 캐러티 Bot Inc. Licensed under the Apache License 2.0.
```
