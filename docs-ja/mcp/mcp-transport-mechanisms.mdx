...
Model Context Protocol (MCP) supports two primary transport mechanisms for communication between Careti and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (Careti) spawns an MCP server as a child process
...
-   **Locality**: Runs on the same machine as Careti
...
-   Single-client scenarios (one Careti instance per server)
...
1. The client (Careti) connects to the server's SSE endpoint via HTTP GET request
...
-   **Remote Access**: Can be hosted on a different machine from your Careti instance
...
STDIO servers run locally on the same machine as Careti, which has several important implications:
...
-   **Execution**: Starts and stops with Careti (child process lifecycle)
...
-   Start when needed by Careti
-   Not require network configuration
-   Need to be installed alongside Careti or via a package manager
...
For detailed information on configuring STDIO and SSE transports in Careti, including examples, see [Configuring MCP Servers](/ja/mcp/configuring-mcp-servers).
