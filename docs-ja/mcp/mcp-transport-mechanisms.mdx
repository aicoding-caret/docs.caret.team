...
description: "Learn about the two primary transport mechanisms for communication between Caret and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases."
---

Model Context Protocol (MCP) supports two primary transport mechanisms for communication between Caret and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (Caret) spawns an MCP server as a child process
...
-   **Locality**: Runs on the same machine as Caret
...
-   Single-client scenarios (one Caret instance per server)
...
1. The client (Caret) connects to the server's SSE endpoint via HTTP GET request
...
-   **Remote Access**: Can be hosted on a different machine from your Caret instance
...
STDIO servers run locally on the same machine as Caret, which has several important implications:
...
-   **Execution**: Starts and stops with Caret (child process lifecycle)
...
-   Start when needed by Caret
-   Not require network configuration
-   Need to be installed alongside Caret or via a package manager
...
For detailed information on configuring STDIO and SSE transports in Caret, including examples, see [Configuring MCP Servers](/ja/mcp/configuring-mcp-servers).
