...
description: "This protocol is designed to streamline the development process of building MCP servers with Caret."
---

> 🚀 **Build and share your MCP servers with the world.** Once you've created a great MCP server, submit it to the [Caret MCP Marketplace](https://github.com/caret/mcp-marketplace) to make it discoverable and one-click installable by thousands of developers.

## What Are MCP Servers?

Model Context Protocol (MCP) servers extend AI assistants like Caret by giving them the ability to:
...
The heart of effective MCP server development is following a structured protocol. This protocol is implemented through a `.agents/context` file that lives at the **root** of your MCP working directory (/Users/your-name/Documents/Caret/MCP).

### Using `.agents/context` Files

A `.agents/context` file is a special configuration that Caret reads automatically when working in the directory where it's placed. These files:

-   Configure Caret's behavior and enforce best practices
-   Switch Caret into a specialized MCP development mode
...
When this `.agents/context` file is present in your working directory, Caret will:
...
First, add a `.agents/context` file to the root of your MCP working directory using the protocol above. This file configures Caret to use the MCP development protocol when working in this folder.

### 2. Start a Chat with a Clear Description

Begin your Caret chat by clearly describing what you want to build. Be specific about:
...
Caret will automatically start in PLAN MODE, guiding you through the planning process:
...
One of the most effective ways to help Caret build your MCP server is to share official API documentation right at the start:
...
Providing comprehensive API details (endpoints, authentication, data structures) significantly improves Caret's ability to implement an effective MCP server.

## Understanding the Two Modes

### PLAN MODE

In this collaborative phase, you work with Caret to design your MCP server:
...
Once planning is complete, Caret helps implement the server:
...
Resources let your MCP servers expose data to Caret without executing code. They're perfect for providing context like files, API responses, or database records that Caret can reference during conversations.
...
Resources make your MCP servers more context-aware, allowing Caret to access specific information without requiring you to copy/paste. For more information, refer to the [official documentation](https://modelcontextprotocol.io/docs/concepts/resources).
