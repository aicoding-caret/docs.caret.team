...
4. Pick a model from the list. <BrandName /> fetches the catalog automatically. You can also paste an exact id
...
The gateway supports a large and changing set of models. <BrandName /> pulls the list from the Gateway API and caches it locally. For the current catalog, see https://vercel.com/ai-gateway/models
...
- Multi-model evals - swap only the model id in <BrandName /> and compare latency and output tokens
