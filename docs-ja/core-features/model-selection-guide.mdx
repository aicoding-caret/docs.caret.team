...
新しいモデルが継続的に登場するため、このガイドは現在 Caret に最適なモデル選択に焦点を当てています。環境の変化に合わせて更新していきます。
...
AIモデルは Caret の「頭脳」です。コード作成、バグ修正、リファクタリングを依頼したときに理解し、応答を生成する主体がモデルです。

**重要ポイント:**
- **モデルは自然言語とコードを理解するように学習されたAIシステム**
- **モデルごとに強みが異なる** 推論に強いものもあれば、速度やコスト重視のものもあります
- **Caret が使うモデルを自分で選べる** 作業タイプに合う専門家を選ぶイメージです
- **モデルはAPIプロバイダー経由で利用** Anthropic、OpenAI、OpenRouter などがモデルをホスティング

**なぜ重要?** 選んだモデルは Caret の性能、応答品質、速度、コストに直結します。高性能モデルは複雑なリファクタリングに強い一方で高価になりがちで、低コストモデルは日常作業に効率的です。
...
まず Caret の設定パネルにアクセスします。

**設定を開く2つの方法:**
- **クイック**: Caret チャットUI右上の **歯車(⚙️)** をクリック
- **コマンドパレット**: **Cmd/Ctrl + Shift + P** → "Caret: Open Settings" と入力
...
| プロバイダー | おすすめ用途 | 備考 |
|----------|----------|-------|
| **Caret** | 最も簡単な開始 | APIキー不要、stealthモデル含む複数モデルにアクセス |
...
<Info>
**初めての方におすすめ:** まず **Caret** プロバイダーから始めましょう。APIキー管理が不要で複数モデルにすぐアクセスでき、パートナープロバイダー経由で無料推論が提供されることもあります。
</Info>
...
- **APIキーは不要** で、CaretアカウントへのログインだけでOK
- 画面の案内に従って **Sign In** をクリック
- [app.caret.team](https://app.caret.team) に移動して認証
- 認証後にIDEへ戻る
...
**設定完了!** これで Caret でコーディングを始められます:

1. **Caret チャットにリクエストを入力**
...
コミュニティの利用傾向は [OpenRouter の Caret 利用統計](https://openrouter.ai/apps?url=https%3A%2F%2Fcaret.team%2F) を確認してください。
