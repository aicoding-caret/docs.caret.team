...
description: "Careti CLI で GitHub イシューを自動分析し根本原因を特定します。"
---

# GitHub 根本原因分析

<BrandName /> CLI を使った GitHub イシューの自動分析サンプルです。<BrandName /> の自律 AI によりイシューを取得・分析し、根本原因を特定します。解析結果はワークフローに組み込みやすい形式で出力されます。

<Note>
**<BrandName /> CLI が初めての方へ** このサンプルは [インストールガイド](/ja/caret-cli/installation) を完了し、`caret auth` で認証済みであることを前提にしています。
</Note>
...
- **<BrandName /> CLI** のインストールと認証 ([インストールガイド](/ja/caret-cli/installation))
- **少なくとも 1 つのモデルプロバイダー** 設定 (例: OpenRouter、Anthropic、OpenAI)
- **<BrandName /> CLI の基本操作** の理解
...
1. <BrandName /> を使った **自律 CLI ツール** の構築
2. <BrandName /> CLI の **JSON 出力解析**
