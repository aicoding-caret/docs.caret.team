...
Careti CLI を使った GitHub イシューの自動分析サンプルです。Careti の自律 AI によりイシューを取得・分析し、根本原因を特定します。解析結果はワークフローに組み込みやすい形式で出力されます。

<Note>
**Careti CLI が初めての方へ** このサンプルは [インストールガイド](/ja/caret-cli/installation) を完了し、`caret auth` で認証済みであることを前提にしています。
</Note>
...
- **Careti CLI** のインストールと認証 ([インストールガイド](/ja/caret-cli/installation))
- **少なくとも 1 つのモデルプロバイダー** 設定 (例: OpenRouter、Anthropic、OpenAI)
- **Careti CLI の基本操作** の理解
...
1. Careti を使った **自律 CLI ツール** の構築
2. Careti CLI の **JSON 出力解析**
