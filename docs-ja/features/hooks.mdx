...
title: "Hooks"
sidebarTitle: "Hooks"
description: "Careti ワークフローにカスタムロジックを注入し、操作検証やツール利用の監視、AI の判断を制御します。"
---

<Note>
キャレット(<BrandName />)基準の文書です。<BrandName /> v3.38.1 マージ版に準拠し、許可スコープや認証/ルーティング、ロギング/テレメトリ差異があれば `<Note>` で示します。
</Note>

<Note>
<BrandName /> のアカウント/組織フックは `caret-docs/features/f04-caret-account.md` に基づき、アカウント情報がフックデータに含まれる場合があります。ログ/テレメトリは `caret-src/utils/` を参照します。
</Note>

Hooks は <BrandName /> ワークフローの特定タイミングでカスタムロジックを注入し、操作の検証、ツール使用の監視、AI の意思決定の制御を可能にします。
...
<BrandName /> で Hooks を有効にする手順:

<Steps>
<Step title="設定で Hooks を有効化">
<BrandName /> 設定で **"Enable Hooks"** を有効にします。
...
<BrandName /> でタスクを開始し、Hook が実行されるか確認します。
</Step>
</Steps>
...
Hooks は <BrandName /> のワークフローと外部エコシステムをつなぐ接着剤です。

## Hook Types

<BrandName /> はさまざまなフックタイプを提供し、AI ワークフローの異なる段階にアクセスできます。
...
stdout にログを出しても構いませんが、**最後は必ず JSON** である必要があります。<BrandName /> は最後の JSON オブジェクトのみを読み取ります。
...
- 文法エラーがないか確認
- VS Code の Output(<BrandName />) を確認
...
- [<BrandName /> Rules](/ja/features/caret-rules)
