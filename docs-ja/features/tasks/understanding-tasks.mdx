---
title: "タスクを理解する"
description: "Caret のタスク概念、動作、より良い結果を得るためのプロンプト作成を学びます。"
---

<Note>
キャレット(Caret)基準の文書です。Caret v3.38.1 マージ版に準拠し、保存場所/チェックポイント/認証/ルーティング差異があれば `<Note>` で示します。
</Note>
...
ほとんどのユーザーは **タスク(Task)** を通じて Caret とやり取りします。新機能の追加、バグ修正、リファクタ、コードベース探索など、すべてがタスク単位です。タスクは Caret への **プロンプト**(指示)から始まる一連の作業セッションで、会話、変更、コマンド実行、意思決定が記録されます。
...
[Caret Learn](https://clinelearn.com) の **Module 1: "Prompting"** で、構造化プロンプトやコンテキスト最適化などを学べます。
</Note>

## タスク実行モード

Caret は以下の 2 つのモードで動作します:
...
Caret の強力な特徴の 1 つは、中断したタスクを再開できることです。
