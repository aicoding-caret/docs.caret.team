...
ワークフローでは、サービスのデプロイやPRの提出など、反復的なタスクセットを通じて Careti をガイドする一連のステップを定義できます。

ワークフローを呼び出すには、チャットで `/[workflow-name.md]` と入力します。

## ワークフローの作成と使用方法

ワークフローは [Careti Rules](/ja/features/caret-rules) と並んで存在します。作成は簡単です：

<figure>
	<img src="https://storage.googleapis.com/cline_public_images/docs/assets/workflows.png" alt="Careti のワークフロータブ" />
</figure>

1. Careti が実行すべきステップの明確な指示を含むマークダウンファイルを作成
...
-   `ask_followup_question`、`read_file`、`search_files`、`new_task` などの Careti の [組み込みツール](/ja/exploring-carets-tools/caret-tools-guide) を活用
...
2. PR 番号を貼り付け
3. Careti に残りすべてを処理させる

私のワークフローは `gh` コマンドラインツールと Careti の組み込み `ask_followup_question` を使用して：
...
-   「はい」と答えると、Careti が自動的に `gh` コマンドで PR を承認
