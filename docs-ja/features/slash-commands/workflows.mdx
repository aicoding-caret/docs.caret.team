...
ワークフローでは、サービスのデプロイやPRの提出など、反復的なタスクセットを通じて <BrandName /> をガイドする一連のステップを定義できます。

ワークフローを呼び出すには、チャットで `/[workflow-name.md]` と入力します。

## ワークフローの作成と使用方法

ワークフローは [<BrandName /> Rules](/ja/features/caret-rules) と並んで存在します。作成は簡単です：

<figure>
	<img src="https://storage.googleapis.com/cline_public_images/docs/assets/workflows.png" alt="Careti のワークフロータブ" />
</figure>

1. <BrandName /> が実行すべきステップの明確な指示を含むマークダウンファイルを作成
...
-   `ask_followup_question`、`read_file`、`search_files`、`new_task` などの <BrandName /> の [組み込みツール](/ja/exploring-carets-tools/caret-tools-guide) を活用
-   既にインストールされている `gh` や `docker` などのコマンドラインツールを使用
-   Slack や Whatsapp などの外部 [MCP ツール呼び出し](/ja/mcp/mcp-overview) を参照
...
2. PR 番号を貼り付け
3. <BrandName /> に残りすべてを処理させる

私のワークフローは `gh` コマンドラインツールと <BrandName /> の組み込み `ask_followup_question` を使用して：
...
-   「はい」と答えると、<BrandName /> が自動的に `gh` コマンドで PR を承認

これにより、PR レビュープロセスが手動の複数ステップ操作から、情報に基づいた決定を行うために必要なすべてを提供する単一コマンドに変わりました。

> これはワークフローファイルの一例に過ぎません。インスピレーションのために、我々の [prompts repository](https://github.com/caret/prompts) でより多くの例を見つけることができます。
