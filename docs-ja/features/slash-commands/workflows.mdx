...
ワークフローは [Caret Rules](/features/caret-rules) と並んで存在します。作成は簡単です：
...
-   `ask_followup_question`、`read_file`、`search_files`、`new_task` などの Caret の [組み込みツール](/exploring-clines-tools/caret-tools-guide) を活用
