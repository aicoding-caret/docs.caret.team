...
<Steps>
  ...
  <Step title="ワークフローを実行">
    ...
    1. <BrandName /> のチャットパネルを開く
    2. `/pr-review.md` の後に PR 番号を入力 (例: `/pr-review.md 42`)
    3. <BrandName /> がPR情報を取得・分析し結果を提示

    <Tip>
      `gh pr view` のようなコマンド実行後に <BrandName /> が止まる場合は、**Proceed While Running** をクリックして続行してください。
    </Tip>
  </Step>
</Steps>
...
繰り返し作業を見つけて、効率的な <BrandName /> ワークフローに変えていきましょう。
